    ...
    <div className="animate-slide-in" data-testid="run-sheet">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6 no-print">
        <div>
          ...
          <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1">Feeding · Medication · Notes · Times — for the team (and the fridge)</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={()=>window.print()} data-testid="rs-print"
                  className="bg-shGreen text-bgHeader px-5 py-2 rounded text-[10px] font-black uppercase tracking-widest shadow"><i className="fas fa-print mr-1"/>Print</button>
        </div>
      </div>
      ...
      <div className="print-card bg-bgPanel border border-bgHover rounded-xl p-6 mb-6 shadow-2xl">
        <div className="flex justify-between items-start mb-2">
          ...
          <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest text-right">{bookings.length} dogs on premises</p>
        </div>
      </div>
                  ...
                  <div key={b.id} className="print-card bg-bgPanel border border-bgHover rounded-xl p-5 shadow-lg" data-testid={`rs-card-${b.id}`}>
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-3">
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mt-1">{d.breed || "—"} · {d.sex} · {d.fixed==="Yes"?"Fixed":"Intact"}</p>
                      </div>
                      <div className="text-right text-xs">
                        <p className="text-gray-300"><span className="text-gray-500 font-black uppercase text-[9px] tracking-widest">Owner</span> {b.client_name}</p>
                        {b.client_phone && <p className="text-gray-300"><i className="fas fa-phone mr-1 text-[10px]"/>{b.client_phone}</p>}
                        {b.client_emerg && <p className="text-red-400 text-[11px]"><i className="fas fa-triangle-exclamation mr-1 text-[10px]"/>{b.client_emerg}</p>}
                      </div>
                    </div>
                    ...
                    {(d.feeding_schedule?.length > 0) && (
                      <div className="mt-4">
                        <p className="text-[10px] font-black text-shGreen uppercase tracking-widest mb-1"><i className="fas fa-bowl-food mr-1"/>Feeding</p>
                    ...
                    {(d.medications?.length > 0) && (
                      <div className="mt-3">
                        <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest mb-1"><i className="fas fa-pills mr-1"/>Medications</p>
                        <ul className="text-xs text-gray-300 space-y-1">
                          {d.medications.map(m => (
                            <li key={m.id} className="flex flex-wrap gap-2 items-baseline">
                              ...
                              {m.with_food && <span className="text-[9px] uppercase font-black tracking-widest bg-shGreen/15 text-shGreen px-2 py-0.5 rounded">With food</span>}
                    ...
                    {d.notes && <p className="mt-3 text-xs text-gray-300 bg-bgBase rounded p-2"><span className="text-shOrange font-black uppercase text-[10px] tracking-widest mr-2">Notes:</span>{d.notes}</p>}
                    {b.notes && <p className="mt-2 text-xs text-gray-300 bg-bgBase rounded p-2"><span className="text-shBlue font-black uppercase text-[10px] tracking-widest mr-2">Booking:</span>{b.notes}</p>}
                    {(d.vet_name || d.vet_phone) && <p className="mt-2 text-[11px] text-gray-400"><i className="fas fa-stethoscope mr-1 text-shBlue"/><span className="font-black text-gray-300">Vet:</span> {d.vet_name} {d.vet_phone && `· ${d.vet_phone}`}</p>}
                  </div>
                );
              })}
            </div>
