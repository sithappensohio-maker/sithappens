      ...
      <div className="relative bg-bgPanel border border-bgHover rounded-2xl w-full max-w-md p-10 shadow-2xl animate-slide-in">
        ...
        <div className="flex gap-2 mb-6 bg-bgBase rounded-lg p-1">
          <button onClick={() => setMode("login")} data-testid="tab-login"
                  className={`flex-1 py-2 rounded text-[10px] font-black uppercase tracking-widest ${mode==="login"?"bg-shBlue text-white":"text-gray-400"}`}>Sign In</button>
          <button onClick={() => setMode("register")} data-testid="tab-register"
                  className={`flex-1 py-2 rounded text-[10px] font-black uppercase tracking-widest ${mode==="register"?"bg-shBlue text-white":"text-gray-400"}`}>Register</button>
        </div>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {error && <div data-testid="login-error" className="text-[11px] text-red-400 bg-red-500/10 rounded p-3 uppercase font-black">{error}</div>}
          <button type="submit" disabled={loading} data-testid="login-submit-button"
                  className="w-full bg-shBlue text-white py-3 rounded font-black text-[11px] uppercase tracking-widest shadow-lg hover:bg-shBlue/90 disabled:opacity-50">
