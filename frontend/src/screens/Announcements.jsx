...
export default function Announcements({ openCreateOnMount = false, onCreateConsumed = () => {} }) {
  ...
  return (
    <div className="space-y-6" data-testid="admin-announcements-screen">
      ...
      {/* Composer */}
      {editing !== null && (
        <div className="bg-[var(--sh-card-base)] border border-shPrimary/40 rounded-xl p-5 shadow-2xl space-y-4" data-testid="ann-composer">
          ...
          <div>
            <label className="text-[11px] font-black text-shTextMuted uppercase tracking-widest block mb-1">Title *</label>
            <input value={form.title} onChange={(e)=>setForm({...form, title: e.target.value})}
                   placeholder="e.g. Closed Thanksgiving — book now"
                   maxLength={180} data-testid="ann-title"
                   className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm focus:border-shPrimary outline-none"/>
          </div>

          <div>
            <label className="text-[11px] font-black text-shTextMuted uppercase tracking-widest block mb-1">Body</label>
          ...
          <div>
            <label className="text-[11px] font-black text-shTextMuted uppercase tracking-widest block mb-1">Optional image</label>
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {form.image && (
                <>
                  <img src={form.image} alt="preview" className="w-20 h-20 object-cover rounded border border-shBorder"/>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.pinned} onChange={(e)=>setForm({...form, pinned: e.target.checked})}
                     data-testid="ann-pinned" className="accent-shPrimary w-4 h-4"/>
              <span className="text-[12px] font-black uppercase tracking-widest text-shText">
                <i className="fas fa-thumbtack mr-1 text-shAccent"/>Pin to top
              </span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.published} onChange={(e)=>setForm({...form, published: e.target.checked})}
                     data-testid="ann-published" className="accent-shPrimary w-4 h-4"/>
              <span className="text-[12px] font-black uppercase tracking-widest text-shText">
                Visible to clients
              </span>
            </label>
            <div>
              <label className="text-[10px] font-black text-shTextMuted uppercase tracking-widest block mb-1">Auto-hide after</label>
          ...
          {editing === "new" && form.published && (
            <p className="text-[11px] text-shPrimary bg-shPrimary/10 border border-shPrimary/30 rounded px-3 py-2"
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-shBorder">
            <button onClick={cancel} disabled={saving}
                    className="text-[13px] font-black uppercase tracking-widest px-3 py-2 text-shTextMuted hover:text-shText disabled:opacity-40"
                    data-testid="ann-cancel">Cancel</button>
            <button onClick={save} disabled={saving} data-testid="ann-save"
                    className="text-[13px] font-black uppercase tracking-widest px-5 py-2 rounded bg-shPrimary text-bgHeader hover:bg-shPrimary/90 disabled:opacity-40 transition">
      ...
      {/* List */}
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl shadow-xl divide-y divide-shBorder" data-testid="ann-list">
        {loading && <p className="p-5 text-[13px] text-shTextMuted"><i className="fas fa-spinner fa-spin mr-2"/>Loading…</p>}
        {!loading && items.length === 0 && (
          <div className="p-8 text-center">
            <i className="fas fa-bullhorn text-3xl text-gray-600 mb-3"/>
            <p className="text-[14px] text-shTextMuted">No announcements posted yet.</p>
            <p className="text-[12px] text-shTextMuted mt-1">Click <span className="text-shPrimary font-black">&quot;New Announcement&quot;</span> to broadcast your first update.</p>
          </div>
        )}
        {!loading && items.map((a) => (
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  {a.pinned && (
                    <span className="text-[9px] font-black uppercase tracking-widest text-shAccent bg-shAccent/15 border border-shAccent/30 px-1.5 py-0.5 rounded">
                      <i className="fas fa-thumbtack mr-1"/>Pinned
                    </span>
                  )}
                  {a.published === false && (
                    <span className="text-[9px] font-black uppercase tracking-widest text-shTextMuted bg-[var(--sh-card-base)] border border-shBorder px-1.5 py-0.5 rounded">
                      Hidden
                    </span>
                  )}
                  <p className="text-[10px] text-shTextMuted uppercase tracking-widest">
                    {fmtDate(a.created_at)}{a.created_by ? ` · ${a.created_by}` : ""}
                  </p>
                </div>
                <h4 className="text-base sm:text-lg font-black text-shText uppercase italic tracking-tight">{a.title}</h4>
              ...
              {a.image && (
                <img src={a.image} alt={a.title}
                     className="w-24 h-24 object-cover rounded border border-shBorder shrink-0"/>
              )}
              <div className="flex gap-2 shrink-0">
                <button onClick={()=>startEdit(a)} data-testid={`ann-edit-${a.id}`}
                        className="text-[11px] font-black uppercase tracking-widest text-shSecondary hover:underline px-2 py-1">
