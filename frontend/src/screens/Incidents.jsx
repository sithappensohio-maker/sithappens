    ...
    <div className="space-y-6 animate-slide-in" data-testid="incidents-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1">Permanent legal record · all bite/injury/escape events</p>
        </div>
        <button onClick={openNew} data-testid="add-incident-button"
                className="bg-red-500 text-white px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-red-500/90">+ Log Incident</button>
      </div>

      <div className="flex flex-wrap gap-2">
        <button onClick={()=>setFilter("all")} className={`px-3 py-1.5 rounded text-[10px] font-black uppercase tracking-widest ${filter==="all"?"bg-shBlue text-white":"bg-bgPanel text-gray-400 border border-bgHover"}`}>All · {incidents.length}</button>
        {TYPES.map(t => {
          const n = incidents.filter(i=>i.type===t.key).length;
          return <button key={t.key} onClick={()=>setFilter(t.key)} className={`px-3 py-1.5 rounded text-[10px] font-black uppercase tracking-widest ${filter===t.key?"bg-shBlue text-white":t.color}`}>{t.label} · {n}</button>;
        })}
      </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <span className={`text-[10px] font-black uppercase px-2 py-1 rounded tracking-widest ${typeStyle(i.type)}`}>{TYPES.find(t=>t.key===i.type)?.label || i.type}</span>
                  <span className={`text-[10px] font-black uppercase px-2 py-1 rounded tracking-widest ${sevStyle(i.severity)}`}>{i.severity}</span>
                  {i.vet_required && <span className="text-[10px] font-black uppercase px-2 py-1 rounded tracking-widest bg-purple-500/15 text-purple-400"><i className="fas fa-stethoscope mr-1"/>Vet</span>}
                  {i.follow_up_required && <span className="text-[10px] font-black uppercase px-2 py-1 rounded tracking-widest bg-shOrange/15 text-shOrange"><i className="fas fa-flag mr-1"/>Follow-up</span>}
                </div>
                ...
                <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1">{i.date}{i.time?` · ${i.time}`:""} · reported by {i.reported_by}</p>
                <p className="text-sm text-gray-300 mt-3 whitespace-pre-wrap">{i.description}</p>
                {i.action_taken && <p className="text-xs text-gray-400 mt-2"><span className="text-shBlue font-black uppercase tracking-widest text-[10px]">Action: </span>{i.action_taken}</p>}
                {i.witnesses && <p className="text-xs text-gray-400 mt-1"><span className="text-shBlue font-black uppercase tracking-widest text-[10px]">Witnesses: </span>{i.witnesses}</p>}
                {i.photos?.length > 0 && (
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-bgPanel border border-bgHover rounded-2xl w-full max-w-2xl p-6 md:p-8 shadow-2xl max-h-[95vh] overflow-y-auto animate-slide-in">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2 mt-1">
                  {TYPES.map(t => (
                    <button key={t.key} onClick={()=>setForm({...form, type:t.key})} data-testid={`incident-type-${t.key}`}
                            className={`py-2 px-2 rounded text-[10px] font-black uppercase tracking-widest border ${form.type===t.key?"bg-shBlue text-white border-shBlue":"bg-bgBase border-bgHover text-gray-400"}`}>
                      {t.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Severity</label>
                <div className="grid grid-cols-3 gap-2 mt-1">
                  {SEVERITIES.map(s => (
                    <button key={s.key} onClick={()=>setForm({...form, severity:s.key})} data-testid={`incident-severity-${s.key}`}
                            className={`py-2 rounded text-[10px] font-black uppercase tracking-widest border ${form.severity===s.key?"bg-shBlue text-white border-shBlue":"bg-bgBase border-bgHover text-gray-400"}`}>
                      {s.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">What happened? (required)</label>
              ...
              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Action Taken</label>
              ...
              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Witnesses</label>
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-300">Vet required</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-300">Needs follow-up</span>
                </label>
              </div>

              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Photos (up to 4)</label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={save} data-testid="save-incident-button"
                        className="bg-red-500 text-white px-8 py-3 rounded font-black text-[10px] uppercase tracking-widest shadow-xl">Save Incident Report</button>
              </div>
            </div>
          </div>
        </div>
