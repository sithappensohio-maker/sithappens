...
const TYPES = [
  ...
  { key: "injury", label: "Injury", color: "bg-shAccent/15 text-shAccent" },
  ...
  { key: "resource_guarding", label: "Resource", color: "bg-shAccent/15 text-shAccent" },
  { key: "reactivity", label: "Reactivity", color: "bg-shSecondary/15 text-shSecondary" },
  ...
  { key: "property_damage", label: "Property", color: "bg-gray-500/15 text-shTextMuted" },
  { key: "other", label: "Other", color: "bg-shSurfaceRaised text-shTextMuted" },
];
const SEVERITIES = [
  // Sprint 110ev — expanded tiers (Phase 5)
  { key: "low", label: "Low", color: "bg-shPrimary/15 text-shPrimary" },
  { key: "medium", label: "Medium", color: "bg-shAccent/15 text-shAccent" },
...
export default function Incidents() {
  ...
  const typeStyle = (k) => TYPES.find(t=>t.key===k)?.color || "bg-shSurfaceRaised text-shTextMuted";
  const sevStyle = (k) => SEVERITIES.find(s=>s.key===k)?.color || "bg-shSurfaceRaised text-shTextMuted";
  ...
  return (
    <div className="space-y-6 animate-slide-in" data-testid="incidents-screen">
      <PageHero
        ...
        right={(
          <button onClick={openNew} data-testid="add-incident-button"
                  className="bg-red-500 text-shText px-5 py-2.5 rounded-lg text-[13px] font-black uppercase tracking-widest shadow-lg hover:bg-red-500/90 transition">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={()=>setFilter("all")} className={`px-3 py-1.5 rounded text-[14px] font-black uppercase tracking-widest ${filter==="all"?"bg-shSecondary text-shText":"bg-[var(--sh-card-base)] text-shTextMuted border border-shBorder"}`}>All · {incidents.length}</button>
        {TYPES.map(t => {
          const n = incidents.filter(i=>i.type===t.key).length;
          return <button key={t.key} onClick={()=>setFilter(t.key)} className={`px-3 py-1.5 rounded text-[14px] font-black uppercase tracking-widest ${filter===t.key?"bg-shSecondary text-shText":t.color}`}>{t.label} · {n}</button>;
        })}
      </div>
          ...
          <div key={i.id} className="card-warning rounded-xl p-5 shadow-lg" data-testid={`incident-${i.id}`}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-shTextMuted mt-3 whitespace-pre-wrap">{i.description}</p>
                {i.action_taken && <p className="text-xs text-shTextMuted mt-2"><span className="text-shSecondary font-black uppercase tracking-widest text-[14px]">Action: </span>{i.action_taken}</p>}
                {i.witnesses && <p className="text-xs text-shTextMuted mt-1"><span className="text-shSecondary font-black uppercase tracking-widest text-[14px]">Witnesses: </span>{i.witnesses}</p>}
                {Array.isArray(i.staff_involved) && i.staff_involved.length > 0 && (
                  <p className="text-xs text-shTextMuted mt-1"><span className="text-shSecondary font-black uppercase tracking-widest text-[14px]">Staff involved: </span>{i.staff_involved.join(", ")}</p>
                )}
                {i.internal_notes && (
                  <p className="text-xs text-shTextMuted mt-2 bg-[var(--sh-card-base)]/60 border-l-2 border-shAccent/40 pl-2 py-1">
                    <span className="text-shAccent font-black uppercase tracking-widest text-[14px]">Internal: </span>{i.internal_notes}
                  </p>
                )}
                {i.photos?.length > 0 && (
                  <div className="flex gap-2 mt-3 flex-wrap">
                    {i.photos.map((p,idx)=><img key={idx} src={p} alt="" loading="lazy" decoding="async" className="h-20 w-20 rounded object-cover border border-shBorder" />)}
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl w-full max-w-2xl p-6 md:p-8 shadow-2xl max-h-[calc(var(--app-height)_-_1rem)] overflow-y-auto animate-slide-in">
            <div className="flex items-center justify-between mb-5">
              <h4 className="text-xl font-black text-shText uppercase italic tracking-tight">{editing?"Edit Incident":"Log Incident"}</h4>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" value={form.date} onChange={(e)=>setForm({...form, date:e.target.value})} className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm" style={{colorScheme:"dark"}} />
                </div>
                <div>
                  <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Time</label>
                  <input type="time" value={form.time} onChange={(e)=>setForm({...form, time:e.target.value})} className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm" style={{colorScheme:"dark"}} />
                </div>
              </div>

              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Type</label>
                <div className="grid grid-cols-3 gap-2 mt-1">
                  {TYPES.map(t => (
                    <button key={t.key} onClick={()=>setForm({...form, type:t.key})} data-testid={`incident-type-${t.key}`}
                            className={`py-2 px-2 rounded text-[14px] font-black uppercase tracking-widest border ${form.type===t.key?"bg-shSecondary text-shText border-shSecondary":"bg-[var(--sh-card-base)] border-shBorder text-shTextMuted"}`}>
                      {t.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Severity</label>
                <div className="grid grid-cols-3 gap-2 mt-1">
                  {SEVERITIES.map(s => (
                    <button key={s.key} onClick={()=>setForm({...form, severity:s.key})} data-testid={`incident-severity-${s.key}`}
                            className={`py-2 rounded text-[14px] font-black uppercase tracking-widest border ${form.severity===s.key?"bg-shSecondary text-shText border-shSecondary":"bg-[var(--sh-card-base)] border-shBorder text-shTextMuted"}`}>
                      {s.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">What happened? (required)</label>
                <textarea value={form.description} onChange={(e)=>setForm({...form, description:e.target.value})} rows={4} placeholder="Detailed account of the incident…" data-testid="incident-description"
                          className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-shText text-sm focus:border-shSecondary outline-none" />
              </div>

              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Action Taken</label>
              ...
              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Witnesses</label>
              ...
              <div className="flex gap-4 flex-wrap">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-[14px] font-black uppercase tracking-widest text-shTextMuted">Vet required</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={!!form.follow_up_required} onChange={(e)=>setForm({...form, follow_up_required:e.target.checked})} data-testid="incident-followup" className="accent-shAccent w-4 h-4" />
                  <span className="text-[14px] font-black uppercase tracking-widest text-shTextMuted">Needs follow-up</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={!!form.manager_reviewed} onChange={(e)=>setForm({...form, manager_reviewed:e.target.checked})} data-testid="incident-manager-reviewed" className="accent-shPrimary w-4 h-4" />
                  <span className="text-[14px] font-black uppercase tracking-widest text-shTextMuted">Manager reviewed</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={!!form.client_notified} onChange={(e)=>setForm({...form, client_notified:e.target.checked})} data-testid="incident-client-notified" className="accent-shSecondary w-4 h-4" />
                  <span className="text-[14px] font-black uppercase tracking-widest text-shTextMuted">Client notified</span>
                </label>
              </div>

              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Staff involved (comma-separated)</label>
              ...
              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Internal notes (not shown to client)</label>
              ...
              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Photos (up to 4)</label>
                <div className="mt-2 flex flex-wrap gap-2">
                  {form.photos.map((p, i) => (
                    <div key={i} className="relative">
                      <img src={p} alt="" loading="lazy" decoding="async" className="h-20 w-20 rounded object-cover border border-shBorder" />
                      <button onClick={()=>setForm({...form, photos: form.photos.filter((_,j)=>j!==i)})} className="absolute -top-2 -right-2 bg-red-500 text-shText rounded-full w-5 h-5 text-xs">×</button>
                    </div>
                  ))}
                  {form.photos.length < 4 && (
                    <label className="h-20 w-20 rounded border-2 border-dashed border-shBorder flex items-center justify-center cursor-pointer hover:border-shSecondary text-shTextMuted hover:text-shSecondary">
              ...
              {editing && (
                <div>
                  <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Reason for this edit</label>
                  ...
                  {editing.edit_history?.length > 0 && (
                    <div className="mt-2 text-[12px] text-shTextMuted space-y-1 max-h-24 overflow-y-auto">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={save} data-testid="save-incident-button"
                        className="bg-red-500 text-shText px-8 py-3 rounded font-black text-[14px] uppercase tracking-widest shadow-xl">Save Incident Report</button>
              </div>
            </div>
          </div>
        </div>
