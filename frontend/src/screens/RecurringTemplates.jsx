...
export default function RecurringTemplates() {
  ...
  return (
    <div className="space-y-6" data-testid="recurring-screen">
      <PageHero
        eyebrow={{ icon: "fa-rotate", text: "Weekly schedules", color: "text-shSecondary" }}
        title="Recurring."
      ...
      {toast && (
        <div data-testid="recurring-toast"
             className={`rounded-lg p-3 text-[14px] font-black uppercase tracking-widest ${toast.ok ? "bg-shPrimary/15 text-shPrimary border border-shPrimary/40" : "bg-red-500/15 text-red-400 border border-red-500/40"}`}>
            ...
            <div key={r.id} data-testid={`recurring-row-${r.id}`}
              ...
              <div className="col-span-6 md:col-span-3">
                <div className="flex flex-wrap gap-1">
                  {WD.map((d, i) => (
                    <span key={i} className={`text-[12px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded ${r.weekdays?.includes(i) ? (r.service_type === "training" ? "bg-purple-500/25 text-purple-300" : "bg-shSecondary/25 text-shSecondary") : "bg-shSurfaceRaised text-gray-600"}`}>{d}</span>
                  ))}
                </div>
                <p className="text-[12px] text-shTextMuted font-black uppercase tracking-widest mt-1">{r.service_name || r.service_type} · {r.default_horizon_weeks}w default</p>
              </div>
              <div className="col-span-6 md:col-span-3 text-[14px] text-shTextMuted font-black uppercase tracking-widest">
              ...
              <div className="col-span-12 md:col-span-2 flex md:justify-end gap-2">
                <button onClick={()=>extend(r)} disabled={busy===r.id || !r.active} data-testid={`extend-btn-${r.id}`}
                        className="bg-shPrimary text-bgHeader px-3 py-1.5 rounded text-[13px] font-black uppercase tracking-widest hover:bg-shPrimary/80 disabled:opacity-40">
      ...
      {open && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={()=>setOpen(false)}>
          <div className="bg-[var(--sh-card-base)] rounded-xl shadow-2xl border border-shBorder max-w-md w-full p-6" onClick={e=>e.stopPropagation()} data-testid="recurring-modal">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-2xl font-black uppercase italic text-shText tracking-tight">{editing ? "Edit Schedule" : "New Recurring Schedule"}</h2>
              <button onClick={()=>setOpen(false)} className="text-shTextMuted hover:text-shText"><i className="fas fa-xmark text-xl"/></button>
            </div>
            <div className="space-y-4">
              ...
              {form.service_type === "training" ? (
                <div>
                  <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Appointment time</label>
                ...
                <div>
                  <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Usual drop-off time (optional)</label>
              ...
              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Weekdays</label>
                <div className="flex gap-1 mt-1.5">
                  {WD.map((d, i) => (
                    <button key={i} type="button" onClick={()=>toggleDay(i)}
                            data-testid={`weekday-${i}`}
                            className={`flex-1 py-2 rounded text-[13px] font-black uppercase tracking-widest transition ${form.weekdays.includes(i) ? "bg-shSecondary text-shText" : "bg-[var(--sh-card-base)] text-shTextMuted hover:bg-shSurfaceRaised"}`}>
                      {d}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Start on</label>
                <input type="date" value={form.start_date}
                       onChange={(e)=>setForm({...form, start_date: e.target.value})}
                       data-testid="template-start-date"
                       style={{colorScheme:"dark"}}
                       className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm"/>
                <p className="text-[13px] text-shTextMuted normal-case mt-1">Leave blank to start today. Useful for "starts next month" patterns. After the first extend, future ones pick up where the last one left off automatically.</p>
              </div>
              <div>
                <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Default extend horizon (weeks)</label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={save} data-testid="save-template-btn"
                        className="bg-shSecondary text-shText px-6 py-2 rounded font-black text-[15px] uppercase tracking-widest hover:bg-shSecondary/90">
