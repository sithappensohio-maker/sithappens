    ...
    <div className="h-full flex flex-col bg-bgBase" data-testid="client-portal">
      <header className="bg-bgHeader border-b border-bgHover h-24 flex items-center justify-between px-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[13px] text-gray-500 font-black uppercase tracking-[0.25em]">Dog Training • Daycare • Boarding</p>
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-6xl mx-auto w-full grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="col-span-1 space-y-6">
          <div className="bg-bgPanel p-6 rounded-xl border border-bgHover text-center shadow-2xl" data-testid="credits-card">
            <p className="text-[12px] text-gray-400 font-black uppercase tracking-widest">Available Credits</p>
            <p className="text-5xl font-black text-shGreen mt-2">{credits}</p>
            <p className="text-[12px] text-gray-500 font-black uppercase tracking-widest mt-2">Each day = 1 credit</p>
          </div>
          ...
          <div className={`p-5 rounded-xl border shadow-2xl ${waiverNeeded?"bg-red-500/10 border-red-500/40":"bg-shGreen/5 border-shGreen/30"}`} data-testid="waiver-status-card">
            <div className="flex items-center justify-between mb-2">
              <p className={`text-[12px] font-black uppercase tracking-widest ${waiverNeeded?"text-red-400":"text-shGreen"}`}>
                <i className={`fas ${waiverNeeded?"fa-exclamation-triangle":"fa-file-signature"} mr-2`} /> Client Waiver
              </p>
              {waiver?.signed && !waiver?.needs_resign && <span className="text-[13px] text-gray-500 font-black uppercase tracking-widest">v{waiver.signature?.waiver_version}</span>}
            </div>
            {waiverNeeded ? (
              <>
                ...
                <button onClick={()=>setShowWaiver(true)} data-testid="open-waiver-button"
                        className="w-full bg-red-500 text-white py-2 rounded font-black text-[12px] uppercase tracking-widest hover:bg-red-500/90">
                  Sign Waiver Now
                </button>
              </>
            ) : (
          ...
          <div className="bg-bgPanel p-6 rounded-xl border border-bgHover shadow-2xl">
            ...
            <div className="grid grid-cols-3 gap-2 mt-1 mb-3">
              {["daycare","boarding","training"].map(t => (
                <button key={t} onClick={()=>{ setBookType(t); if(t==="boarding") setIsRecurring(false); }} data-testid={`book-service-${t}`}
                        className={`py-2 rounded text-[12px] font-black uppercase tracking-widest ${bookType===t?"bg-shBlue text-white":"bg-bgBase border border-bgHover text-gray-400"}`}>{t}</button>
              ))}
            </div>
            ...
            {bookType !== "boarding" && (
              <label className="flex items-center gap-2 mb-3 cursor-pointer">
                ...
                <span className="text-[12px] font-black uppercase tracking-widest text-gray-300">Recurring booking</span>
              </label>
            )}

            <label className="text-[12px] font-black text-gray-500 uppercase tracking-widest">{bookType==="boarding"?"Start Date":isRecurring?"Recurrence Start":"Date"}</label>
            ...
            {bookType==="boarding" && <>
              <label className="text-[12px] font-black text-gray-500 uppercase tracking-widest">End Date</label>
            ...
            {isRecurring && bookType!=="boarding" && <>
              <label className="text-[12px] font-black text-gray-500 uppercase tracking-widest">Repeat Until</label>
              <input type="date" value={recEnd} onChange={(e)=>setRecEnd(e.target.value)} data-testid="rec-end"
                     className="w-full mt-1 mb-3 bg-bgBase border border-bgHover rounded p-2 text-white text-xs" style={{colorScheme:"dark"}} />
              <label className="text-[12px] font-black text-gray-500 uppercase tracking-widest">Repeat On</label>
              <div className="grid grid-cols-7 gap-1 mt-1 mb-3">
                {["M","T","W","T","F","S","S"].map((d,i)=>(
                  <button key={i} onClick={()=>toggleRecDay(i)} data-testid={`rec-day-${i}`}
                          className={`py-2 rounded text-[12px] font-black uppercase ${recDays.includes(i)?"bg-shGreen text-bgHeader":"bg-bgBase border border-bgHover text-gray-400"}`}>{d}</button>
                ))}
              </div>
            </>}

            {avail && (
              <div className={`text-[12px] font-black p-3 rounded uppercase text-center tracking-widest mb-3 ${!avail.vaccine_ok?"bg-red-500/20 text-red-400":avail.open_slots<=0?"bg-shOrange/20 text-shOrange":"bg-shGreen/10 text-shGreen"}`} data-testid="availability-message">
            ...
            {err && <div className="text-[12px] font-black p-3 rounded uppercase text-center tracking-widest mb-3 bg-red-500/15 text-red-400">{err}</div>}
            {success && <div className="text-[12px] font-black p-3 rounded uppercase text-center tracking-widest mb-3 bg-shGreen/15 text-shGreen">{success}</div>}

            <button onClick={book} disabled={!canBook} data-testid="portal-book-button"
                    className={`w-full py-3 rounded font-black uppercase text-[12px] tracking-widest shadow-lg ${canBook?"bg-shBlue text-white hover:bg-shBlue/90":"bg-bgBase text-gray-500 cursor-not-allowed border border-bgHover"}`}>
              Book Now
            </button>
          </div>
        </div>
              ...
              {dogs.map(d => (
                <div key={d.id} className="bg-bgPanel rounded-xl border border-bgHover overflow-hidden shadow-lg">
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-[12px] text-shBlue font-black uppercase tracking-widest">{d.breed || "Unknown"}</p>
                    <p className="text-[12px] text-gray-400 mt-2">Rabies: <span className={d.vaccines?.rabies && d.vaccines.rabies>=todayISO()?"text-shGreen":"text-red-400"}>{d.vaccines?.rabies||"Missing"}</span></p>
                  </div>
                </div>
              ))}
            </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <span className={`text-[12px] font-black uppercase px-2 py-0.5 rounded tracking-widest ${h.status==="completed"?"bg-shGreen/15 text-shGreen":"bg-shOrange/15 text-shOrange"}`}>{h.status}</span>
                          <span className="text-[12px] text-shBlue font-black uppercase tracking-widest">{h.dog_name}</span>
                          {h.due_date && <span className="text-[12px] text-gray-400 font-black uppercase tracking-widest">Due {h.due_date}</span>}
                        </div>
                        ...
                        {h.video_url && <a href={h.video_url} target="_blank" rel="noreferrer" className="inline-block mt-2 text-[12px] text-shBlue hover:underline font-black uppercase tracking-widest"><i className="fas fa-play mr-1"/>Watch demo</a>}
                      </div>
                      {h.status !== "completed" && (
                        <button onClick={()=>{setHwModal(h); setHwNote(""); setHwPhoto("");}} data-testid={`portal-complete-${h.id}`}
                                className="shrink-0 bg-shGreen text-bgHeader px-4 py-2 rounded font-black uppercase text-[12px] tracking-widest hover:bg-shGreen/90">Mark Done</button>
                      )}
                    </div>
                ...
                <div key={b.id} className="bg-bgPanel border border-bgHover rounded-xl overflow-hidden">
                  <div className="flex items-center justify-between p-4">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-[12px] font-black uppercase px-2 py-1 rounded ${b.status==="approved"?"bg-shGreen/15 text-shGreen":b.status==="pending"?"bg-shOrange/15 text-shOrange":b.status==="rejected"?"bg-red-500/15 text-red-400":b.status==="completed"?"bg-shBlue/15 text-shBlue":"bg-gray-500/15 text-gray-400"}`}>{b.status}</span>
                      {(b.status==="pending"||b.status==="approved") && <button onClick={()=>cancel(b.id)} className="text-[12px] font-black uppercase text-red-400 hover:underline tracking-widest">Cancel</button>}
                    </div>
                  </div>
                  {b.report_card && (
                    <div className="border-t border-bgHover/50 bg-gradient-to-br from-shGreen/5 to-shBlue/5 p-4" data-testid={`report-card-${b.id}`}>
                      <p className="text-[12px] font-black text-shGreen uppercase tracking-widest mb-3"><i className="fas fa-paw mr-1"/> Pup Report Card</p>
                      ...
                      {b.report_card.mood_tags?.length > 0 && (
                        <div className="flex flex-wrap gap-1 mb-3">
                          {b.report_card.mood_tags.map(m => (
                            <span key={m} className="text-[13px] font-black uppercase tracking-widest bg-shGreen/15 text-shGreen px-2 py-1 rounded-full">{m}</span>
                          ))}
                        </div>
                      )}
      ...
      {hwModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-bgPanel border border-bgHover rounded-2xl w-full max-w-md p-6 md:p-8 shadow-2xl animate-slide-in">
            ...
            <p className="text-[12px] text-shBlue font-black uppercase tracking-widest mb-4">{hwModal.title}</p>
            <div className="space-y-4">
              <div>
                <label className="text-[12px] font-black text-gray-500 uppercase tracking-widest">How did it go? (optional)</label>
              ...
              <div>
                <label className="text-[12px] font-black text-gray-500 uppercase tracking-widest">Proof Photo (optional)</label>
              ...
              <div className="flex justify-end gap-3">
                <button onClick={()=>setHwModal(null)} className="text-gray-500 font-black uppercase text-[12px] tracking-widest">Cancel</button>
                <button onClick={completeHw} data-testid="hw-complete-button" className="bg-shGreen text-bgHeader px-8 py-3 rounded font-black text-[12px] uppercase tracking-widest shadow-xl">Mark Complete</button>
              </div>
            </div>
          </div>
        </div>
