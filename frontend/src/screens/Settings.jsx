...
import AdminPhotographyPanel from "../components/AdminPhotographyPanel";
  ...
  return (
    <div className="animate-slide-in space-y-5 sh-settings-workspace" data-testid="settings-screen">
      <PageHero
        ...
        right={msg ? (<span className={`text-[12px] font-black uppercase tracking-widest px-3 py-2 rounded ${msg==="Saved"?"bg-shPrimary/15 text-shPrimary border border-shPrimary/30":"bg-red-500/15 text-red-400 border border-red-500/30"}`}>{msg==="Saved"&&<i className="fas fa-check mr-1"/>}{msg}</span>) : null}
      ...
      <div className="sh-settings-commandbar flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
        <div className="flex items-center gap-2 text-[13px] font-black uppercase tracking-widest min-w-0 flex-wrap" data-testid="settings-breadcrumb">
          <button
            ...
            className="md:hidden bg-shSurfaceRaised/60 border border-shBorder rounded px-3 py-1.5 text-shSecondary"
            aria-label="Toggle settings nav"
          >
            <i className={`fas ${mobileNavOpen ? "fa-xmark" : "fa-bars"}`}/>
          </button>
          <i className="fas fa-sliders text-shSecondary text-[11px]"/>
          <span className="text-shTextMuted">Settings</span>
          <i className="fas fa-chevron-right text-[9px] text-gray-600"/>
          <span className="text-shText">{activeCategory.label}</span>
          {activeSub && (
            <>
              <i className="fas fa-chevron-right text-[9px] text-gray-600"/>
              <span className="text-shSecondary">{activeSub.label}</span>
            </>
          )}
        </div>
        <div className="flex-1 md:max-w-md md:ml-auto relative">
          <i className="fas fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-shTextMuted text-[12px]"/>
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Search every setting…"
            data-testid="settings-search"
            className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded-lg pl-9 pr-3 py-2 text-[14px] text-shText focus:border-shSecondary/60 outline-none"
          />
          {searchHits.length > 0 && (
            <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded-lg shadow-2xl max-h-80 overflow-y-auto" data-testid="settings-search-results">
              {searchHits.slice(0, 12).map(s => (
                <button
                  key={s.id}
                  onClick={() => { openSub(s); setSearch(""); }}
                  data-testid={`settings-search-hit-${s.id}`}
                  className="w-full text-left px-3 py-2 hover:bg-shSurfaceRaised/60 border-b border-shBorder/30 last:border-b-0"
                >
                  <div className="flex items-center gap-2">
                    <i className={`fas ${s.icon} text-shSecondary text-[12px] w-4`}/>
                    <span className="text-[14px] font-black uppercase tracking-widest text-shText">{s.label}</span>
                    <span className="text-[11px] text-shTextMuted normal-case tracking-normal ml-auto">{s._cat.label}</span>
                  </div>
                  <p className="text-[12px] text-shTextMuted normal-case mt-0.5 ml-6">{s.desc}</p>
                </button>
              ))}
            </div>
          )}
            ...
            return (
              <button
                ...
                className={`w-full text-left rounded-xl px-3 py-2.5 transition border-l-4 ${
                  isActive
                    ? "bg-[var(--sh-card-base)] border-shSecondary shadow-md"
                    : "bg-[var(--sh-card-base)]/40 border-transparent hover:bg-[var(--sh-card-base)]/70 hover:border-shSecondary/40"
                }`}
              >
                <div className="flex items-center gap-2.5">
                  <i className={`fas ${cat.icon} text-[14px] ${isActive ? "text-shPrimary" : "text-shSecondary/70"}`}/>
                  <span className={`text-[13px] font-black uppercase tracking-[0.18em] ${isActive ? "text-shText" : "text-shTextMuted"}`}>{cat.label}</span>
                </div>
                <p className="text-[12px] text-shTextMuted normal-case tracking-normal mt-1 ml-6 leading-snug">{cat.blurb}</p>
              </button>
            );
          })}
        </nav>
          ...
          {isOverview ? (
            ...
            <>
              ...
              <button
                ...
                className="text-[12px] font-black uppercase tracking-widest text-shSecondary hover:text-shPrimary transition mb-4 inline-flex items-center gap-1"
              >
              ...
              {tab === "account" && (
                <div className="space-y-5 max-w-md" data-testid="account-panel">
                  ...
                  <div className="border-t border-shBorder pt-5 space-y-3">
                    ...
                    {pwMsg && <div className={`text-[14px] font-black uppercase tracking-widest p-2 rounded ${pwMsg==="Password updated"?"bg-shPrimary/15 text-shPrimary":"bg-red-500/15 text-red-400"}`}>{pwMsg}</div>}
                    <button onClick={changePw} data-testid="save-password" className="bg-shSecondary text-shText px-6 py-2 rounded font-black text-[14px] uppercase tracking-widest shadow">Update Password</button>
                  </div>
                </div>
              )}
            </>
...
function CategoryOverview({ category, onOpen, showPlanned = false }) {
  ...
  return (
    <div className="space-y-5" data-testid={`category-overview-${category.id}`}>
      <div>
        <h2 className="text-2xl font-black text-shText uppercase tracking-wider">{category.label}</h2>
    ...
    >
      <div className="flex items-start gap-3">
        <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-[16px] shrink-0 ${
          isComing ? "bg-shSurfaceRaised/40 text-shTextMuted" : "bg-shSecondary/15 text-shSecondary"
        }`}>
          <i className={`fas ${sub.icon}`}/>
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className={`text-[15px] font-black uppercase tracking-widest ${isComing ? "text-shTextMuted" : "text-shText"}`}>{sub.label}</h3>
            {(sub.badges || []).map(b => <Badge key={b} label={b} />)}
          </div>
          <p className="text-[13px] text-shTextMuted mt-1 normal-case leading-snug">{sub.desc}</p>
          {sub.note && (
            <p className="text-[11px] text-shAccent mt-1.5 normal-case italic">
              <i className="fas fa-circle-info mr-1"/>{sub.note}
            </p>
          )}
        </div>
        {!isComing && (
          <i className="fas fa-chevron-right text-shTextMuted self-center"/>
        )}
      </div>
    </button>
  );
...
const FEATURE_VISIBILITY_META = [
  // service features
  { id: "daycare",         label: "Daycare",         icon: "fa-sun",            color: "shPrimary",
  ...
  { id: "boarding",        label: "Boarding",        icon: "fa-moon",           color: "shAccent",
  ...
  { id: "grooming",        label: "Grooming",        icon: "fa-scissors",       color: "shSecondary",
  ...
  { id: "photography",     label: "Photography",     icon: "fa-camera",         color: "shSecondary",
  ...
  { id: "retail",          label: "Retail",          icon: "fa-shop",           color: "shPrimary",
  ...
  { id: "payment_plans",   label: "Payment Plans",   icon: "fa-file-invoice-dollar", color: "shAccent",
  ...
  { id: "manual_payments", label: "Manual Payments", icon: "fa-money-bills",    color: "shPrimary",
  ...
  // engagement features
  { id: "rewards",         label: "Rewards / Trophies", icon: "fa-trophy",      color: "shAccent",
  ...
  { id: "client_messaging",label: "Client Messaging",icon: "fa-comments",       color: "shPrimary",
    desc: "Two-way client/admin chat. Disable to hide Messages from portal and admin nav.",
    affects: ["Admin nav", "Portal", "Quick links"] },
  { id: "waitlist",        label: "Waitlist",        icon: "fa-hourglass-half", color: "shAccent",
...
function FeatureVisibilityPanel() {
  ...
  if (!s) return <div className="text-shTextMuted">Loading...</div>;
  ...
  return (
    <div className="space-y-4" data-testid="feature-visibility-panel">
      <div className="bg-[var(--sh-card-base)] border-2 border-shSecondary/40 rounded-2xl p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.35em] text-shSecondary mb-1">
              <i className="fas fa-toggle-on mr-1.5"/>Feature Visibility
            </p>
            <h2 className="text-2xl sm:text-3xl font-black text-shText uppercase italic tracking-tight">
              App-Wide Feature <span className="text-shPrimary">Switches.</span>
            </h2>
            <p className="text-[13px] text-shTextMuted mt-2 max-w-2xl">
          ...
          <span className="text-[11px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border bg-shPrimary/15 text-shPrimary border-shPrimary/40"
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <button
            ...
            className="bg-[var(--sh-card-base)] border border-shBorder text-shTextMuted font-black uppercase tracking-widest text-[12px] px-4 py-2 rounded hover:border-shSecondary"
          >
            <i className="fas fa-rotate-left mr-2"/>Enable all
          </button>
          {msg && (
            <span className="text-[12px] text-shPrimary ml-2" data-testid="feature-visibility-msg">{msg}</span>
          )}
        </div>
      </div>
        ...
        {FEATURE_VISIBILITY_META.map((f) => {
          ...
          return (
            <div
              ...
              className={`rounded-xl border p-4 transition ${enabled ? "bg-[var(--sh-card-base)] border-shBorder" : "bg-[var(--sh-card-base)]/60 border-shBorder/60 opacity-75"}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <i className={`fas ${f.icon} text-${f.color}`}/>
                    <p className={`text-[15px] font-black uppercase italic tracking-tight ${enabled ? "text-shText" : "text-shTextMuted"}`}>
                      {f.label}
                    </p>
                    <span className={`text-[10px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded border ${
                      enabled
                        ...
                        : "bg-gray-700/30 text-shTextMuted border-gray-600/40"
                    }`}>
                      {enabled ? "On" : "Off"}
                    </span>
                  </div>
                  <p className="text-[12px] text-shTextMuted mt-1 leading-snug">{f.desc}</p>
                  <p className="text-[10px] text-shTextMuted mt-2 uppercase tracking-widest">
                    Affects: <span className="text-shTextMuted">{f.affects.join(" · ")}</span>
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  onClick={() => toggle(f.id)}
                  className={`relative shrink-0 w-12 h-7 rounded-full transition ${enabled ? "bg-shPrimary" : "bg-gray-600"}`}
                >
...
const CPC_ANNOUNCEMENT_STYLES = [
  { id: "info",    label: "Info",     color: "shSecondary" },
  { id: "success", label: "Success",  color: "shPrimary" },
  { id: "warning", label: "Warning",  color: "shAccent" },
...
function CpcSwitch({ on, onClick, testid, disabled }) {
  return (
    <button type="button" role="switch" aria-checked={on}
            ...
            className={`relative shrink-0 w-12 h-7 rounded-full transition ${disabled ? "bg-gray-700 opacity-50" : on ? "bg-shPrimary" : "bg-gray-600"}`}>
...
function ClientPortalControlsPanel() {
  ...
  if (!cpc) return <div className="text-shTextMuted">Loading...</div>;
  ...
  return (
    <div className="space-y-5" data-testid="client-portal-controls-panel">
      <div className="bg-[var(--sh-card-base)] border-2 border-shPrimary/40 rounded-2xl p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3 flex-wrap mb-2">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-shText uppercase italic tracking-tight">
              What clients <span className="text-shPrimary">see &amp; do.</span>
            </h2>
        ...
        {msg && <span className="text-[12px] text-shPrimary" data-testid="cpc-msg">{msg}</span>}
      </div>

      {/* ── Sections ────────────────────────────────────────────────── */}
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-5">
        <p className="text-[11px] font-black uppercase tracking-[0.3em] text-shSecondary mb-3">
          ...
          {CPC_SECTION_META.map(({ id, label, desc, masterFeature }) => {
            ...
            return (
              <div key={id} data-testid={`cpc-section-row-${id}`}
                   className={`flex items-start gap-3 p-3 rounded-lg border ${on && !masterOff ? "border-shBorder bg-[var(--sh-card-base)]" : "border-shBorder bg-[var(--sh-card-base)]/40 opacity-70"}`}>
                <div className="flex-1 min-w-0">
                  <p className={`text-[14px] font-black uppercase tracking-wide ${on && !masterOff ? "text-shText" : "text-shTextMuted"}`}>{label}</p>
                  <p className="text-[11px] text-shTextMuted mt-0.5 leading-snug">{desc}</p>
                  {masterOff && (
                    <p className="text-[10px] text-shAccent mt-1 uppercase tracking-widest">
      ...
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-5" data-testid="cpc-smart-priority-info">
        <p className="text-[11px] font-black uppercase tracking-[0.3em] text-shSecondary mb-2">
      ...
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Title</label>
            <input value={cpc.announcement?.title || ""} onChange={e => setAnn("title", e.target.value)}
                   data-testid="cpc-announcement-title"
                   className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded px-3 py-2 text-[13px] text-shText"
                   placeholder="e.g. Holiday hours" />
          </div>
          <div>
            <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Style</label>
          ...
          <div className="md:col-span-2">
            <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Message</label>
          ...
          <div>
            <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Start date (optional)</label>
          ...
          <div>
            <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">End date (optional)</label>
      ...
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {Object.entries(cpc.labels || {}).map(([k, v]) => (
            <div key={k}>
              <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">{k.replace(/_/g, " ")}</label>
      ...
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-5 space-y-4">
        <div>
          ...
          <p className="text-[11px] text-shTextMuted mb-2">Shown when the client tries to book but Requirements to Book is incomplete.</p>
          <textarea rows={2} value={cpc.booking_locked_message || ""}
                    ...
                    className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded px-3 py-2 text-[13px] text-shText"/>
        </div>
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.3em] text-shSecondary mb-2">
...
function PaymentOptionsPanel() {
  ...
  if (!rows) return <div className="text-shTextMuted">Loading...</div>;
  ...
  return (
    <div className="space-y-4" data-testid="payment-options-panel">
      <div className="bg-[var(--sh-card-base)] border-2 border-shPrimary/40 rounded-2xl p-5 shadow-2xl flex items-start justify-between gap-3 flex-wrap">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.35em] text-shPrimary mb-1">
            <i className="fas fa-money-bill-wave mr-1.5"/>Payment Options
          </p>
          <h2 className="text-2xl sm:text-3xl font-black text-shText uppercase italic tracking-tight">
            How clients <span className="text-shPrimary">can pay.</span>
          </h2>
          <p className="text-[13px] text-shTextMuted mt-2 max-w-2xl leading-snug">
            Toggle the methods you accept. Enabled methods show up in the client portal and on the
            booking-submitted screen. <span className="text-shAccent">Booking is never blocked on
        ...
        {msg && <span className="text-[12px] text-shPrimary w-full" data-testid="pay-options-msg">{msg}</span>}
      </div>

      {rows.map((row, idx) => (
        <div key={row.key || idx} className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-4 space-y-3"
             data-testid={`pay-row-${row.key}`}>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <p className="text-[15px] font-black uppercase italic tracking-tight text-shText flex items-center gap-2">
              <i className={`fas ${ICONS[row.key] || "fa-money-bill"} text-shPrimary`}/>
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className={`text-[12px] font-black uppercase tracking-widest ${row.enabled ? "text-shPrimary" : "text-shTextMuted"}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Display name</label>
            ...
            <div>
              <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Link (optional)</label>
          ...
          <div>
            <label className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Instructions for the client</label>
...
function BookingFlowControlsPanel() {
  ...
  if (!bfc) return <div className="text-shTextMuted">Loading...</div>;
  ...
  return (
    <div className="space-y-4" data-testid="booking-flow-controls-panel">
      <div className="bg-[var(--sh-card-base)] border-2 border-shSecondary/40 rounded-2xl p-5 shadow-2xl flex items-start justify-between gap-3 flex-wrap">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.35em] text-shSecondary mb-1">
            <i className="fas fa-calendar-check mr-1.5"/>Booking Flow Controls
          </p>
          <h2 className="text-2xl sm:text-3xl font-black text-shText uppercase italic tracking-tight">
            Per-Service <span className="text-shPrimary">Rules.</span>
          </h2>
          <p className="text-[13px] text-shTextMuted mt-2 max-w-2xl leading-snug">
        ...
        {msg && <span className="text-[12px] text-shPrimary w-full" data-testid="bfc-msg">{msg}</span>}
      </div>

      <div className="bg-[var(--sh-card-base)] border-2 border-shPrimary/35 rounded-2xl p-4 space-y-4" data-testid="catalog-service-booking-rules">
        <div>
        ...
        {catalogServices.filter(s => s.active !== false && !s.is_addon).length === 0 && (
          <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl p-4 text-[13px] text-shTextMuted">
        ...
        {catalogServices
          ...
          .map(service => {
            ...
            return (
              <div key={service.id} className={`bg-[var(--sh-card-base)] border rounded-xl p-4 space-y-3 ${hasOverride ? "border-shPrimary/45" : "border-shBorder"} ${masterOff ? "opacity-60" : ""}`}
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-[15px] text-shText font-black uppercase italic tracking-tight truncate">{service.name}</p>
                  ...
                  <div className="flex items-center gap-2">
                    {masterOff && <span className="text-[10px] text-shAccent uppercase tracking-widest"><i className="fas fa-lock mr-1"/>Feature OFF</span>}
                    {hasOverride && (
                      <button type="button" onClick={() => resetCatalogSvc(service.id)}
                              className="text-[10px] font-black uppercase tracking-widest text-shTextMuted hover:text-shText border border-shBorder rounded px-2 py-1">
                        Use defaults
                      </button>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  <label className="flex items-start gap-2 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 cursor-pointer">
                    ...
                    <span><span className="text-[12px] font-black uppercase tracking-widest text-shText block">Client booking enabled</span><span className="text-[10px] text-shTextMuted">Show this exact service in the client booking picker.</span></span>
                  </label>
                  <label className="flex items-start gap-2 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 cursor-pointer">
                    <input type="checkbox" checked={effective.require_approval === true}
                           ...
                           disabled={masterOff} className="mt-1"/>
                    <span><span className="text-[12px] font-black uppercase tracking-widest text-shText block">Require approval</span><span className="text-[10px] text-shTextMuted">Client request stays pending until staff approves it.</span></span>
                  </label>
                  <label className="flex items-start gap-2 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 cursor-pointer">
                    <input type="checkbox" checked={effective.instant_book === true}
                           ...
                           disabled={masterOff} className="mt-1"/>
                    <span><span className="text-[12px] font-black uppercase tracking-widest text-shText block">Instant confirmation</span><span className="text-[10px] text-shTextMuted">Automatically approve valid client bookings.</span></span>
                  </label>
                  <label className="flex items-start gap-2 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 cursor-pointer">
                    <input type="checkbox" checked={effective.same_day === true}
                           onChange={e => setCatalogSvc(service.id, "same_day", e.target.checked)}
                           disabled={masterOff} className="mt-1"/>
                    <span><span className="text-[12px] font-black uppercase tracking-widest text-shText block">Same-day allowed</span><span className="text-[10px] text-shTextMuted">Allow clients to request this service for today.</span></span>
                  </label>
                  <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3">
                    <p className="text-[10px] text-shTextMuted font-black uppercase tracking-widest">Minimum lead time (hours)</p>
      ...
      <div className="pt-2">
        <p className="text-[11px] font-black uppercase tracking-[0.3em] text-shTextMuted">Category fallback rules</p>
        <p className="text-[11px] text-shTextMuted mt-1">Used by older bookings and any service left on “Use defaults.”</p>
      </div>
      ...
      {BFC_SERVICES.map(svc => {
        ...
        return (
          <div key={svc.id} data-testid={`bfc-svc-${svc.id}`}
               className={`bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-4 ${masterOff ? "opacity-60" : ""}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                <label key={t.k} className="flex items-start gap-2 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 cursor-pointer">
                  <input type="checkbox" checked={cur[t.k] === true} onChange={e => setSvc(svc.id, t.k, e.target.checked)}
                         disabled={masterOff}
                         data-testid={`bfc-${svc.id}-${t.k}`}
                         className="mt-1"/>
                  <span>
                    <span className="text-[13px] font-black uppercase tracking-widest text-shText block">{t.label}</span>
                    <span className="text-[11px] text-shTextMuted">{t.desc}</span>
                  </span>
                </label>
              ))}
              <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3">
                ...
                <input type="number" min={0} value={cur.min_lead_hours ?? ""} placeholder="—"
                       ...
                       className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded px-2 py-1.5 text-[13px] text-shText"/>
                <p className="text-[10px] text-shTextMuted mt-1">Blank = use global</p>
              </div>
              <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3">
                <p className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Max advance (days)</p>
      ...
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-4 space-y-3">
        ...
        <label className="flex items-start gap-2 cursor-pointer">
          ...
          <span>
            <span className="text-[13px] font-black uppercase tracking-widest text-shText block">Auto-offer waitlist when full</span>
            <span className="text-[11px] text-shTextMuted">If Feature Visibility waitlist is off, this is ignored.</span>
          </span>
        </label>
        <div>
          <p className="text-[11px] text-shTextMuted font-black uppercase tracking-widest">Capacity-reached message</p>
      ...
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-4 space-y-3" data-testid="bfc-price-estimate-card">
        <p className="text-[11px] font-black uppercase tracking-[0.3em] text-shPrimary">
        ...
        <label className="flex items-start gap-2 cursor-pointer">
          ...
          <span>
            <span className="text-[13px] font-black uppercase tracking-widest text-shText block">Show booking price estimates</span>
            <span className="text-[11px] text-shTextMuted">
...
function DashboardWidgetsPanel() {
  ...
  if (!dw) return <div className="text-shTextMuted">Loading...</div>;
    ...
    <div className="space-y-4" data-testid="dashboard-widgets-panel">
      <div className="bg-[var(--sh-card-base)] border-2 border-shPrimary/40 rounded-2xl p-5 shadow-2xl flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-shText uppercase italic tracking-tight">
            Slim Your <span className="text-shPrimary">Dashboard.</span>
          </h2>
          <p className="text-[13px] text-shTextMuted mt-2 max-w-2xl leading-snug">
        ...
        {msg && <span className="text-[12px] text-shPrimary w-full" data-testid="dw-msg">{msg}</span>}
      </div>
        ...
        {DASHBOARD_WIDGET_META.map(w => {
          ...
          return (
            <div key={w.id} data-testid={`dw-row-${w.id}`}
                 className={`flex items-start gap-3 p-3 rounded-lg border ${on && !masterOff ? "bg-[var(--sh-card-base)] border-shBorder" : "bg-[var(--sh-card-base)]/40 border-shBorder/60 opacity-70"}`}>
              <div className="flex-1 min-w-0">
                <p className={`text-[14px] font-black uppercase tracking-wide ${on && !masterOff ? "text-shText" : "text-shTextMuted"}`}>{w.label}</p>
                <p className="text-[11px] text-shTextMuted mt-0.5 leading-snug">{w.desc}</p>
                {masterOff && (
                  <p className="text-[10px] text-shAccent mt-1 uppercase tracking-widest">
...
function PermissionMatrixPanel() {
  ...
  if (!matrix) return <div className="text-shTextMuted">Loading permission matrix...</div>;
  ...
  return (
    <div className="space-y-4" data-testid="permission-matrix-panel">
      <div className="bg-[var(--sh-card-base)] border-2 border-shSecondary/40 rounded-2xl p-5 shadow-2xl">
        ...
        <h2 className="text-2xl sm:text-3xl font-black text-shText uppercase italic tracking-tight">
          Roles × <span className="text-shPrimary">Permissions.</span>
        </h2>
        <p className="text-[13px] text-shTextMuted mt-2 max-w-3xl leading-snug">
        ...
        {msg && <p className={`text-[12px] mt-3 ${msg.startsWith("Saved") ? "text-shPrimary" : "text-shAccent"}`} data-testid="perm-matrix-msg">{msg}</p>}
      </div>

      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl p-4 overflow-x-auto">
        <table className="w-full text-[12px] min-w-[900px]" data-testid="perm-matrix-table">
          <thead>
            <tr className="border-b border-shBorder">
              <th className="text-left py-2 px-2 sticky left-0 bg-[var(--sh-card-base)] z-10" style={{ minWidth: 200 }}>
                <span className="font-black uppercase tracking-widest text-shTextMuted">Permission</span>
              </th>
              {roles.map(r => (
                <th key={r} className="text-center py-2 px-2 font-black uppercase tracking-widest"
                    data-testid={`perm-col-${r}`}>
                  <span className={r === "owner" ? "text-shAccent" : "text-shText"}>{r.replace(/_/g, " ")}</span>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {PERM_META.map(p => (
              <tr key={p.key} className="border-b border-shBorder/40 hover:bg-[var(--sh-card-base)]/30">
                <td className="py-2 px-2 sticky left-0 bg-[var(--sh-card-base)] z-10" title={p.desc}>
                  <p className="text-shText font-black">{p.label}</p>
                  <p className="text-[10px] text-shTextMuted leading-snug">{p.desc}</p>
                </td>
                {roles.map(r => {
            ...
            <tr>
              <td className="py-3 px-2 sticky left-0 bg-[var(--sh-card-base)] z-10"></td>
              {roles.map(r => (
                <td key={r} className="text-center py-3 px-2">
                  {r === "owner" ? (
                    <span className="text-[10px] uppercase tracking-widest text-shAccent">Locked</span>
                  ) : (
                    <button
                      onClick={() => saveRole(r)}
                      disabled={!dirty[r] || savingRole === r}
                      data-testid={`perm-save-${r}`}
                      className="bg-shPrimary text-bgHeader font-black uppercase tracking-widest text-[10px] px-3 py-1.5 rounded disabled:opacity-50"
                    >
...
function ThemeGroup({ title, subtitle, testid, children }) {
  ...
  return (
    <div className="rounded-lg border border-shBorder bg-[var(--sh-card-base)]/40" data-testid={testid}>
      <button type="button" onClick={()=>setOpen(o=>!o)}
              className="w-full flex items-center justify-between px-4 py-3 text-left"
              data-testid={`${testid}-toggle`}>
        <div>
          <p className="text-[14px] font-black text-shText uppercase tracking-widest">{title}</p>
          {subtitle && <p className="text-[12px] text-shTextMuted mt-0.5">{subtitle}</p>}
        </div>
...
function ColorField({ label, sub, value, onChange, testid }) {
  return (
    <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3">
      ...
      <p className="text-[13px] text-shTextMuted mt-0.5">{sub}</p>
      <div className="flex items-center gap-2 mt-2">
        <input
          type="color"
          value={value || "#000000"}
...
function Field({ label, value, onChange, type="text", testId }) {
  return (
    <div>
      <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">{label}</label>
  ...
  return (
    <div className="space-y-6" data-testid="hours-panel">
      ...
      <Section title="Boarding" subtitle="Boarding is treated as 24/7 by default. Capacity is enforced per night.">
        <div className="text-[14px] font-black text-shPrimary uppercase tracking-widest bg-shPrimary/10 rounded p-3">24/7 — overnight stays allowed</div>
      </Section>
      ...
      <Section title="Closed Days" subtitle="Holidays / vacations. Clients can't self-book on these dates; admin overrides still work.">
        <div className="space-y-2" data-testid="closed-dates-list">
          {closedDates.length === 0 && <p className="text-[14px] text-shTextMuted italic normal-case">No closed days configured.</p>}
          {[...closedDates].sort().map((d) => (
            <div key={d} className="flex items-center gap-2 bg-[var(--sh-card-base)] rounded p-2" data-testid={`closed-date-${d}`}>
              <i className="fas fa-calendar-xmark text-shAccent text-xs"/>
              <span className="flex-1 text-sm font-black text-shText">{new Date(d + "T12:00:00").toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</span>
        ...
        <div className="flex gap-2 mt-3">
          <input type="date" value={newClosed} onChange={(e)=>setNewClosed(e.target.value)}
                 ...
                 className="flex-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-sm text-shText" style={{colorScheme:"dark"}}/>
          <button onClick={()=>{
                     if (!newClosed) return;
                  ...
                  className="bg-shAccent text-bgHeader px-4 py-2 rounded text-[15px] font-black uppercase tracking-widest hover:bg-shAccent/80">
            + Add
          </button>
        </div>
      </Section>
...
function DayRow({ day, val, onChange, testPrefix }) {
  return (
    <div className="grid grid-cols-12 items-center gap-3 py-2 border-b border-shBorder/30">
      <div className="col-span-3 text-[15px] font-black uppercase tracking-widest text-shTextMuted">{day}</div>
      <div className="col-span-3">
        <input type="time" value={val.open||""} disabled={val.closed} onChange={(e)=>onChange("open", e.target.value)} data-testid={`${testPrefix}-open`}
               className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-xs text-shText disabled:opacity-40" style={{colorScheme:"dark"}} />
      </div>
      <div className="col-span-3">
      ...
      <label className="col-span-3 flex items-center gap-2 text-[14px] font-black uppercase tracking-widest text-shTextMuted cursor-pointer">
        <input type="checkbox" checked={!!val.closed} onChange={(e)=>onChange("closed", e.target.checked)} data-testid={`${testPrefix}-closed`} className="accent-shAccent" />
        Closed
      </label>
    </div>
  );
    ...
    <div className="space-y-6" data-testid="capacity-panel">
      ...
      <Section title="Kennels / Rooms" subtitle="Named spaces for boarding assignment.">
        <div className="space-y-2">
          {kennels.map((k, i) => (
            <div key={i} className="flex items-center gap-2 bg-[var(--sh-card-base)] rounded p-2">
              <input value={k} onChange={(e)=>{const c=[...kennels]; c[i]=e.target.value; setKennels(c);}}
                     className="flex-1 bg-transparent text-sm text-shText outline-none" data-testid={`kennel-${i}`} />
          ...
          <div className="flex gap-2 pt-2">
            <input value={newK} onChange={(e)=>setNewK(e.target.value)} placeholder="New kennel/room name"
                   className="flex-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-sm text-shText" data-testid="new-kennel-input" />
            <button onClick={()=>{ if(newK.trim()){ setKennels([...kennels, newK.trim()]); setNewK(""); } }} data-testid="add-kennel"
                    className="bg-shPrimary text-bgHeader px-4 py-2 rounded font-black text-[14px] uppercase tracking-widest">+ Add</button>
          </div>
        </div>
      </Section>
    ...
    <div className="space-y-6" data-testid="rules-panel">
      <Section title="Booking Policy">
        ...
        <label className="flex items-center gap-3 mt-4 cursor-pointer">
          <input type="checkbox" checked={!!r.auto_approve} onChange={(e)=>set("auto_approve", e.target.checked)} data-testid="auto-approve" className="accent-shPrimary w-4 h-4" />
          <span className="text-[15px] font-black uppercase tracking-widest text-shTextMuted">Auto-approve client bookings (skip pending step)</span>
        </label>
      </Section>
      ...
      <Section title="Stay-duration pricing (auto-bill at check-out)"
        ...
        <label className="flex items-center gap-3 cursor-pointer mb-4">
          ...
          <span className="text-[15px] font-black uppercase tracking-widest text-shTextMuted">Auto-price stays at check-out (recommended)</span>
        </label>
      ...
      <Section title="Multi-dog household discount" subtitle="Auto-applied at check-out for the 2nd-and-later dog from the same client on the same date. Each service has its OWN discount tier. Sit Happens default is 50% off the BASE service price for additional daycare/boarding dogs.">
        <label className="flex items-center gap-3 cursor-pointer mb-4">
          <input type="checkbox" checked={mdEnabled}
                 ...
                 className="accent-shPrimary w-4 h-4" />
          <span className="text-[15px] font-black uppercase tracking-widest text-shTextMuted">Enable multi-dog discount (master switch)</span>
        </label>
        <div className={`space-y-3 ${mdEnabled ? "" : "opacity-50 pointer-events-none"}`}>
          {SERVICES.map(svc => {
            const v = byService[svc.key] || {};
            return (
              <div key={svc.key} className="bg-[var(--sh-card-base)] rounded-lg p-3 border border-shBorder" data-testid={`multi-dog-svc-${svc.key}`}>
                <label className="flex items-center gap-2 cursor-pointer mb-3">
                  <input type="checkbox" checked={!!v.enabled}
                         ...
                         className="accent-shPrimary w-4 h-4" />
                  <i className={`fas ${svc.icon} text-shSecondary mr-1`}/>
                  <span className="text-[14px] font-black uppercase tracking-widest text-shText">{svc.label}</span>
                </label>
                <div className={`grid grid-cols-3 gap-3 ${v.enabled ? "" : "opacity-60"}`}>
                  <div>
                    <label className="text-[11px] font-black text-shTextMuted uppercase tracking-widest block">Mode</label>
        ...
        <p className="text-[12px] text-shTextMuted mt-2 italic">
    ...
    <div className="space-y-6" data-testid={section ? `day-to-day-section-${section}` : "operator-quick-controls-panel"}>
      {!isQuickHub && (
        <div className="bg-[var(--sh-card-base)]/40 border border-shBorder rounded-lg p-4">
          <h3 className="text-[18px] font-black text-shPrimary uppercase tracking-widest mb-1">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {VAX_OPTIONS.map(v => (
            <button key={v.key} onClick={()=>toggle(v.key)} data-testid={`vax-${v.key}`}
                    className={`py-3 px-4 rounded border text-[15px] font-black uppercase tracking-widest transition ${req.includes(v.key)?"bg-shPrimary text-bgHeader border-shPrimary":"bg-[var(--sh-card-base)] text-shTextMuted border-shBorder hover:border-shPrimary/50"}`}>
              {v.label}
            </button>
          ))}
        </div>
      ...
      <Section title="Pup Report Card Mood Tags" subtitle="These appear as pill buttons on the report card modal. Pick an optional icon AND color for each one.">
          ...
          {tags.map((t,i)=>{
            ...
            <div key={i}
              ...
              <button type="button" onClick={()=>setPickerOpen(pickerOpen === i ? -1 : i)}
                      ...
                      className="w-7 h-7 rounded-full grid place-items-center bg-[var(--sh-card-base)] border hover:opacity-80"
        ...
        <div className="flex gap-2 mt-4">
          <input value={newT} onChange={(e)=>setNewT(e.target.value)} placeholder="Add a tag (e.g. Loves the Hose)"
                 className="flex-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-sm text-shText" data-testid="new-tag-input" />
          <button onClick={()=>{ if(newT.trim()){ setTags([...tags, { label: newT.trim(), icon: "", color: "" }]); setNewT(""); } }} data-testid="add-tag"
                  className="bg-shPrimary text-bgHeader px-4 py-2 rounded font-black text-[14px] uppercase tracking-widest">+ Add Tag</button>
        </div>
      </Section>
    ...
    <div className="space-y-6" data-testid="waiver-panel">
      <Section title="Waiver Text" subtitle="Markdown bold (**Heading**) becomes a green section heading in the portal.">
        <textarea value={text} onChange={(e)=>setText(e.target.value)} rows={14} data-testid="waiver-text-edit"
                  className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-shText text-xs font-mono focus:border-shSecondary outline-none" />
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={required} onChange={(e)=>setRequired(e.target.checked)} data-testid="waiver-required" className="accent-shPrimary w-4 h-4" />
            <span className="text-[14px] font-black uppercase tracking-widest text-shTextMuted">Require waiver before client can book</span>
          </label>
          <span className="text-[14px] font-black uppercase tracking-widest text-shTextMuted">Current version: v{version}</span>
        </div>
      </Section>

      <Section title="Signed Waivers" subtitle="Bump version after material changes to require all clients to re-sign.">
        <button onClick={()=>setShowSigs(!showSigs)} className="mb-3 text-[14px] font-black uppercase tracking-widest text-shSecondary hover:underline">
        ...
        {showSigs && (
          <div className="space-y-2 max-h-80 overflow-y-auto" data-testid="waiver-signatures-list">
            {signatures.length === 0 && <p className="text-xs text-shTextMuted uppercase font-black">No signatures yet.</p>}
            {signatures.map(sig => (
              <div key={sig.id} className="bg-[var(--sh-card-base)] rounded p-3 text-xs flex flex-col md:flex-row md:items-center justify-between gap-2" data-testid={`sig-${sig.id}`}>
                <div>
                  <p className="text-shText font-black">{sig.typed_name} <span className="text-shTextMuted font-normal">· {sig.client_name}</span></p>
                  <p className="text-[14px] text-shTextMuted uppercase font-black tracking-widest mt-1">Signed {(sig.signed_at||"").slice(0,19).replace("T"," ")} · v{sig.waiver_version}</p>
                </div>
                <span className="text-[14px] font-black uppercase tracking-widest bg-shPrimary/15 text-shPrimary px-2 py-1 rounded">Valid</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="flex flex-col md:flex-row justify-end gap-3 pt-4 border-t border-shBorder">
        <button onClick={()=>saveAndMaybeBump(false)} disabled={saving} data-testid="save-waiver-noversion"
                className="bg-shSecondary text-shText px-6 py-3 rounded font-black text-[14px] uppercase tracking-widest shadow-xl disabled:opacity-50">
          Save Without Bumping
        </button>
        <button onClick={()=>saveAndMaybeBump(true)} disabled={saving} data-testid="save-waiver-bump"
                className="bg-shPrimary text-bgHeader px-6 py-3 rounded font-black text-[14px] uppercase tracking-widest shadow-xl disabled:opacity-50">
      ...
      <Section title="Marketing QR Code"
        ...
        <div className="flex flex-col md:flex-row items-stretch gap-5">
          <div className="bg-white p-4 rounded-xl flex items-center justify-center w-full md:w-64 h-64 shrink-0">
            {loading && !previewUrl && <p className="text-shTextMuted text-xs font-black uppercase tracking-widest">Generating…</p>}
            {previewUrl && <img src={previewUrl} alt="App QR code" className="max-w-full max-h-full" data-testid="qr-preview-img" />}
          </div>
          <div className="flex-1 space-y-4">
            <div>
              <label className="text-[14px] text-shTextMuted font-black uppercase tracking-widest">Points to</label>
              <p className="text-[15px] text-shSecondary break-all mt-1 font-mono" data-testid="qr-target-url">{targetUrl || "—"}</p>
            </div>
            <div>
              <label className="text-[14px] text-shTextMuted font-black uppercase tracking-widest">Tracking tag <span className="text-gray-600 normal-case tracking-normal">(optional — e.g. flyer, postcard, fb-ad)</span></label>
              <input
                type="text"
                value={ref}
              ...
              />
              <p className="text-[13px] text-shTextMuted mt-1">Appended as <span className="font-mono text-shSecondary">?ref=…</span> so future analytics can show where each scan came from.</p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <button onClick={()=>download(512)} data-testid="qr-download-small"
                      className="bg-[var(--sh-card-base)] border border-shBorder hover:border-shSecondary text-shSecondary py-2 rounded text-[14px] font-black uppercase tracking-widest">
                <i className="fas fa-download mr-1"/>Small<br /><span className="text-[12px] text-shTextMuted">512px</span>
              </button>
              <button onClick={()=>download(1024)} data-testid="qr-download-medium"
                      className="bg-shSecondary/15 border border-shSecondary/40 hover:bg-shSecondary/30 text-shSecondary py-2 rounded text-[14px] font-black uppercase tracking-widest">
              ...
              <button onClick={()=>download(2048)} data-testid="qr-download-large"
                      className="bg-shPrimary text-bgHeader py-2 rounded text-[14px] font-black uppercase tracking-widest shadow">
                <i className="fas fa-download mr-1"/>Poster<br /><span className="text-[12px] opacity-70">2048px</span>
              </button>
            </div>
            <p className="text-[13px] text-shTextMuted">Higher resolutions = sharper print at larger sizes. Use Poster for full-page flyers, Print for cards, Small for screen sharing.</p>
          </div>
        </div>
      </Section>
    </div>
...
function ReviewLinksPanel() {
  ...
  if (!links) return <p className="text-shTextMuted text-sm">Loading…</p>;
  ...
  return (
    <div className="space-y-4" data-testid="review-links-panel">
      <p className="text-[13px] text-shTextMuted">
        ...
        placeholders you can use: <code className="text-shSecondary">{"{first_name}"}</code> and <code className="text-shSecondary">{"{dog_name}"}</code>.
      </p>

      <div>
        <label className="text-[12px] font-black text-shTextMuted uppercase tracking-widest"><i className="fab fa-google mr-1"/>Google review URL</label>
      ...
      <div>
        <label className="text-[12px] font-black text-shTextMuted uppercase tracking-widest"><i className="fab fa-facebook mr-1"/>Facebook reviews URL</label>
      ...
      <div>
        <label className="text-[12px] font-black text-shTextMuted uppercase tracking-widest"><i className="fab fa-yelp mr-1"/>Yelp URL <span className="text-gray-600 normal-case">(optional)</span></label>
      ...
      <div>
        <label className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Default request message</label>
...
function PortalFirstVisitPanel({ s, save, saving }) {
  ...
  return (
    <div className="space-y-5" data-testid="portal-first-visit-panel">
      <p className="text-[14px] text-shTextMuted">
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" checked={enabled} onChange={(e)=>setEnabled(e.target.checked)}
               className="accent-shPrimary w-4 h-4" data-testid="fv-enabled"/>
        <span className="text-[13px] font-black uppercase tracking-widest text-shText">Show this card on the client portal</span>
      </label>

      <Section title={<span><i className="fas fa-heading text-shSecondary mr-2"/>Heading</span>}>
        <input value={heading} onChange={(e)=>setHeading(e.target.value)} data-testid="fv-heading"
               className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-shText text-sm"/>
      </Section>

      <Section title={<span><i className="fas fa-list-ol text-shPrimary mr-2"/>Bullets ({bullets.length}/8)</span>}>
        <div className="space-y-3" data-testid="fv-bullets">
          {bullets.map((b, i) => (
            <div key={i} className="bg-[var(--sh-card-base)]/60 border border-shBorder rounded-lg p-3 space-y-2"
                 data-testid={`fv-bullet-${i}`}>
              <div className="flex items-center justify-between gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-shTextMuted">Bullet {i + 1}</span>
                <div className="flex gap-1">
                  <button type="button" onClick={()=>move(i, -1)} disabled={i === 0}
                          className="text-[11px] px-2 py-1 rounded bg-shSurfaceRaised hover:bg-[var(--sh-card-base)] text-shTextMuted disabled:opacity-30"
          ...
          {bullets.length < 8 && (
            <button type="button" onClick={addBullet}
                    ...
                    className="text-[12px] font-black uppercase tracking-widest px-3 py-2 rounded bg-shSecondary/15 text-shSecondary border border-shSecondary/30 hover:bg-shSecondary/25 transition">
      ...
      <Section title={<span><i className="fas fa-comment-dots text-shPrimary mr-2"/>Closing line (italic)</span>}>
        <input value={footer} onChange={(e)=>setFooter(e.target.value)} data-testid="fv-footer"
               className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-shText text-sm"/>
        <p className="text-[12px] text-shTextMuted mt-1">Shown italicized at the bottom of the card. Leave blank to hide.</p>
      </Section>
...
function PortalLinksPanel({ s, save, saving }) {
  ...
  const [links, setLinks] = useState({
    ...
    shopify_store_url: initial.shopify_store_url || "",
    photography_portfolio_url: initial.photography_portfolio_url || "",
  });
  ...
  return (
    <div className="space-y-5" data-testid="portal-links-panel">
      ...
      <Section title={<span><i className="fas fa-shirt text-shAccent mr-2"/>Shopify Apparel Store</span>}>
        <input
          type="url"
          value={links.shopify_store_url}
          onChange={(e)=>setLinks(l => ({ ...l, shopify_store_url: e.target.value.trim() }))}
          placeholder="https://your-store.myshopify.com"
          data-testid="link-shopify-input"
          className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-shText text-sm font-mono"
        />
        <p className="text-[13px] text-shTextMuted mt-1">
          Shows a "Shop Apparel" section in the client Shop that links out to your Shopify store (print-on-demand shirts, hoodies, mugs, etc.). Leave blank to hide it — apparel stays on Shopify, everything else stays on Sit Happens.
        </p>
      </Section>
      <Section title={<span><i className="fas fa-globe text-shSecondary mr-2"/>Your Website</span>}>
        <input
          type="url"
          ...
          className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-shText text-sm font-mono"
        />
        <p className="text-[13px] text-shTextMuted mt-1">Shows up as a "Visit Our Website" button on the portal.</p>
      </Section>
      <Section title={<span><i className="fas fa-camera-retro text-shAccent mr-2"/>Photography Portfolio (Public Showcase)</span>}>
        <input
          type="url"
          value={links.photography_portfolio_url}
          onChange={(e)=>setLinks(l => ({ ...l, photography_portfolio_url: e.target.value.trim() }))}
          placeholder="https://your-business.pixieset.com"
          data-testid="link-photography-portfolio-input"
          className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-shText text-sm font-mono"
        />
        <p className="text-[13px] text-shTextMuted mt-1">
          Powers the &quot;View Galleries&quot; button at the top of the client Photography page — your public Pixieset portfolio/showcase, not a specific client's finished gallery. Leave blank to hide the button.
        </p>
      </Section>
      <Section title={<span><i className="fas fa-images text-shPrimary mr-2"/>Photo Gallery (Fallback Only)</span>}>
        <input
          type="url"
        ...
        <p className="text-[13px] text-shTextMuted mt-1">
          ...
          Photo galleries are now <span className="font-black text-shText">per-client</span> — set each client's gallery URL on their record in the Clients screen. This field is only used as a fallback for clients who don't have their own gallery URL set.
        </p>
      </Section>
      <SaveBar onSave={onSave} saving={saving} />
    </div>
...
function ServiceInfoPanel({ s, save, saving }) {
  ...
  const services = [
    { k: "daycare", label: "Daycare", icon: "fa-sun", color: "text-shSecondary" },
    { k: "boarding", label: "Boarding", icon: "fa-moon", color: "text-shPrimary" },
    ...
    { k: "photography", label: "Photography", icon: "fa-camera-retro", color: "text-shAccent" },
  ];
  return (
    <div className="space-y-5" data-testid="service-info-panel">
      <p className="text-[14px] text-shTextMuted">
        These appear when your clients tap the <i className="fas fa-circle-info text-shSecondary mx-1"/> next to a service on the booking form. Keep them short and reassuring — what is it, who it's for, what to bring.
      </p>
      {services.map(svc => (
        <Section key={svc.k} title={<span><i className={`fas ${svc.icon} ${svc.color} mr-2`}/>{svc.label}</span>}>
          <textarea
            ...
            className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-shText text-sm leading-relaxed"
          />
        </Section>
      ))}
...
function Section({ title, subtitle, children }) {
  return (
    <div>
      <h4 className="text-xs font-black text-shSecondary uppercase tracking-widest">{title}</h4>
      {subtitle && <p className="text-[15px] text-shTextMuted mt-1">{subtitle}</p>}
...
function SaveBar({ onSave, saving }) {
  return (
    <div className="flex justify-end pt-4 border-t border-shBorder">
  ...
  return (
    <div className="space-y-4 max-w-3xl" data-testid="errors-panel">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h4 className="text-sm font-black text-shAccent uppercase tracking-widest mb-1">
            <i className="fas fa-triangle-exclamation mr-2"/>Recent Server Errors
          </h4>
          <p className="text-[15px] text-shTextMuted leading-relaxed">
        ...
        <div className="flex gap-2 shrink-0">
          <button onClick={load} disabled={loading} data-testid="errors-refresh-btn"
                  className="bg-shSurfaceRaised text-gray-200 px-3 py-1.5 rounded text-[14px] font-black uppercase tracking-widest hover:bg-[var(--sh-card-base)] disabled:opacity-50">
      ...
      {msg && <p className="text-[15px] text-shTextMuted">{msg}</p>}

      {errors.length === 0 && !loading ? (
        <div className="bg-[var(--sh-card-base)] border border-shBorder rounded p-6 text-center">
          <i className="fas fa-check-circle text-shPrimary text-2xl mb-2 block"/>
          <p className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">All clear</p>
          <p className="text-[14px] text-shTextMuted mt-1">No unhandled errors recorded.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {errors.map((er) => {
            ...
            return (
              <div key={er.id} className="bg-[var(--sh-card-base)] border border-shBorder rounded overflow-hidden" data-testid={`error-row-${er.id}`}>
                <button onClick={()=>setExpanded(isOpen ? null : er.id)}
                        className="w-full text-left p-3 hover:bg-shSurfaceRaised/40 transition">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[13px] font-black text-shAccent uppercase tracking-widest px-2 py-0.5 bg-shAccent/10 rounded">
                          {er.type}
                        </span>
                        <span className="text-[13px] font-mono text-shTextMuted">{er.method} {er.path}</span>
                      </div>
                      <p className="text-[15px] text-gray-200 break-words">{er.message || "(no message)"}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-[13px] text-shTextMuted">{fmtTs(er.ts)}</p>
                      <i className={`fas fa-chevron-${isOpen?"up":"down"} text-shTextMuted text-[13px] mt-1`}/>
                    </div>
                  </div>
                </button>
                {isOpen && (
                  <pre className="text-[13px] text-shTextMuted bg-black/40 p-3 overflow-x-auto whitespace-pre-wrap border-t border-shBorder">
    ...
    <div className="space-y-6 max-w-2xl" data-testid="backup-panel">
      ...
      <div className="border-t border-shBorder pt-6">
        ...
        <p className="text-[14px] text-shTextMuted mb-3 leading-relaxed">
          Saves a full snapshot of <span className="text-shText font-black">everything that matters</span>:
        ...
        <button onClick={download} disabled={busy} data-testid="backup-download"
                className="bg-shPrimary text-bgHeader px-6 py-3 rounded font-black text-[14px] uppercase tracking-widest shadow-lg disabled:opacity-50">
          <i className="fas fa-download mr-2"/>{busy ? "Working…" : "Download Backup (.json)"}
        </button>
      </div>

      <div className="border-t border-shBorder pt-6">
        <h4 className="text-sm font-black text-shSecondary uppercase tracking-widest mb-2"><i className="fas fa-file-csv mr-2"/>Year-End Income Export</h4>
        <p className="text-[14px] text-shTextMuted mb-3 leading-relaxed">
        ...
        <button onClick={downloadIncomeCsv} disabled={busy} data-testid="income-csv-download"
                className="bg-shSecondary text-bgHeader px-6 py-3 rounded font-black text-[14px] uppercase tracking-widest shadow-lg disabled:opacity-50">
      ...
      <div className="border-t border-shBorder pt-6" data-testid="config-backup-section">
        ...
        <button onClick={downloadConfig} disabled={busy} data-testid="config-download"
                className="bg-purple-500 text-shText px-6 py-3 rounded font-black text-[14px] uppercase tracking-widest shadow-lg disabled:opacity-50">
          <i className="fas fa-download mr-2"/>{busy ? "Working…" : "Download Config (.json)"}
        </button>

        <div className="mt-5 space-y-3">
          <label className="block">
            <span className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Restore config from file</span>
            <input type="file" accept=".json,application/json" onChange={onPickConfigFile} data-testid="config-file"
                   className="block mt-1 w-full text-sm text-shTextMuted file:mr-3 file:py-2 file:px-4 file:rounded file:border-0 file:bg-[var(--sh-card-base)] file:text-purple-400 file:font-black file:uppercase file:text-[14px] file:tracking-widest hover:file:bg-shSurfaceRaised cursor-pointer" />
          </label>

          {configPreview && (
            <div className="bg-[var(--sh-card-base)] border border-shBorder rounded p-3 space-y-3" data-testid="config-preview">
              <div>
                <p className="text-[14px] font-black text-purple-400 uppercase tracking-widest">Config preview · what will be replaced</p>
                <p className="text-[14px] text-shTextMuted">Exported {configPreview.exportedAt?.slice(0,19).replace("T", " ")}</p>
              </div>
              {/* Sprint 110di-24 — Before/after diff so the operator sees
                  exactly what's going to change BEFORE they hit Restore. */}
              <div className="text-[14px]">
                <div className="grid grid-cols-12 gap-2 px-2 py-1 bg-[var(--sh-card-base)]/40 rounded text-shTextMuted font-black uppercase tracking-widest text-[11px]">
                ...
                {Object.keys(configPreview.incoming).map((k) => {
                  ...
                  const diff = inc - cur;
                  return (
                    <div key={k} className="grid grid-cols-12 gap-2 px-2 py-1 border-b border-shBorder/50" data-testid={`config-preview-row-${k}`}>
                      <div className="col-span-6 text-shTextMuted uppercase font-black tracking-widest text-[12px] truncate">{k}</div>
                      <div className="col-span-3 text-right text-shTextMuted">{cur}</div>
                      <div className="col-span-3 text-right">
                        <span className="text-shText font-black">{inc}</span>
                        {diff !== 0 && (
                          <span className={`ml-2 text-[11px] font-black ${diff > 0 ? "text-shPrimary" : "text-shAccent"}`}>
                            ({diff > 0 ? "+" : ""}{diff})
                          </span>
                        )}
                      </div>
              ...
              <div className="bg-shPrimary/10 border border-shPrimary/40 rounded px-3 py-2 text-[12px] text-shPrimary flex items-start gap-2" data-testid="config-preview-untouched">
              ...
              <div className="bg-shSecondary/10 border border-shSecondary/40 rounded px-3 py-2 text-[12px] text-shSecondary flex items-start gap-2" data-testid="config-preview-autosnap">
      ...
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded p-4 text-[14px] text-shTextMuted leading-relaxed">
        <p className="text-shSecondary font-black uppercase tracking-widest mb-2"><i className="fas fa-circle-info mr-2"/>What's in a backup?</p>
        <p>Clients, dogs, bookings, incidents, homework, waiver signatures, and your settings. Admin login credentials are <span className="text-shText font-black">not</span> included — your password stays in the database and is never exported. Backups are plain JSON and safe to email to yourself or store in cloud storage.</p>
      </div>
      ...
      <div className="space-y-3">
        <label className="flex items-center gap-3 cursor-pointer">
          <input type="checkbox" checked={!!draft.enabled}
                 ...
                 className="w-5 h-5 accent-shPrimary"/>
          <span className="text-[14px] font-black text-shText uppercase tracking-widest">Enable sales tax</span>
        </label>
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Rate (%)</span>
            <input type="number" min={0} max={50} step="0.001"
                   ...
                   className="mt-1 block w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm font-mono"/>
          </label>
          <label className="block">
            <span className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Label on receipts</span>
            <input type="text" value={draft.label || ""}
                   onChange={(e) => setDraft({ ...draft, label: e.target.value })}
                   placeholder="Sales Tax"
                   data-testid="sales-tax-label"
                   className="mt-1 block w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm"/>
          </label>
        </div>
        <div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={save} disabled={busy} data-testid="sales-tax-save"
                  className="bg-shPrimary text-bgHeader px-5 py-2 rounded font-black text-[13px] uppercase tracking-widest shadow disabled:opacity-50">
            <i className="fas fa-save mr-1"/>Save
          </button>
          {msg && <span className={`text-[13px] font-black uppercase tracking-widest ${msg.startsWith("Saved") ? "text-shPrimary" : "text-red-400"}`}>{msg}</span>}
        </div>
      </div>
      {summary && (
        <div className="mt-4 bg-[var(--sh-card-base)] border border-shBorder rounded p-3" data-testid="sales-tax-summary">
          <p className="text-[12px] font-black uppercase tracking-widest text-shTextMuted mb-1">YTD Tax Collected · {summary.start_date} → {summary.end_date}</p>
          <p className="text-2xl font-black text-shPrimary">${Number(summary.total_tax_collected || 0).toFixed(2)}</p>
          <div className="grid grid-cols-2 gap-2 mt-2 text-[12px] text-shTextMuted">
            <div className="bg-[var(--sh-card-base)] rounded px-2 py-1 flex justify-between">
              <span className="font-black uppercase tracking-widest text-shTextMuted">Bookings</span>
              <span className="font-black">${Number(summary.bookings_tax_total || 0).toFixed(2)}</span>
            </div>
            <div className="bg-[var(--sh-card-base)] rounded px-2 py-1 flex justify-between">
              <span className="font-black uppercase tracking-widest text-shTextMuted">Retail</span>
...
function YearEndPayrollPanel() {
  ...
  return (
    <div className="border-t border-shBorder pt-6" data-testid="payroll-yearend-panel">
      <h4 className="text-sm font-black text-shSecondary uppercase tracking-widest mb-2"><i className="fas fa-file-invoice-dollar mr-2"/>Year-End Payroll · 1099 / W2 Prep</h4>
      <p className="text-[14px] text-shTextMuted mb-3 leading-relaxed">
      ...
      <div className="flex items-end gap-3 flex-wrap">
        <label className="block">
          <span className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Year</span>
          <input type="number" min={2020} max={2099} value={year}
                 onChange={(e) => setYear(e.target.value)}
                 data-testid="payroll-year"
                 className="mt-1 block w-32 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm font-mono"/>
        </label>
        <label className="flex items-center gap-2 cursor-pointer pb-2">
          ...
          <span className="text-[13px] text-shTextMuted font-black uppercase tracking-widest">Include per-entry detail</span>
        </label>
        <button onClick={download} data-testid="payroll-download"
                className="bg-shSecondary text-bgHeader px-5 py-2 rounded font-black text-[13px] uppercase tracking-widest shadow">
...
function MeetNGreetPanel() {
  ...
  return (
    <div className="border-t border-shBorder pt-6 space-y-6" data-testid="meet-n-greet-panel">
      <div>
        <h4 className="text-sm font-black text-shAccent uppercase tracking-widest mb-2"><i className="fas fa-handshake mr-2"/>Meet-n-Greet Required</h4>
        <p className="text-[14px] text-shTextMuted mb-3 leading-relaxed">
        ...
        <button onClick={toggle} disabled={busy || on === null}
                ...
                className={`px-4 py-2 rounded font-black text-[13px] uppercase tracking-widest transition ${
                  on ? "bg-shAccent text-bgHeader hover:bg-shAccent/90" : "bg-[var(--sh-card-base)] text-shTextMuted hover:bg-shSurfaceRaised border border-shBorder"
                } disabled:opacity-50`}>
      ...
      {mg && (
        <div className="border-t border-shBorder pt-6" data-testid="meet-greet-availability-panel">
          <h4 className="text-sm font-black text-shAccent uppercase tracking-widest mb-2"><i className="fas fa-calendar-check mr-2"/>Meet &amp; Greet Availability</h4>
          <p className="text-[14px] text-shTextMuted mb-3 leading-relaxed">
          ...
          <label className="flex items-center gap-2 text-[14px] font-black uppercase tracking-widest text-shTextMuted mb-4 cursor-pointer">
            <input type="checkbox" checked={mg.enabled} onChange={(e)=>setMg({...mg, enabled: e.target.checked})}
                   data-testid="meet-greet-enabled" className="accent-shAccent"/>
          ...
          <button onClick={saveAvailability} disabled={mgBusy}
                  ...
                  className="bg-shPrimary text-bgHeader px-6 py-2.5 rounded font-black text-[13px] uppercase tracking-widest hover:bg-shPrimary/90 disabled:opacity-50">
...
function PreUpdateSafetyPanel() {
  ...
  const goTone = report?.pre_update_ok ? "border-shPrimary/50 bg-shPrimary/10 text-shPrimary" : "border-red-500/50 bg-red-500/10 text-red-300";
  const file = report?.latest_file || {};
  return (
    <div className="border border-shSecondary/40 rounded-xl p-4 bg-[var(--sh-card-base)]" data-testid="pre-update-safety-panel">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        <div>
          <h4 className="text-sm font-black text-shSecondary uppercase tracking-widest"><i className="fas fa-shield-halved mr-2"/>Pre-Update Safety Check</h4>
          <p className="text-[13px] text-shTextMuted">Use this before pulling GitHub updates on the Bazzite box. It does not change your data.</p>
        </div>
      ...
      {msg && <p className={`text-[13px] mb-2 ${msg.includes("✓") ? "text-shPrimary" : "text-shAccent"}`}>{msg}</p>}
      {report && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
            <HealthMini label="Danger" value={dangers} color={dangers ? "text-red-300" : "text-shPrimary"}/>
            <HealthMini label="Warnings" value={warns} color={warns ? "text-shAccent" : "text-shPrimary"}/>
            <HealthMini label="Info" value={infos} color="text-shSecondary"/>
            <HealthMini label="Backup age" value={report.latest_age_hours == null ? "—" : `${Math.round(report.latest_age_hours)}h`} color={report.latest_age_hours > 24 ? "text-shAccent" : "text-shPrimary"}/>
          </div>
          <div className="bg-[var(--sh-card-base)]/70 border border-shBorder rounded p-3 mb-3 text-[13px] text-shTextMuted">
            <p><span className="text-shTextMuted font-black uppercase tracking-widest">Latest in-app backup:</span> {file.exists ? `${file.size_mb} MB` : "not found"}</p>
            <p className="truncate"><span className="text-shTextMuted font-black uppercase tracking-widest">File:</span> {file.path || "—"}</p>
            <p><span className="text-shTextMuted font-black uppercase tracking-widest">Critical counts:</span> clients {report.critical_counts?.clients ?? "—"} · dogs {report.critical_counts?.dogs ?? "—"} · bookings {report.critical_counts?.bookings ?? "—"} · archive {report.critical_counts?.bookings_archive ?? "—"}</p>
          </div>
          <div className="space-y-2 mb-3">
            {(report.checklist || []).map(item => (
              <div key={item.key} className="flex gap-2 bg-[var(--sh-card-base)]/50 border border-shBorder rounded p-2">
                <i className={`fas ${item.ok === true ? "fa-circle-check text-shPrimary" : item.ok === false ? "fa-triangle-exclamation text-red-300" : "fa-square-check text-shSecondary"} mt-0.5`}/>
                <div>
                  <p className="text-[12px] font-black uppercase tracking-widest text-shText">{item.label}</p>
                  <p className="text-[12px] text-shTextMuted">{item.detail}</p>
                </div>
              </div>
            ))}
          </div>
          {warnings.length > 0 && (
            <div className="space-y-2 mb-3">
              {warnings.map(w => (
                <div key={w.key} className={`rounded border p-2 ${w.severity === "danger" ? "bg-red-500/10 border-red-500/40" : w.severity === "warn" ? "bg-shAccent/10 border-shAccent/40" : "bg-shSecondary/10 border-shSecondary/40"}`}>
                  <p className={`text-[11px] font-black uppercase tracking-widest ${w.severity === "danger" ? "text-red-300" : w.severity === "warn" ? "text-shAccent" : "text-shSecondary"}`}><i className="fas fa-circle-info mr-1"/>{w.title}</p>
                  <p className="text-[12px] text-shTextMuted mt-0.5">{w.detail}</p>
                </div>
              ))}
            </div>
          )}
          <div className="flex flex-wrap gap-2">
            <button onClick={load} disabled={busy} data-testid="pre-update-refresh"
                    className="bg-[var(--sh-card-base)] border border-shBorder text-shTextMuted px-4 py-2 rounded text-[12px] font-black uppercase tracking-widest hover:border-shSecondary disabled:opacity-50">
          ...
          {valid && (
            <div className={`mt-3 rounded border p-3 ${valid.ok ? "bg-shPrimary/10 border-shPrimary/40" : "bg-shAccent/10 border-shAccent/40"}`} data-testid="latest-backup-validation-result">
              <p className={`text-[12px] font-black uppercase tracking-widest ${valid.ok ? "text-shPrimary" : "text-shAccent"}`}>{valid.ok ? "Backup parse test passed" : "Backup parse test needs review"}</p>
              <p className="text-[13px] text-shTextMuted">Version {valid.version || "—"} · {valid.collections || 0} collections · {valid.total_docs || 0} documents</p>
              {!!(valid.warnings || []).length && <p className="text-[12px] text-shAccent mt-1">{valid.warnings.length} warning(s). Open the validation details before relying on this file.</p>}
            </div>
          )}
          {history.length > 0 && (
            <details className="mt-3">
              <summary className="cursor-pointer text-[12px] text-shTextMuted font-black uppercase tracking-widest">Recent validation history</summary>
              <div className="mt-2 space-y-1">
                {history.map(h => <div key={h.id} className="text-[12px] text-shTextMuted flex justify-between gap-2 bg-[var(--sh-card-base)]/50 rounded px-2 py-1"><span>{new Date(h.created_at).toLocaleString()}</span><span className={h.ok ? "text-shPrimary" : "text-shAccent"}>{h.ok ? "OK" : "Review"} · {h.total_docs || 0} docs</span></div>)}
              </div>
            </details>
          )}
        </>
...
function ProductionHealthPanel() {
  ...
  return (
    <div className="border border-shBorder rounded-xl p-4 bg-[var(--sh-card-base)]" data-testid="production-health-panel">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div>
          <h4 className="text-sm font-black text-shPrimary uppercase tracking-widest"><i className="fas fa-heart-pulse mr-2"/>Production Health</h4>
          <p className="text-[13px] text-shTextMuted">Quick safety check before updates: database, backups, disk, email, and risky env defaults.</p>
        </div>
        <button onClick={load} className="bg-[var(--sh-card-base)] border border-shBorder text-shTextMuted px-3 py-1.5 rounded text-[12px] font-black uppercase tracking-widest hover:border-shPrimary"><i className="fas fa-rotate mr-1"/>Refresh</button>
      </div>
      {err && <p className="text-red-400 text-[13px]">{err}</p>}
      {data && (
        <>
          <div className="grid grid-cols-3 gap-2 mb-3">
            <HealthMini label="Danger" value={bad} color={bad ? "text-red-300" : "text-shPrimary"}/>
            <HealthMini label="Warnings" value={warn} color={warn ? "text-shAccent" : "text-shPrimary"}/>
            <HealthMini label="Collections" value={Object.keys(data.counts || {}).length} color="text-shSecondary"/>
          </div>
          <div className="space-y-2">
            {checks.map(c => (
              <div key={c.key} className={`rounded border p-2 ${c.ok ? "bg-shPrimary/10 border-shPrimary/40" : c.severity === "danger" ? "bg-red-500/10 border-red-500/40" : "bg-shAccent/10 border-shAccent/40"}`}>
                <p className={`text-[11px] font-black uppercase tracking-widest ${c.ok ? "text-shPrimary" : c.severity === "danger" ? "text-red-300" : "text-shAccent"}`}><i className={`fas ${c.ok ? "fa-check-circle" : "fa-triangle-exclamation"} mr-1`}/>{c.label}</p>
                <p className="text-[12px] text-shTextMuted mt-0.5">{c.detail}</p>
              </div>
            ))}
          </div>
          <p className="text-[11px] text-shTextMuted mt-2 uppercase tracking-widest font-black">Checked {new Date(data.checked_at).toLocaleString()}</p>
        </>
      )}
    </div>
  );
...
function HealthMini({ label, value, color }) {
  return <div className="bg-[var(--sh-card-base)]/60 border border-shBorder rounded p-2"><p className="text-[10px] text-shTextMuted font-black uppercase tracking-widest">{label}</p><p className={`text-lg font-black ${color}`}>{value}</p></div>;
}
...
function DiskUsagePanel() {
  ...
  const tone = (v) => v === "danger" ? "text-red-400 bg-red-500/15 border-red-500/40"
                    : v === "warn"   ? "text-shAccent bg-shAccent/15 border-shAccent/40"
                    :                  "text-shPrimary bg-shPrimary/15 border-shPrimary/40";
  return (
    <div data-testid="disk-usage-panel">
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-sm font-black text-shSecondary uppercase tracking-widest"><i className="fas fa-hard-drive mr-2"/>Disk Usage</h4>
        <button onClick={load} disabled={busy} data-testid="disk-usage-refresh"
                className="text-[12px] font-black uppercase tracking-widest text-shTextMuted hover:text-shText disabled:opacity-50">
          <i className={`fas fa-rotate ${busy ? "fa-spin" : ""} mr-1`}/>Refresh
        </button>
      </div>
      <p className="text-[14px] text-shTextMuted mb-3 leading-relaxed">
        Live snapshot of every drive/path this container can see. Mount more host drives into <code className="text-shSecondary">docker-compose.yml</code> to make them appear here.
      </p>
      {err && <p className="text-red-400 text-[14px] mb-2">{err}</p>}
      {!data && !err && <p className="text-[14px] text-shTextMuted"><i className="fas fa-circle-notch fa-spin mr-2"/>Reading…</p>}
      {data && data.mountpoints.length === 0 && <p className="text-[14px] text-shTextMuted">No mountpoints found.</p>}
      <div className="space-y-2">
        {(data?.mountpoints || []).map((m) => (
          <div key={m.path} className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3"
               data-testid={`disk-row-${m.path}`}>
            <div className="flex items-baseline justify-between gap-3 mb-1">
              <div className="min-w-0">
                <p className="text-sm font-black text-shText uppercase truncate">{m.label}</p>
                <p className="text-[12px] text-shTextMuted font-mono truncate">{m.path} · {m.fs_type}{m.likely_ephemeral ? " · ephemeral!" : ""}</p>
              </div>
              <span className={`text-[12px] font-black uppercase tracking-widest px-2 py-0.5 rounded border ${tone(m.verdict)}`}>
                {m.pct_used}% used
              </span>
            </div>
            <div className="h-2 rounded-full bg-shSurfaceRaised overflow-hidden">
              <div className={`h-full ${m.verdict === "danger" ? "bg-red-500" : m.verdict === "warn" ? "bg-shAccent" : "bg-shPrimary"}`}
                   style={{ width: `${Math.min(100, m.pct_used)}%` }} />
            </div>
            <p className="text-[12px] text-shTextMuted font-black uppercase tracking-widest mt-1">
      ...
      {data && (
        <p className="text-[11px] text-shTextMuted mt-2 uppercase tracking-widest font-black">
...
function AutoBackupPanel() {
  ...
  return (
    <div className="border-t border-shBorder pt-6" data-testid="auto-backup-panel">
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-sm font-black text-shPrimary uppercase tracking-widest"><i className="fas fa-clock-rotate-left mr-2"/>Auto-Backup · Nightly</h4>
        <span className={`text-[12px] font-black uppercase tracking-widest px-2 py-1 rounded ${cfg.enabled ? "bg-shPrimary/15 text-shPrimary" : "bg-gray-500/15 text-shTextMuted"}`}
              data-testid="auto-backup-status">
          {cfg.enabled ? "On" : "Off"}
        </span>
      </div>
      <p className="text-[14px] text-shTextMuted mb-3 leading-relaxed">
        Writes and verifies a gzipped JSON snapshot of <strong className="text-shText">every business collection</strong> at the scheduled hour. Backups are stored on the host-mounted persistent backup folder, so they survive container rebuilds.
      </p>

      {err && <p className="text-red-400 text-[14px] mb-2" data-testid="auto-backup-error">{err}</p>}
      {msg && <p className="text-shPrimary text-[14px] mb-2" data-testid="auto-backup-msg"><i className="fas fa-check mr-1"/>{msg}</p>}
      ...
      <div className="space-y-3">
        <label className="flex items-center gap-3 cursor-pointer">
          <input type="checkbox" checked={!!draft.enabled}
                 ...
                 className="w-5 h-5 accent-shPrimary"/>
          <span className="text-[14px] font-black text-shText uppercase tracking-widest">Enable nightly backup</span>
        </label>

        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Hour (0-23)</span>
          ...
          <label className="block">
            <span className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Minute</span>
        ...
        <label className="block">
          <span className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Persistent backup folder</span>
          <input type="text" value={draft.path || cfg.backup_root || "/app/backups"}
                 ...
                 className="mt-1 block w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm font-mono"/>
          <p className="text-[12px] mt-1 text-shTextMuted">
            Must be <span className="font-mono text-shTextMuted">{cfg.backup_root || "/app/backups"}</span> or a subfolder inside it. This maps to the host's <span className="font-mono text-shTextMuted">./backups</span> folder.
          </p>
          {cfg.path_info && (
            <p className={`text-[12px] mt-1 ${targetVerdict === "danger" ? "text-red-400" : targetVerdict === "warn" ? "text-shAccent" : "text-shPrimary"}`}>
        ...
        <label className="block">
          <span className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Retain (days)</span>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={runNow} disabled={running} data-testid="auto-backup-run-now"
                  className="bg-shSecondary text-bgHeader px-5 py-2 rounded font-black text-[13px] uppercase tracking-widest shadow disabled:opacity-50">
      ...
      {cfg.last_run && (
        <div className="mt-4 bg-[var(--sh-card-base)] border border-shBorder rounded p-3 text-[13px] text-shTextMuted" data-testid="auto-backup-last-run">
          <p className="text-[12px] font-black uppercase tracking-widest text-shTextMuted">Last run</p>
          <p>
            {new Date(cfg.last_run).toLocaleString()} · {cfg.last_ok ? <span className="text-shPrimary font-black">OK</span> : <span className="text-red-400 font-black">FAILED</span>}
            {cfg.last_verified && <span className="text-shSecondary font-black"> · VERIFIED</span>}
            {cfg.last_size_bytes ? ` · ${fmtBytes(cfg.last_size_bytes)}` : ""}
          </p>
          {cfg.last_file && <p className="text-[12px] text-shTextMuted font-mono mt-1 truncate">{cfg.last_file}</p>}
      ...
      {runs.length > 0 && (
        <details className="mt-3" data-testid="auto-backup-history">
          <summary className="text-[12px] font-black uppercase tracking-widest text-shTextMuted cursor-pointer hover:text-shText">
            Show last {runs.length} run{runs.length === 1 ? "" : "s"}
          </summary>
          <div className="mt-2 space-y-1">
            {runs.map(r => (
              <div key={r.id} className="bg-[var(--sh-card-base)]/60 border border-shBorder/40 rounded px-3 py-2 text-[12px] flex items-center justify-between gap-3">
                <span className="font-mono text-shTextMuted truncate">{new Date(r.started_at).toLocaleString()}</span>
                <span className="text-shTextMuted font-black uppercase tracking-widest">{r.trigger}</span>
                <span className={r.ok ? "text-shPrimary font-black" : r.status === "skipped" ? "text-shAccent font-black" : "text-red-400 font-black"}>
...
function UserMigrationSection() {
  ...
  return (
    <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl p-4 space-y-3" data-testid="user-migration-section">
      <div>
        <p className="text-shPrimary font-black uppercase tracking-widest text-[14px]"><i className="fas fa-key mr-2"/>Migrate User Logins (with passwords)</p>
        <p className="text-[14px] text-shTextMuted mt-1 leading-relaxed">
          Move clients to a new host without making them reset their passwords. On your <span className="text-shText font-black">old instance</span>, click Export. On your <span className="text-shText font-black">new instance</span>, click Import and pick that file. Existing passwords keep working.
        </p>
      </div>
      <div className="flex flex-wrap gap-2">
        <button onClick={downloadExport} disabled={busy} data-testid="user-export-btn"
                className="bg-shPrimary text-bgHeader px-5 py-2.5 rounded font-black text-[14px] uppercase tracking-widest shadow-lg disabled:opacity-50">
          <i className="fas fa-download mr-2"/>Export Users
        </button>
        <label className="bg-shSecondary text-shText px-5 py-2.5 rounded font-black text-[14px] uppercase tracking-widest shadow-lg cursor-pointer hover:bg-shSecondary/90 disabled:opacity-50">
      ...
      {msg && (
        <div className={`text-[14px] font-black uppercase tracking-widest p-2 rounded ${msg.includes("✓") ? "bg-shPrimary/15 text-shPrimary" : "bg-red-500/15 text-red-400"}`} data-testid="user-migration-msg">
          {msg}
        </div>
      )}
      {result && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[14px]" data-testid="user-migration-result">
          <StatChip label="Inserted" value={result.inserted} color="text-shPrimary" />
          <StatChip label="Updated" value={result.updated} color="text-shSecondary" />
          <StatChip label="Skipped (Self)" value={result.skipped_self} color="text-shTextMuted" />
          <StatChip label="Skipped (No Hash)" value={result.skipped_no_email_or_hash} color="text-shTextMuted" />
        </div>
      )}
    </div>
  );
...
function BulkClaimEmailsSection() {
  ...
  return (
    <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl p-4 space-y-3" data-testid="bulk-claim-section">
      <div>
        <p className="text-shSecondary font-black uppercase tracking-widest text-[14px]"><i className="fas fa-paper-plane mr-2"/>Mass Claim Emails (Recovery)</p>
        <p className="text-[14px] text-shTextMuted mt-1 leading-relaxed">
      ...
      <button onClick={send} disabled={busy} data-testid="bulk-claim-btn"
              className="bg-shSecondary text-shText px-6 py-3 rounded font-black text-[14px] uppercase tracking-widest shadow-lg disabled:opacity-50">
      ...
      {result && (
        <div className="bg-[var(--sh-card-base)] rounded p-3 space-y-2" data-testid="bulk-claim-result">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[14px]">
            <StatChip label="Total Clients" value={result.total_clients} color="text-shTextMuted" />
            <StatChip label="Sent ✓" value={result.sent_count} color="text-shPrimary" />
            <StatChip label="No Email" value={result.skipped_no_email_count} color="text-shTextMuted" />
            <StatChip label="Already Linked" value={result.skipped_already_linked_count} color="text-shTextMuted" />
          </div>
          {result.errors_count > 0 && (
            <div className="bg-red-500/10 border border-red-500/30 rounded p-2 text-[15px]">
              ...
              <ul className="text-shTextMuted space-y-0.5">
                {result.errors.slice(0,5).map((e,i)=>(
                  <li key={i}>· {e.name} ({e.email}): {e.error}</li>
                ))}
                {result.errors.length > 5 && <li className="text-shTextMuted">…and {result.errors.length-5} more</li>}
              </ul>
            </div>
          )}
        </div>
...
function StatChip({ label, value, color }) {
  return (
    <div className="bg-[var(--sh-card-base)] rounded px-2 py-2 flex flex-col items-center">
      <span className="text-[13px] text-shTextMuted font-black uppercase tracking-widest">{label}</span>
...
function PhotoCompressionPanel() {
  ...
  return (
    <div className="border-t border-shBorder pt-6" data-testid="photo-compress-panel">
      ...
      {status && (
        <div className="bg-[var(--sh-card-base)] border border-shBorder rounded p-4 mb-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-center" data-testid="photo-compress-stats">
          <div>
            <p className="text-[12px] uppercase tracking-widest text-shTextMuted font-black">Scanned</p>
            <p className="text-shSecondary text-2xl font-black">{status.scanned}</p>
          </div>
          <div>
            <p className="text-[12px] uppercase tracking-widest text-shTextMuted font-black">Compressed</p>
            <p className="text-shPrimary text-2xl font-black">{status.compressed}</p>
          </div>
          <div>
            <p className="text-[12px] uppercase tracking-widest text-shTextMuted font-black">Skipped</p>
            <p className="text-shTextMuted text-2xl font-black">{status.skipped}</p>
          </div>
          <div>
            <p className="text-[12px] uppercase tracking-widest text-shTextMuted font-black">Space saved</p>
      ...
      {status?.running && (
        <div className="bg-shSecondary/10 border border-shSecondary/40 rounded p-3 mb-3 flex items-center gap-3" data-testid="photo-compress-progress">
          <i className="fas fa-circle-notch fa-spin text-shSecondary text-xl"/>
          <div className="flex-1">
            <p className="text-[14px] font-black text-shSecondary uppercase tracking-widest">{stageLabel}</p>
            <p className="text-[14px] text-shTextMuted mt-0.5">Working in the background. You can leave this page and come back later.</p>
          </div>
        </div>
      )}
      ...
      {finished && !status.error_message && status.compressed > 0 && (
        <p className="text-shPrimary text-[14px] font-black mb-3" data-testid="photo-compress-done">
...
function CommandsPanel() {
  ...
  if (!meta) return <p className="text-shTextMuted text-sm">Loading…</p>;
  ...
  return (
    <div className="space-y-5 max-w-3xl" data-testid="commands-panel">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-sm font-black text-shSecondary uppercase tracking-widest"><i className="fas fa-graduation-cap mr-2"/>Service-Dog Command Library</h4>
          <p className="text-[14px] text-shTextMuted mt-1">Curate the master list every dog's curriculum is built from. Seeded with TADSAW service-dog commands.</p>
        </div>
        <button onClick={startNew} data-testid="cmd-new"
                className="bg-shPrimary text-bgHeader px-4 py-2 rounded font-black text-[15px] uppercase tracking-widest shadow"><i className="fas fa-plus mr-1"/>New</button>
      </div>

      {err && <div className="text-[15px] text-red-400 bg-red-500/10 rounded p-2 uppercase font-black">{err}</div>}

      {grouped.map(g => (
        <div key={g.key} className="bg-[var(--sh-card-base)]/40 border border-shBorder rounded">
          <div className="px-3 py-2 border-b border-shBorder" style={{background: g.color + "12"}}>
          ...
          <div className="divide-y divide-shBorder">
            {g.items.map(c => (
              <div key={c.id} className="px-3 py-2 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-black text-shText truncate">{c.name} {c.is_default && <span className="text-[13px] text-shTextMuted font-black tracking-widest ml-2">(default)</span>}</p>
                  <p className="text-[15px] text-shTextMuted truncate">{c.description}</p>
                </div>
                {c.video_url && <i className="fab fa-youtube text-red-500"/>}
                <button onClick={()=>setEdit({...c})} data-testid={`cmd-edit-${c.id}`} className="text-shSecondary hover:text-shText text-sm"><i className="fas fa-pen"/></button>
      ...
      {edit && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50" data-testid="cmd-modal">
          <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl w-full max-w-lg p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-base font-black text-shText uppercase italic">{edit.id?"Edit Command":"New Command"}</h4>
              <button onClick={()=>setEdit(null)} className="text-shTextMuted hover:text-shText"><i className="fas fa-times text-xl"/></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-[15px] font-black text-shTextMuted uppercase tracking-widest">Name *</label>
                <input value={edit.name} onChange={(e)=>setEdit({...edit, name:e.target.value})} data-testid="cmd-name"
                       className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm" />
              </div>
              <div>
                <label className="text-[15px] font-black text-shTextMuted uppercase tracking-widest">Category</label>
              ...
              <div>
                <label className="text-[15px] font-black text-shTextMuted uppercase tracking-widest">Description</label>
                <textarea value={edit.description||""} onChange={(e)=>setEdit({...edit, description:e.target.value})} rows={2}
                          className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm" />
              </div>
              <div>
                <label className="text-[15px] font-black text-shTextMuted uppercase tracking-widest">YouTube video URL (optional)</label>
                <input value={edit.video_url||""} onChange={(e)=>setEdit({...edit, video_url:e.target.value})} data-testid="cmd-video"
                       placeholder="https://youtu.be/..."
                       className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm" />
              </div>
              <div className="flex justify-end gap-3 pt-2">
                <button onClick={()=>setEdit(null)} className="text-shTextMuted font-black uppercase text-[15px] tracking-widest">Cancel</button>
                <button onClick={save} data-testid="cmd-save"
                        className="bg-shPrimary text-bgHeader px-6 py-2 rounded font-black text-[15px] uppercase tracking-widest shadow">Save</button>
              </div>
            </div>
          </div>
        </div>
...
function AutomationPanel() {
  ...
  return (
    <div className="space-y-5 max-w-2xl" data-testid="automation-panel">
      <div>
        <h4 className="text-lg font-black text-shText uppercase italic tracking-tight">Email Automation & Notifications</h4>
        <p className="text-[15px] text-shTextMuted font-black uppercase tracking-widest mt-1 normal-case">Background jobs that run automatically at most once per day, triggered the first time the admin dashboard loads each morning.</p>
      </div>

      {/* Sprint 105 — Per-step homework email toggle */}
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-4" data-testid="email-per-step-row">
        <div className="flex items-start gap-3">
          <i className="fas fa-list-check text-shSecondary text-xl mt-1 w-7 text-center"/>
          <div className="flex-1">
            <p className="text-shText font-black text-[14px] uppercase tracking-widest">Per-step homework emails</p>
            <p className="text-[14px] text-shTextMuted normal-case mt-1">
          ...
          <button onClick={toggleEmailPerStep} disabled={stepBusy || emailPerStep === null}
                  ...
                  className={`shrink-0 px-3 py-1.5 rounded text-[12px] font-black uppercase tracking-widest transition ${
                    emailPerStep ? "bg-shPrimary text-bgHeader hover:bg-shPrimary/90"
                                  : "bg-[var(--sh-card-base)] text-shTextMuted hover:bg-shSurfaceRaised border border-shBorder"
                  } disabled:opacity-50`}>
      ...
      <div className="space-y-3">
        <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-4 flex items-start gap-3">
          <i className="fas fa-cake-candles text-shPrimary text-xl mt-1 w-7 text-center"/>
          <div className="flex-1">
            <p className="text-shText font-black text-[14px] uppercase tracking-widest">Dog Birthday Cards</p>
            <p className="text-[14px] text-shTextMuted normal-case mt-1">Emails the owner a celebratory card with their dog's photo on the dog's birthday. De-duped per dog per year.</p>
          </div>
          <button onClick={toggleBday} disabled={bdayBusy || bdayOn === null}
                  data-testid="birthday-email-toggle"
                  className={`shrink-0 px-3 py-1.5 rounded text-[12px] font-black uppercase tracking-widest transition ${
                    bdayOn ? "bg-shPrimary text-bgHeader hover:bg-shPrimary/90"
                            : "bg-[var(--sh-card-base)] text-shTextMuted hover:bg-shSurfaceRaised border border-shBorder"
                  } disabled:opacity-50`}>
        ...
        <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-4 flex items-start gap-3">
          <i className="fas fa-syringe text-shAccent text-xl mt-1 w-7 text-center"/>
          <div className="flex-1">
            <p className="text-shText font-black text-[14px] uppercase tracking-widest">Vaccine Renewal Nudge</p>
            <p className="text-[14px] text-shTextMuted normal-case mt-1">30 days before any vaccine expires, the owner gets a single email listing which renewals are due with a link to upload the updated record.</p>
          </div>
          <span className="bg-shAccent/15 text-shAccent text-[12px] font-black uppercase tracking-widest px-2 py-1 rounded">On</span>
        </div>
        <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-4 flex items-start gap-3">
          <i className="fas fa-envelope-open-text text-shSecondary text-xl mt-1 w-7 text-center"/>
          <div className="flex-1">
            <p className="text-shText font-black text-[14px] uppercase tracking-widest">Homework Daily Roll-up</p>
            <p className="text-[14px] text-shTextMuted normal-case mt-1">One email at the end of each day summarising every step a client checked off — grouped by dog + plan. Replaces per-step spam.</p>
          </div>
          <span className="bg-shPrimary/15 text-shPrimary text-[12px] font-black uppercase tracking-widest px-2 py-1 rounded">On</span>
        </div>
      </div>

      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-4">
        <p className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Manual trigger</p>
        <p className="text-[14px] text-shTextMuted normal-case mt-1">Force-runs all daily jobs right now (clears today's "already ran" flag). Useful for testing.</p>
        <button onClick={run} disabled={busy} data-testid="run-daily-jobs-btn"
                className="mt-3 bg-shSecondary text-shText px-5 py-2 rounded font-black text-[15px] uppercase tracking-widest hover:bg-shSecondary/90 disabled:opacity-50">
        ...
        {last && (
          <div className="mt-3 grid grid-cols-2 gap-3 text-[14px] font-black uppercase tracking-widest">
            <div className="bg-[var(--sh-card-base)] rounded p-3">
              <p className="text-shTextMuted">Birthdays</p>
              <p className="text-shPrimary text-[20px]">{last.birthdays?.sent ?? 0}<span className="text-shTextMuted text-[14px] ml-1">sent</span></p>
              <p className="text-gray-600 text-[13px]">{last.birthdays?.skipped ?? 0} skipped</p>
            </div>
            <div className="bg-[var(--sh-card-base)] rounded p-3">
              <p className="text-shTextMuted">Vaccine Renewals</p>
              <p className="text-shAccent text-[20px]">{last.vaccine_expiry?.sent ?? 0}<span className="text-shTextMuted text-[14px] ml-1">sent</span></p>
...
function TriviaPanel() {
  ...
  const tagColor = {
    ...
    history: "text-blue-300", training: "text-shAccent", anatomy: "text-pink-300",
    fun: "text-shPrimary", myth: "text-red-300",
  };

  return (
    <div className="border-t border-shBorder pt-6 mt-6" data-testid="trivia-panel">
      <div className="flex justify-between items-center flex-wrap gap-2 mb-3">
        <div>
          <h3 className="text-shText font-black uppercase italic"><i className="fas fa-puzzle-piece text-shSecondary mr-2"/>Dog Trivia</h3>
          <p className="text-[12px] text-shTextMuted">{active} active question{active === 1 ? "" : "s"} · {rows.length} total · Wordle-style daily card on client portal</p>
        </div>
        <div className="flex gap-1 bg-[var(--sh-card-base)] rounded p-1 border border-shBorder">
          {[["leaderboard","Leaderboard","fa-trophy"],["rewards","Goals","fa-bullseye"],["questions","Questions","fa-list"]].map(([k,l,i])=>(
            <button key={k} onClick={()=>setView(k)} data-testid={`trivia-view-${k}`}
                    className={`px-3 py-1 rounded text-[11px] font-black uppercase tracking-widest ${view===k ? "bg-shSecondary text-bgHeader" : "text-shTextMuted hover:text-shText"}`}>
      ...
      {view === "leaderboard" && (
        <div className="space-y-3" data-testid="trivia-admin-leaderboard">
          {!lb ? <p className="text-shTextMuted text-sm">Loading…</p> : (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                ...
                <Kpi label="Pending perks" value={lb.pending_milestones.length} color="shAccent" testId="trivia-lb-pending"/>
                <Kpi label="Top streak" value={lb.players[0]?.current_streak ?? 0} color="shPrimary" testId="trivia-lb-topstreak"/>
              </div>

              {lb.pending_milestones.length > 0 && (
                <div className="bg-shAccent/5 border border-shAccent/40 rounded-xl p-3" data-testid="trivia-pending-list">
                  ...
                  <div className="space-y-1">
                    {lb.pending_milestones.map((m,i) => (
                      <div key={`${m.client_id}-${m.days}-${m.earned_on}-${i}`}
                           className="flex justify-between items-center bg-[var(--sh-card-base)] rounded p-2 text-[13px]"
                           data-testid={`trivia-perk-${m.client_id}-${m.days}`}>
                        <div>
                          <p className="text-shText font-bold">{m.client_name}{m.dogs?.length ? <span className="text-shTextMuted font-normal"> · {m.dogs.join(", ")}</span> : null}</p>
                          <p className="text-[11px] text-shTextMuted">
                            <strong className="text-shAccent">{m.days}-day streak</strong> · earned {m.earned_on}
                          </p>
                        </div>
              ...
              {lb.players.length === 0 ? (
                <p className="text-shTextMuted text-sm italic">No one has played yet — the daily card lives on the client portal home.</p>
              ) : (
                <div className="bg-[var(--sh-card-base)] rounded-xl border border-shBorder overflow-hidden">
                  <table className="w-full text-[13px]">
                    <thead className="text-[10px] font-black uppercase tracking-widest text-shTextMuted border-b border-shBorder bg-[var(--sh-card-base)]/60">
                      <tr>
                    ...
                    <tbody>
                      {lb.players.map(p => (
                        <tr key={p.client_id} className="border-b border-shBorder/40 hover:bg-[var(--sh-card-base)]/40"
                            data-testid={`trivia-lb-player-${p.client_id}`}>
                          <td className="px-3 py-2 text-shTextMuted font-black">#{p.rank}</td>
                          <td className="px-3 py-2">
                            <p className="text-shText font-bold">{p.name}</p>
                            <p className="text-[11px] text-shTextMuted">{p.email}</p>
                          </td>
                          <td className="px-3 py-2 text-shTextMuted text-[12px] truncate max-w-[160px]">{p.dogs.join(", ") || "—"}</td>
                          <td className="px-3 py-2 text-right">
                            <span className={`font-black ${p.current_streak >= 7 ? "text-shPrimary" : p.current_streak >= 3 ? "text-shAccent" : "text-shText"}`}>
                              {p.current_streak}d
                            </span>
                          </td>
                          <td className="px-3 py-2 text-right text-shTextMuted">{p.best_streak}d</td>
                          <td className="px-3 py-2 text-right text-shTextMuted">{p.total_correct}/{p.total_attempts}</td>
                          <td className="px-3 py-2 text-right text-shTextMuted hidden sm:table-cell">{p.accuracy_pct}%</td>
                          <td className="px-3 py-2 text-shTextMuted text-[12px] hidden md:table-cell">{p.last_played}</td>
                          <td className="px-3 py-2 text-[11px] text-shTextMuted">
                            {p.milestones && p.milestones.length > 0 ? (
                              <span className="space-x-1">
                                {p.milestones.map((m,i)=>(
                                  <span key={i} className={`inline-block px-1.5 py-0.5 rounded ${m.redeemed_at ? "bg-[var(--sh-card-base)] text-shTextMuted" : "bg-shAccent/15 text-shAccent border border-shAccent/30 font-bold"}`}
      ...
      {view === "rewards" && (
        <div className="space-y-3" data-testid="trivia-rewards-view">
          <div className="bg-shPrimary/5 border border-shPrimary/30 rounded-xl p-3 text-[13px] text-shTextMuted">
            <i className="fas fa-circle-info text-shPrimary mr-2"/>
            Pick your own streak goals + perks. Each entry is awarded when a client's
            <strong className="text-shPrimary"> current daily-trivia streak </strong> reaches or passes the day count.
            The label is what the client sees on their portal and what shows in your Rewards Center pending list.
          </div>
          {!rewards ? <p className="text-shTextMuted text-sm">Loading…</p> : (
            <>
              <div className="space-y-2">
                {rewards.length === 0 && (
                  <p className="text-shTextMuted italic text-sm" data-testid="trivia-rewards-empty">No goals set — clients won't earn perks. Tap "Add goal" below.</p>
                )}
                {rewards.map((r, i) => (
                  <div key={i} className="bg-[var(--sh-card-base)] rounded-lg border border-shBorder p-3 grid grid-cols-12 gap-2 items-start" data-testid={`trivia-reward-${i}`}>
                    <label className="col-span-3 sm:col-span-2 block">
                      <span className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Day #</span>
                    ...
                    <label className="col-span-9 sm:col-span-7 block">
                      <span className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Perk message (clients see this)</span>
                    ...
                    <label className="col-span-9 sm:col-span-2 block">
                      <span className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Tag (internal)</span>
                    ...
                    <label className="col-span-6 sm:col-span-2 block">
                      <span className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Auto credit</span>
                    ...
                    <label className="col-span-3 sm:col-span-1 block">
                      <span className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Credits</span>
                    ...
                    <div className="col-span-3 sm:col-span-1 flex justify-end items-center pt-5">
                      <button onClick={() => removeReward(i)}
                              ...
                              className="text-shTextMuted hover:text-red-400 p-2">
                        <i className="fas fa-trash"/>
                      </button>
                    </div>
                  </div>
              ...
              <div className="flex justify-between flex-wrap gap-2">
                <div className="flex gap-2">
                  <button onClick={addReward} data-testid="trivia-reward-add"
                          className="bg-[var(--sh-card-base)] border border-shBorder text-gray-200 hover:border-shSecondary px-3 py-1.5 rounded text-[12px] font-black uppercase tracking-widest">
                    <i className="fas fa-plus mr-1"/>Add goal
                  </button>
                  <button onClick={resetRewards} data-testid="trivia-reward-reset"
                          className="text-shTextMuted hover:text-shAccent px-2 py-1.5 text-[12px] font-black uppercase tracking-widest">
                ...
                <button onClick={saveRewards} disabled={savingRewards}
                        ...
                        className="bg-shPrimary text-bgHeader px-4 py-1.5 rounded text-[12px] font-black uppercase tracking-widest disabled:opacity-50">
          ...
          {rows.length === 0 ? (
            ...
            <div key={q.id} className={`flex items-start gap-2 p-2 rounded border ${q.active ? "border-shBorder bg-[var(--sh-card-base)]/50" : "border-shBorder/40 bg-[var(--sh-card-base)]/20 opacity-50"}`} data-testid={`trivia-row-${q.id}`}>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-shTextMuted mt-0.5">
                  <span className={`font-black uppercase ${tagColor[q.tag] || "text-shTextMuted"}`}>{q.tag}</span>
                  <span className="mx-1 text-gray-600">·</span>
                  <span className="text-shTextMuted">{q.difficulty}</span>
                  <span className="mx-1 text-gray-600">·</span>
                  <span className="text-shTextMuted">A: {q.choices[q.correct_index]}</span>
                  {q.source === "manual" && <><span className="mx-1 text-gray-600">·</span><span className="text-shPrimary font-black"><i className="fas fa-user-pen mr-0.5"/>mine</span></>}
                  {q.times_used > 0 && <><span className="mx-1 text-gray-600">·</span><span className="text-shTextMuted">used {q.times_used}x</span></>}
                </p>
              </div>
              <button onClick={()=>setEditing({ mode: "edit", q })}
                      data-testid={`trivia-edit-${q.id}`}
                      className="text-shTextMuted hover:text-shSecondary text-[12px] px-1">
                <i className="fas fa-pen-to-square"/>
              </button>
              <button onClick={()=>toggle(q)} data-testid={`trivia-toggle-${q.id}`}
                      className={`text-[11px] font-black uppercase tracking-widest px-2 py-1 rounded ${q.active ? "text-shPrimary hover:bg-shPrimary/15" : "text-shTextMuted hover:bg-gray-500/15"}`}>
                {q.active ? "active" : "off"}
              </button>
              <button onClick={()=>del(q)} data-testid={`trivia-delete-${q.id}`}
                      className="text-shTextMuted hover:text-red-400 text-[12px] px-1">
                <i className="fas fa-trash"/>
              </button>
            </div>
          ))}
...
function Kpi({ label, value, color = "white", testId }) {
  return (
    <div className="bg-[var(--sh-card-base)] rounded-lg p-3 border border-shBorder" data-testid={testId}>
...
function TriviaQuestionEditor({ initial, onClose, onSaved }) {
  ...
  return (
    <div className="bg-[var(--sh-card-base)] rounded-xl border border-shPrimary/40 p-4 mb-3 space-y-3" data-testid="trivia-q-editor">
      <div className="flex justify-between items-center">
        <p className="text-[12px] font-black uppercase tracking-widest text-shPrimary">
          <i className={`fas ${isEdit ? "fa-pen-to-square" : "fa-plus"} mr-2`}/>
          {isEdit ? "Edit question" : "New question"}
        </p>
        <button onClick={onClose} data-testid="trivia-q-close"
                className="text-shTextMuted hover:text-red-400 text-sm"><i className="fas fa-xmark"/></button>
      </div>
      {err && <div className="bg-red-500/10 border border-red-400 text-red-300 rounded p-2 text-[13px]">{err}</div>}
      <label className="block">
        <span className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Question</span>
      ...
      <div className="space-y-2" data-testid="trivia-q-choices">
        <p className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Choices — tap the radio to mark correct</p>
        {choices.map((c, i) => (
          <div key={i} className={`flex items-center gap-2 bg-[var(--sh-card-base)] border rounded p-2 ${correctIndex === i ? "border-shPrimary" : "border-shBorder"}`}>
            ...
            <span className="text-[11px] font-black w-5 text-shTextMuted">{["A","B","C","D"][i]}</span>
            <input type="text" value={c} onChange={e=>updateChoice(i, e.target.value)} maxLength={80}
                   ...
                   className="flex-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-1.5 text-shText text-sm"/>
            {correctIndex === i && <span className="text-[10px] font-black text-shPrimary uppercase">correct</span>}
          </div>
        ))}
      </div>
      <div className="grid grid-cols-3 gap-2">
        <label className="block">
          <span className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Difficulty</span>
        ...
        <label className="block">
          <span className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Tag</span>
        ...
        <label className="flex items-end gap-2 pb-2">
          <input type="checkbox" checked={active} onChange={e=>setActive(e.target.checked)}
                 ...
                 className="w-4 h-4 accent-shPrimary"/>
          <span className="text-[12px] font-black uppercase tracking-widest text-shTextMuted">Active</span>
        </label>
      </div>
      <div className="flex justify-end gap-2">
        <button onClick={onClose} data-testid="trivia-q-cancel"
                className="bg-[var(--sh-card-base)] border border-shBorder px-3 py-1.5 rounded text-[12px] font-black uppercase tracking-widest text-shTextMuted hover:border-red-400">
          Cancel
        </button>
...
function DogFactsPanel() {
  ...
  return (
    <div className="border-t border-shBorder pt-6" data-testid="dog-facts-panel">
      <h4 className="text-sm font-black text-shPrimary uppercase tracking-widest mb-2"><i className="fas fa-paw mr-2"/>Dog Fact of the Day</h4>
      <p className="text-[14px] text-shTextMuted mb-3 leading-relaxed">
        One fact rotates daily on the client portal and your dashboard. <strong className="text-shText">{activeCount}</strong> facts in active rotation — roughly <strong>{Math.round(activeCount / 30 * 10) / 10} months</strong> of unique content before anything repeats.
      </p>

      {today && (
        <div className="bg-[var(--sh-card-base)] border border-shPrimary/30 rounded-lg p-3 mb-4" data-testid="dog-fact-today-preview">
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-shPrimary mb-1">Today's pick</p>
          <p className="text-shText text-[14px]"><span className="text-xl mr-2">{today.emoji}</span>{today.text}</p>
        </div>
      )}
      ...
      <CsvImportRow
        ...
        onComplete={load}
        borderColor="border-shSecondary/30"
        accentColor="text-shSecondary"
      />

      {/* Add new */}
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 mb-4">
        <p className="text-[12px] font-black uppercase tracking-widest text-shTextMuted mb-2"><i className="fas fa-plus mr-1"/>Add your own fact</p>
        <div className="flex gap-2 items-end flex-wrap">
          <input type="text" value={draft.emoji} onChange={(e)=>setDraft({...draft, emoji: e.target.value})}
                 ...
                 className="w-16 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-lg text-center"/>
          ...
          <input type="text" value={draft.text} onChange={(e)=>setDraft({...draft, text: e.target.value})}
                 ...
                 className="flex-1 min-w-[260px] bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm"/>
          <button onClick={add} disabled={!draft.text.trim()} data-testid="dog-facts-add"
                  className="bg-shPrimary text-bgHeader px-4 py-2 rounded font-black text-[12px] uppercase tracking-widest shadow disabled:opacity-50">
            Add
          </button>
        </div>
      </div>
      ...
      {/* List */}
      <div className="max-h-[420px] overflow-y-auto space-y-1 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-2">
        {visible.length === 0 && <p className="text-[13px] text-shTextMuted italic p-2">No facts match this filter.</p>}
        {visible.map(f => (
          <div key={f.id} className={`flex items-start gap-2 p-2 rounded ${f.active ? "" : "opacity-50"} hover:bg-[var(--sh-card-base)]/60`}
              ...
              {editing?.id === f.id ? (
                <div className="space-y-2">
                  <input value={editing.text} onChange={(e)=>setEditing({...editing, text: e.target.value})}
                         className="block w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm"
                         data-testid={`dog-fact-edit-text-${f.id}`}/>
                  <div className="flex gap-2">
                    <button onClick={saveEdit} className="text-[11px] font-black uppercase tracking-widest bg-shPrimary text-bgHeader px-3 py-1 rounded"
                            data-testid={`dog-fact-save-${f.id}`}>Save</button>
                    <button onClick={()=>setEditing(null)} className="text-[11px] font-black uppercase tracking-widest text-shTextMuted px-3 py-1 rounded">Cancel</button>
                  </div>
                </div>
              ) : (
                <>
                  <p className="text-[13px] text-shText leading-snug">{f.text}</p>
                  <p className="text-[10px] font-black uppercase tracking-widest text-shTextMuted mt-0.5">
            ...
            {editing?.id !== f.id && (
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={()=>toggle(f)} title={f.active ? "Disable" : "Activate"}
                        ...
                        className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded ${f.active ? "bg-shPrimary/15 text-shPrimary" : "bg-[var(--sh-card-base)] text-shTextMuted"}`}>
                  {f.active ? "On" : "Off"}
                </button>
                <button onClick={()=>setEditing(f)} title="Edit"
                        data-testid={`dog-fact-edit-${f.id}`}
                        className="text-shTextMuted hover:text-shText p-1"><i className="fas fa-pencil text-[12px]"/></button>
                <button onClick={()=>remove(f)} title="Delete"
                        data-testid={`dog-fact-delete-${f.id}`}
                        className="text-shTextMuted hover:text-red-400 p-1"><i className="fas fa-trash text-[12px]"/></button>
              </div>
            )}
          </div>
        ))}
