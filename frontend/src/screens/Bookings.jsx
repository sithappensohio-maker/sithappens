    ...
    <div className="space-y-6 animate-slide-in" data-testid="bookings-screen">
      <div className="flex justify-between items-center">
        ...
        <button onClick={()=>setShowModal(true)} data-testid="new-booking-button"
                className="bg-shGreen text-bgHeader px-5 py-2 rounded-lg text-[12px] font-black uppercase tracking-widest shadow-lg hover:bg-shGreen/90">+ New Booking</button>
      </div>
      ...
      <div className="bg-bgPanel rounded-xl border border-bgHover overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="text-[12px] text-gray-500 font-black uppercase">
          ...
          <tbody data-testid="bookings-body">
            ...
            {bookings.map(b => (
              <tr key={b.id} className="border-t border-bgHover/40">
                ...
                <td className="px-6 py-4 text-[12px] font-black uppercase text-gray-300">{b.service_type}</td>
                <td className="px-6 py-4 text-xs text-gray-300">{b.date}{b.end_date && b.end_date !== b.date ? ` → ${b.end_date}` : ""}</td>
                <td className="px-6 py-4"><span className={`text-[12px] font-black uppercase px-2 py-1 rounded ${statusStyle(b.status)}`}>{b.status}</span></td>
                <td className="px-6 py-4 text-right space-x-2">
                  <button onClick={()=>setEditing(b)} data-testid={`edit-${b.id}`} className="text-[12px] font-black uppercase text-shBlue hover:underline">Edit</button>
                  {b.status === "pending" && <>
                    <button onClick={()=>approve(b.id)} data-testid={`approve-${b.id}`} className="text-[12px] font-black uppercase text-shGreen hover:underline">Approve</button>
                    <button onClick={()=>reject(b.id)} className="text-[12px] font-black uppercase text-red-400 hover:underline">Reject</button>
                  </>}
                  {(b.status === "approved" || b.status === "pending") && <button onClick={()=>cancel(b.id)} className="text-[12px] font-black uppercase text-gray-400 hover:underline">Cancel</button>}
                </td>
              </tr>
            ))}
          </tbody>
