          ...
          {loading ? <p className="text-shTextMuted text-sm">Loading…</p> : visibleTemplates.length === 0 ? (
            ...
            <div className="grid gap-3" data-testid="templates-list">
              {visibleTemplates.map(t => (
                <div key={t.id} className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl p-5 shadow-lg" data-testid={`template-${t.id}`}>
              ...
              {visibleSubmissions.map(s => {
                ...
                return (
                  <button key={s.id} onClick={()=>setReviewer(s)} data-testid={`sub-${s.id}`}
                          className="text-left bg-[var(--sh-card-base)] border border-shBorder rounded-xl p-4 hover:border-shSecondary transition flex items-center gap-3 flex-wrap">
