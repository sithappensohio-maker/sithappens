    ...
    <div className="space-y-6 animate-slide-in" data-testid="homework-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1">Assign exercises to clients between sessions</p>
        </div>
        <button onClick={openNew} data-testid="add-homework-button" className="bg-shBlue text-white px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-shBlue/90">+ Assign Homework</button>
      </div>

      <div className="flex gap-2">
        {["all","assigned","completed"].map(k => (
          <button key={k} onClick={()=>setFilter(k)} data-testid={`hw-filter-${k}`}
                  className={`px-4 py-2 rounded text-[10px] font-black uppercase tracking-widest ${filter===k?"bg-shBlue text-white":"bg-bgPanel text-gray-400 border border-bgHover"}`}>
            {k} · {counts[k]}
          </button>
        ))}
      </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <span className={`text-[10px] font-black uppercase px-2 py-1 rounded tracking-widest ${h.status==="completed"?"bg-shGreen/15 text-shGreen":"bg-shOrange/15 text-shOrange"}`}>{h.status}</span>
                  {h.due_date && <span className="text-[10px] font-black uppercase tracking-widest text-gray-400"><i className="fas fa-calendar mr-1"/>Due {h.due_date}</span>}
                </div>
                <h4 className="text-lg font-black text-white uppercase tracking-tight">{h.title}</h4>
                <p className="text-[11px] text-shBlue font-black uppercase tracking-widest mt-1">{h.dog_name} · {h.client_name}</p>
                {h.instructions && <p className="text-sm text-gray-300 mt-2 whitespace-pre-wrap">{h.instructions}</p>}
                {h.video_url && <a href={h.video_url} target="_blank" rel="noreferrer" className="inline-block mt-2 text-[10px] text-shBlue hover:underline font-black uppercase tracking-widest"><i className="fas fa-video mr-1"/>Watch demo video</a>}
                {h.status === "completed" && (
                  <div className="mt-3 bg-shGreen/5 border border-shGreen/20 rounded p-3">
                    <p className="text-[10px] font-black text-shGreen uppercase tracking-widest mb-1"><i className="fas fa-check mr-1"/>Marked done {(h.completed_at||"").slice(0,10)}</p>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-bgPanel border border-bgHover rounded-2xl w-full max-w-lg p-6 md:p-8 shadow-2xl animate-slide-in">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Instructions</label>
                <textarea value={form.instructions} onChange={(e)=>setForm({...form, instructions:e.target.value})} rows={4} data-testid="hw-instructions"
                          className="w-full mt-1 bg-bgBase border border-bgHover rounded p-2 text-white text-sm" />
              </div>
              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Video URL (YouTube link to a demo, optional)</label>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button onClick={save} data-testid="save-homework" className="bg-shBlue text-white px-8 py-3 rounded font-black text-[10px] uppercase tracking-widest shadow-xl">Assign</button>
              </div>
            </div>
          </div>
        </div>
