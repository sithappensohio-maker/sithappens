...
const STATUS_META = {
  waiting:  { label: "Waiting",  cls: "bg-shSecondary/15 text-shSecondary",  icon: "fa-hourglass-half" },
  offered:  { label: "Offered",  cls: "bg-shAccent/15 text-shAccent ring-1 ring-shAccent/40", icon: "fa-hand-holding" },
  booked:   { label: "Booked",   cls: "bg-shPrimary/15 text-shPrimary",   icon: "fa-check" },
  declined: { label: "Declined", cls: "bg-shSurfaceRaised text-shTextMuted",     icon: "fa-xmark" },
  expired:  { label: "Expired",  cls: "bg-shSurfaceRaised text-shTextMuted",     icon: "fa-clock" },
  removed:  { label: "Removed",  cls: "bg-shSurfaceRaised text-shTextMuted",     icon: "fa-trash" },
};

const PRIORITY_META = {
  high:   { label: "High",   cls: "bg-red-500/15 text-red-300 ring-1 ring-red-400/40" },
  normal: { label: "Normal", cls: "bg-shSurfaceRaised text-shTextMuted" },
...
export default function Waitlist() {
  ...
  return (
    <div className="space-y-6 animate-slide-in" data-testid="waitlist-screen">
      <PageHero
        eyebrow={{ icon: "fa-hourglass-half", text: `${counts.waiting || 0} waiting · ${counts.offered || 0} offered`, color: "text-shSecondary" }}
        title="Waitlist."
      ...
      {loading ? <p className="text-shTextMuted text-sm">Loading…</p> : visible.length === 0 ? (
        <div className="bg-shAccent/5 border border-shAccent/25 rounded-xl p-10 text-center" data-testid="waitlist-empty">
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className="text-base text-shText font-black uppercase tracking-tight">{e.dog_name}</span>
                    ...
                    <p className="text-[13px] text-shTextMuted">
                      <span className="text-shTextMuted">For </span>{e.requested_date}
                      {e.requested_end_date && e.requested_end_date !== e.requested_date && <> → {e.requested_end_date}</>}
                      <span className="text-shTextMuted"> · {e.client_name}</span>
                    ...
                    {e.booking_id && (
                      <p className="text-[11px] text-shPrimary font-black uppercase tracking-widest mt-2">
                    ...
                    {e.status === "waiting" && (
                      <>
                        ...
                        <button onClick={()=>updateStatus(e, "offered")} data-testid={`offer-${e.id}`}
                                className="text-[12px] font-black uppercase tracking-widest bg-shAccent/20 text-shAccent border border-shAccent/40 px-3 py-1.5 rounded">
                    ...
                    {e.status === "offered" && (
                      <>
                        ...
                        <button onClick={()=>updateStatus(e, "declined")} data-testid={`decline-${e.id}`}
                                className="text-[12px] font-black uppercase tracking-widest bg-shSurfaceRaised text-shTextMuted px-3 py-1.5 rounded">
  ...
  return (
    <div className="relative">
      <button onClick={()=>setOpen(o=>!o)}
              className="text-[12px] font-black uppercase tracking-widest bg-shSurfaceRaised text-shTextMuted px-2 py-1.5 rounded">
        <i className="fas fa-flag mr-1"/>{entry.priority}
      </button>
      {open && (
        <div className="absolute right-0 mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded shadow-2xl z-10">
          {Object.entries(PRIORITY_META).map(([k, m]) => (
            <button key={k} onClick={()=>{ onPick(k); setOpen(false); }} data-testid={`set-priority-${entry.id}-${k}`}
                    className={`block w-full text-left text-[12px] font-black uppercase tracking-widest px-3 py-2 hover:bg-shSurfaceRaised ${entry.priority===k?"text-shPrimary":"text-shTextMuted"}`}>
              {m.label}
            </button>
          ))}
        </div>
        ...
        <div className="absolute right-0 mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded shadow-2xl z-10">
          {statuses.map(s => (
            <button key={s} onClick={()=>{ onPick(s); setOpen(false); }} data-testid={`set-status-${entry.id}-${s}`}
                    className={`block w-full text-left text-[12px] font-black uppercase tracking-widest px-3 py-2 hover:bg-shSurfaceRaised ${entry.status===s?"text-shPrimary":"text-shTextMuted"}`}>
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl w-full max-w-md p-6 shadow-2xl animate-slide-in" data-testid="waitlist-modal">
        <div className="flex items-center justify-between mb-4">
          <h4 className="text-lg font-black text-shText uppercase italic tracking-tight">Add to Waitlist</h4>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Service</label>
            <div className="mt-1 grid grid-cols-4 gap-2">
              {SERVICE_TYPES.map(s => (
                <button key={s} type="button" onClick={()=>setForm({ ...form, service_type: s })}
                        data-testid={`waitlist-service-${s}`}
                        className={`text-[11px] font-black uppercase tracking-widest px-2 py-2 rounded border ${form.service_type===s?"bg-shPrimary text-bgBase border-shPrimary":"bg-[var(--sh-card-base)] border-shBorder text-shTextMuted"}`}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Date</label>
          ...
          {availability?.has_limit && (
            <div className={`rounded p-3 text-[12px] font-black uppercase tracking-widest border ${
              availability.is_full ? "bg-red-500/10 text-red-300 border-red-500/40"
                                   : "bg-shPrimary/10 text-shPrimary border-shPrimary/40"
          ...
          <div>
            <label className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Priority</label>
            <div className="mt-1 grid grid-cols-3 gap-2">
              {Object.entries(PRIORITY_META).map(([k, m]) => (
                <button key={k} type="button" onClick={()=>setForm({ ...form, priority: k })}
                        data-testid={`waitlist-priority-${k}`}
                        className={`text-[11px] font-black uppercase tracking-widest px-2 py-2 rounded border ${form.priority===k?"bg-shPrimary text-bgBase border-shPrimary":"bg-[var(--sh-card-base)] border-shBorder text-shTextMuted"}`}>
                  {m.label}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Notes</label>
