...
export default function LegacyPricingModal({ client, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 grid place-items-center p-4"
      ...
      <div className="bg-bgCard border border-shBorder rounded-2xl shadow-2xl w-full max-w-2xl max-h-[calc(var(--app-height)_-_2rem)] overflow-y-auto">
        <div className="sticky top-0 bg-gradient-to-r from-amber-500/20 to-amber-500/5 border-b border-amber-500/30 p-5 z-10">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-[13px] text-shTextMuted mt-1 max-w-md leading-relaxed">
        ...
        <div className="p-5 space-y-4">
          ...
          {adding && (
            <div className="bg-[var(--sh-card-base)]/60 border border-shBorder rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[11px] text-shTextMuted mb-1 uppercase tracking-widest font-black">Item</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] text-shTextMuted mb-1 uppercase tracking-widest font-black">Locked price ($)</label>
                  <input type="number" min="0" step="0.01" value={draft.override_price}
                         ...
                         className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm" />
                  {savingsPreview > 0 && (
                    <p className="text-[12px] text-shPrimary mt-1">
                      <i className="fas fa-tag mr-1"/>Saves ${savingsPreview.toFixed(2)} per use
                    </p>
                  )}
                </div>
                <div>
                  <label className="block text-[11px] text-shTextMuted mb-1 uppercase tracking-widest font-black">Expires on</label>
                  <input type="date" value={draft.expires_on}
                         onChange={(e)=>setDraft(d=>({...d, expires_on: e.target.value}))}
                         data-testid="legacy-expiry-input"
                         className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm" />
                  <p className="text-[11px] text-shTextMuted mt-1 italic">Leave blank to lock the rate forever.</p>
                </div>
              </div>
              <div>
                <label className="block text-[11px] text-shTextMuted mb-1 uppercase tracking-widest font-black">Note (private — admin only)</label>
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={()=>{ setAdding(false); setErr(""); }}
                        className="text-shTextMuted font-black uppercase text-[13px] tracking-widest px-3 py-2 hover:text-shText">
                  Cancel
                </button>
            ...
            <ul className="space-y-2" data-testid="legacy-overrides-list">
              {overrides.map(row => (
                <li key={row.id}
                    ...
                    className={`bg-[var(--sh-card-base)]/40 border rounded-lg p-3 flex items-center gap-3 ${row.active ? "border-amber-500/30" : "border-shBorder opacity-50"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[12px] text-shTextMuted flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5">
                      ...
                      {row.savings > 0 && <span className="text-shPrimary">saves ${row.savings.toFixed(2)}</span>}
                      <span className={row.expires_on ? "" : "text-shPrimary"}>
                    ...
                    {row.note && <div className="text-[12px] text-shTextMuted italic mt-1">{row.note}</div>}
                  </div>
