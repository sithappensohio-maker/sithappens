...
const MOOD_DOT_COLOR = (avg) => {
  ...
  if (avg >= 4.0) return "#8cc63f";         // shPrimary
  if (avg >= 3.0) return "#00a9e0";         // shSecondary
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/85 backdrop-blur flex items-center justify-center p-3 sm:p-6"
      ...
      <div onClick={(e) => e.stopPropagation()}
           className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl w-full max-w-6xl max-h-[calc(var(--app-height)_-_1.5rem)] overflow-y-auto shadow-2xl animate-slide-in">
        <div className="sticky top-0 z-10 flex items-center justify-between gap-3 px-5 sm:px-6 py-4 bg-[var(--sh-card-base)]/95 backdrop-blur border-b border-shBorder">
          ...
          <button onClick={onClose} data-testid="homework-analytics-close"
                  className="text-shTextMuted hover:text-shText text-2xl p-1 leading-none">
            <i className="fas fa-times"/>
          </button>
        </div>

        <div className="p-5 sm:p-6 space-y-6">
          {loading && <p className="text-[14px] text-shTextMuted">Crunching numbers…</p>}
          ...
          {data && (
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3" data-testid="homework-analytics-tiles">
                <Tile label="Active plans"      value={data.global.active_plans}    color="text-shSecondary"   icon="fa-play" />
                <Tile label="Completed"         value={data.global.completed_plans} color="text-shPrimary"  icon="fa-trophy" />
                <Tile label="Completion rate"   value={`${data.global.completion_rate}%`} color="text-shAccent" icon="fa-percent" />
                <Tile label="Avg active streak" value={`${data.global.avg_streak}d`} color="text-purple-300" icon="fa-fire" />
              </div>

              {/* Templates list */}
              <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl overflow-hidden" data-testid="homework-analytics-templates">
                <div className="hidden md:grid grid-cols-12 gap-3 px-4 py-2.5 text-[11px] font-black uppercase tracking-widest text-shTextMuted border-b border-shBorder bg-[var(--sh-card-base)]/40">
                  ...
                  return (
                    <div key={t.template_id || "custom"} data-testid={`homework-analytics-row-${t.template_id || "custom"}`}>
                      <button
                        ...
                        className="w-full grid grid-cols-12 gap-3 px-4 py-3 text-left hover:bg-[var(--sh-card-base)]/50 border-b border-shBorder transition items-center">
                        <div className="col-span-12 md:col-span-4 flex items-center gap-2 min-w-0">
                          <i className={`fas ${isOpen ? "fa-chevron-down" : "fa-chevron-right"} text-shTextMuted text-xs shrink-0`}/>
                          <div className="min-w-0">
                            <p className="text-[14px] font-black text-shText truncate">{t.title}</p>
                            <p className="text-[11px] text-shTextMuted uppercase tracking-widest md:hidden">
                              {t.total_days}d · {t.assigned_count} assigned · {t.completion_rate}%
                            </p>
                          </div>
                        </div>
                        <div className="hidden md:block col-span-1 text-center text-[14px] text-shTextMuted">{t.total_days}</div>
                        <div className="hidden md:block col-span-1 text-center text-[14px] font-black text-shText">{t.assigned_count}</div>
                        <div className="hidden md:flex col-span-2 items-center justify-center gap-2">
                          <div className="h-1.5 w-16 bg-shSurfaceRaised rounded overflow-hidden">
                            <div className="h-full bg-shPrimary" style={{ width: `${t.completion_rate}%` }}/>
                          </div>
                          <span className="text-[13px] font-black text-shPrimary w-12 text-right">{t.completion_rate}%</span>
                        </div>
                        <div className="hidden md:block col-span-2 text-center text-[14px] text-shTextMuted">
                        ...
                        <div className="hidden md:flex col-span-2 justify-center gap-1.5 text-[11px] font-black uppercase tracking-widest">
                          {t.dropoff_day_stale != null && (
                            <span className="bg-shAccent/15 text-shAccent px-1.5 py-0.5 rounded border border-shAccent/30"
                      ...
                      {isOpen && (
                        <div className="bg-[var(--sh-card-base)]/40 px-5 py-4 border-b border-shBorder space-y-4"
                          ...
                          <div>
                            <p className="text-[11px] font-black uppercase tracking-widest text-shTextMuted mb-2">
                              <i className="fas fa-chart-column text-shPrimary mr-1"/>Per-day engagement &amp; mood
                            </p>
                              ...
                              <div className="flex gap-1.5 min-w-max" data-testid={`homework-analytics-perday-${t.template_id || "custom"}`}>
                                {t.per_day.map((d) => (
                                  <div key={d.day_number}
                                    ...
                                    <div className="h-20 w-full bg-[var(--sh-card-base)] border border-shBorder rounded flex flex-col justify-end overflow-hidden relative" title={`${d.logged_count}/${t.assigned_count} clients logged (${d.engagement_pct}%)`}>
                                      <div className="bg-shPrimary w-full transition-all"
                                           style={{ height: `${Math.max(d.engagement_pct, 2)}%` }}/>
                                      <span className="absolute inset-0 flex items-center justify-center text-[10px] font-black text-shText drop-shadow">
                                        {d.engagement_pct}%
                                      </span>
                                    </div>
                                    {/* Mood dot */}
                                    ...
                                    <p className="text-[10px] font-black text-shTextMuted uppercase tracking-widest">D{d.day_number}</p>
                                    {(d.needs_redo > 0 || d.questions > 0) && (
                                      <div className="flex gap-0.5 text-[9px] font-black">
                                        ...
                                        {d.questions > 0 && <span className="bg-shSecondary/20 text-shSecondary px-1 rounded" title={`${d.questions} questions`}>{d.questions}q</span>}
                                      </div>
                                    )}
                                  </div>
                                ))}
                            ...
                            <div className="flex items-center gap-3 mt-2 text-[11px] text-shTextMuted">
                              <span className="flex items-center gap-1"><span className="w-2 h-2 bg-shPrimary inline-block rounded"/>engagement</span>
                          ...
                          {t.recent_completions.length > 0 && (
                            <div>
                              <p className="text-[11px] font-black uppercase tracking-widest text-shTextMuted mb-2">
                                <i className="fas fa-graduation-cap text-shPrimary mr-1"/>Recent completions
                              </p>
                              <div className="flex flex-wrap gap-1.5">
                                {t.recent_completions.map((c, i) => (
                                  <span key={i}
                                        className="text-[12px] bg-shPrimary/10 border border-shPrimary/30 text-shPrimary px-2 py-1 rounded"
                                        data-testid={`homework-analytics-recent-${t.template_id || "custom"}-${i}`}>
                                    <i className="fas fa-paw mr-1"/>{c.dog_name}
                                    <span className="text-shTextMuted normal-case"> · {(c.completed_at || "").slice(0,10) || "—"}</span>
                                  </span>
                                ))}
                              </div>
                            </div>
                          ...
                          {(t.dropoff_day_stale != null || t.dropoff_day_engagement != null) && (
                            <div className="bg-shSecondary/10 border border-shSecondary/30 rounded p-3 text-[13px] text-shSecondary">
                              <p className="font-black uppercase tracking-widest text-[11px] mb-1">
                                <i className="fas fa-lightbulb mr-1"/>What to check
                              </p>
                              <p className="text-gray-200">
                                {t.dropoff_day_stale != null && (
                                  <>Plans most often go silent after <span className="font-black text-shText">Day {t.dropoff_day_stale}</span>. Consider an extra prompt, easier step, or short check-in message on that day. </>
                                )}
                                {t.dropoff_day_engagement != null && (
                                  <>Engagement drops sharpest at <span className="font-black text-shText">Day {t.dropoff_day_engagement}</span> — review the instructions or step difficulty there.</>
                                )}
                              </p>
                            </div>
                          )}
...
function Tile({ label, value, color, icon }) {
  return (
    <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl p-4" data-testid={`homework-analytics-tile-${label.replace(/\s+/g, "-").toLowerCase()}`}>
