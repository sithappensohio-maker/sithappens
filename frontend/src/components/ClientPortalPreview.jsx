  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-0 sm:p-4" onClick={onClose} data-testid="client-portal-preview">
      <div className="bg-bgBase border border-bgHover rounded-none sm:rounded-2xl w-full sm:max-w-3xl h-full sm:h-[90vh] flex flex-col shadow-2xl" onClick={e=>e.stopPropagation()}>
        {/* Banner */}
        <div className="shrink-0 bg-gradient-to-r from-red-500/80 to-orange-500/70 text-white px-4 py-2 flex items-center justify-between gap-3 text-[12px] font-black uppercase tracking-widest">
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-bgPanel border border-bgHover rounded-xl p-4 text-center">
                  ...
                  <p className="text-[10px] text-gray-500 font-black uppercase">Days left</p>
                </div>
                <div className="bg-bgPanel border border-bgHover rounded-xl p-4 text-center">
                ...
                <div className={`rounded-xl p-4 text-center border ${data.waiver.signed && !data.waiver.needs_resign ? "bg-shGreen/10 border-shGreen/40" : "bg-red-500/10 border-red-500/40"}`}>
                  ...
                  {data.waiver.signature?.signed_at && <p className="text-[10px] text-gray-500 mt-1">{data.waiver.signature.signed_at.slice(0,10)}</p>}
                </div>
              </div>
              ...
              <section>
                ...
                {data.dogs.length === 0 ? (
                  <p className="text-gray-500 text-[13px] italic">No dogs on file yet.</p>
                ) : (
                    ...
                    {data.dogs.map(d => (
                      <div key={d.id} className="bg-bgPanel border border-bgHover rounded-xl overflow-hidden" data-testid={`preview-dog-${d.id}`}>
                        ...
                        <div className="p-3">
                          ...
                          <p className="text-[12px] text-gray-400 mt-1">Rabies: <span className={d.vaccines?.rabies && d.vaccines.rabies >= today ? "text-shGreen" : "text-red-400"}>{d.vaccines?.rabies || "Missing"}</span></p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <section>
                ...
                {upcoming.length === 0 ? (
                  <p className="text-gray-500 text-[13px] italic">No upcoming bookings.</p>
                ) : (
                  <ul className="space-y-2" data-testid="preview-upcoming">
                    {upcoming.map(b => (
                      <li key={b.id} className="bg-bgPanel border border-bgHover rounded p-3 flex items-center justify-between text-[13px]">
                        <div className="min-w-0">
                          <p className="font-black uppercase text-white">{b.dog_name} · <span className="text-shBlue">{b.service_type}</span></p>
                          <p className="text-[12px] text-gray-400">{b.date}{b.end_date && b.end_date !== b.date ? ` → ${b.end_date}` : ""}</p>
                        </div>
                        <span className={`shrink-0 text-[11px] font-black uppercase px-2 py-1 rounded ${b.status==="approved"?"bg-shGreen/15 text-shGreen":b.status==="pending"?"bg-shOrange/15 text-shOrange":"bg-gray-500/15 text-gray-400"}`}>{b.status}</span>
                      </li>
                    ))}
                  </ul>
                )}
                    ...
                    {data.dogs.map(d => (data.enrollments_by_dog[d.id] || []).map(e => (
                      <div key={e.id} className="bg-bgPanel border border-bgHover rounded p-3 flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-[13px] font-black uppercase text-white truncate">{d.name} · {e.program_snapshot?.name || "Program"}</p>
                          <p className="text-[11px] text-gray-500 font-black uppercase tracking-widest">Started {e.started_at} · {e.status}</p>
                        </div>
                      </div>
                    )))}
                  </div>
              ...
              {data.homework.length > 0 && (
                <section>
                  ...
                  <ul className="space-y-2" data-testid="preview-homework">
                    {data.homework.map(h => (
                      <li key={h.id} className="bg-bgPanel border border-bgHover rounded p-3 text-[13px]">
                        <p className="font-black uppercase text-white">{h.title || h.template_name || "Homework"}</p>
                        <p className="text-[12px] text-gray-400 mt-1">Due {h.due_date || "—"} · Status: <span className="text-shBlue">{h.status}</span></p>
                      </li>
                    ))}
                  </ul>
                </section>
              ...
              {recent.length > 0 && (
                <section>
                  ...
                  <ul className="space-y-1.5" data-testid="preview-recent">
                    {recent.map(b => (
                      <li key={b.id} className="text-[12px] text-gray-400 flex items-center justify-between">
