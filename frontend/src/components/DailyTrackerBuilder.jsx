...
export default function DailyTrackerBuilder({ dogs, defaultDogId = "", onClose, onAssigned }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50" onClick={onClose} data-testid="daily-tracker-builder">
      <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-2xl w-full max-w-5xl max-h-[calc(var(--app-height)_-_1.5rem)] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 bg-[var(--sh-card-base)] border-b border-shBorder p-5 flex items-center justify-between z-10">
          <div>
            <h3 className="text-xl font-black text-shText uppercase italic tracking-tight">
              <i className="fas fa-calendar-check text-shPrimary mr-2" />Create Daily Tracker
            </h3>
          ...
          <button onClick={onClose} className="text-shTextMuted hover:text-shText text-xl" data-testid="dtb-close">
        ...
        {step === 1 ? (
          <div className="p-5 space-y-4 max-w-2xl">
            ...
            <div>
              <label className="text-[14px] font-black text-shTextMuted uppercase tracking-widest">Overview (optional)</label>
            ...
            <div className="bg-shSecondary/5 border border-shSecondary/30 rounded p-3">
              <p className="text-[13px] text-shSecondary font-black uppercase tracking-widest mb-1">
                <i className="fas fa-info-circle mr-1" />How the daily tracker works
              </p>
              <p className="text-[14px] text-shTextMuted leading-relaxed">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={() => setStep(2)} disabled={!canGoStep2} data-testid="dtb-next"
                      className="bg-shSecondary text-shText px-6 py-2 rounded font-black text-[14px] uppercase tracking-widest disabled:opacity-50">
                Next: Build days →
              </button>
            </div>
          </div>
          ...
          <div className="p-5 grid grid-cols-1 lg:grid-cols-4 gap-5">
            ...
            <div className="lg:col-span-1 space-y-2">
              <p className="text-[13px] font-black uppercase tracking-widest text-shTextMuted">Days · {days.length}</p>
                ...
                {days.map((d, idx) => (
                  <button key={idx} onClick={() => setActiveDayIdx(idx)} data-testid={`dtb-day-tab-${idx}`}
                          className={`w-full text-left rounded p-2.5 border transition flex items-start gap-2
                            ${activeDayIdx === idx ? "bg-shPrimary/15 border-shPrimary text-shText" : "bg-[var(--sh-card-base)] border-shBorder text-shTextMuted hover:border-shPrimary/40"}`}>
                    <span className="font-black text-[14px] uppercase tracking-widest min-w-[44px]">Day {d.day_number}</span>
                    <span className="text-[13px] truncate flex-1">{d.day_focus || <em className="text-shTextMuted">add focus…</em>}</span>
                  </button>
                ))}
              </div>
              <button onClick={addDay} data-testid="dtb-add-day"
                      className="w-full bg-shPrimary/10 text-shPrimary border border-shPrimary/30 rounded p-2 text-[14px] font-black uppercase tracking-widest hover:bg-shPrimary/20">
              ...
              <div className="border-t border-shBorder pt-3 mt-3 space-y-2">
                <label className="flex items-start gap-2 cursor-pointer">
                  <input type="checkbox" checked={saveAsTemplate} onChange={(e) => setSaveAsTemplate(e.target.checked)} data-testid="dtb-save-template"
                         className="accent-shPrimary w-4 h-4 mt-0.5" />
                  <span className="text-[13px] text-shTextMuted font-black uppercase tracking-widest">Also save as a reusable template</span>
                </label>
                {saveAsTemplate && (
              ...
              <div className="border-t border-shBorder pt-3 mt-3">
                ...
                <p className="text-[12px] text-shTextMuted mb-2">PDFs or links available on every day card.</p>
                  ...
                  {planResources.map((r) => (
                    <div key={r.id} className="space-y-1" data-testid={`dtb-plan-res-${r.id}`}>
                      <input value={r.name} onChange={(e) => updatePlanResource(r.id, { name: e.target.value })}
                             ...
                             className="w-full bg-[var(--sh-card-base)] border border-shBorder rounded p-1.5 text-shText text-[13px]" />
                      <div className="flex items-center gap-1">
                        {r.media_id ? (
                          ...
                          <input value={r.url || ""} onChange={(e) => updatePlanResource(r.id, { url: e.target.value })}
                                 placeholder="https://..."
                                 className="flex-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-1.5 text-shText text-[13px]" />
                        )}
                        <button onClick={() => removePlanResource(r.id)} className="text-shTextMuted hover:text-red-400 px-1.5" data-testid={`dtb-remove-plan-res-${r.id}`}>
                          <i className="fas fa-times"/>
                        </button>
                      </div>
                    </div>
            ...
            <div className="lg:col-span-3 space-y-4">
              <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[14px] font-black uppercase tracking-widest text-shPrimary">Day {activeDay.day_number}</p>
                  <div className="flex gap-1">
                    <button onClick={() => duplicateDay(activeDayIdx)} className="text-[12px] font-black uppercase tracking-widest text-shTextMuted hover:text-shSecondary px-2 py-1" data-testid="dtb-dup-day">
                      <i className="fas fa-clone mr-1" />Duplicate
                    </button>
                    {days.length > 1 && (
                      <button onClick={() => removeDay(activeDayIdx)} className="text-[12px] font-black uppercase tracking-widest text-shTextMuted hover:text-red-400 px-2 py-1" data-testid="dtb-remove-day">
                ...
                <label className="text-[13px] font-black text-shTextMuted uppercase tracking-widest">Today's focus</label>
                ...
                <label className="text-[13px] font-black text-shTextMuted uppercase tracking-widest mt-3 block">Detailed instructions (optional)</label>
                ...
                <label className="text-[13px] font-black text-shTextMuted uppercase tracking-widest mt-3 block">
                  <i className="fas fa-toolbox mr-1 text-shAccent"/>Equipment / treats needed (comma-separated)
                </label>
                <input value={(activeDay.equipment || []).join(", ")}
                       ...
                       className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm" />
                <p className="text-[12px] text-shTextMuted mt-1">Shown to the client at the top of the day card so they don't show up empty-handed.</p>
              </div>

              {/* Action steps (Sprint 103+105) — checkable list with per-step minutes */}
              <div className="bg-[var(--sh-card-base)] border border-shPrimary/30 rounded-xl p-4">
                <div className="flex items-center justify-between mb-1 gap-2 flex-wrap">
                  ...
                  {(() => {
                    ...
                    return total > 0 ? (
                      <span className="text-[12px] text-shPrimary font-black uppercase tracking-widest" data-testid="dtb-day-minutes-total">
                  ...
                  {(activeDay.steps || []).map((s, i) => (
                    <div key={s.id} className="space-y-1.5" data-testid={`dtb-step-${s.id}`}>
                      <div className="flex items-center gap-2">
                        <div className="flex flex-col">
                          <button onClick={() => moveStep(activeDayIdx, s.id, -1)} disabled={i === 0} data-testid={`dtb-step-up-${s.id}`}
                                  className="text-shTextMuted hover:text-shPrimary disabled:opacity-30 disabled:cursor-not-allowed text-xs leading-none h-3">
                            <i className="fas fa-chevron-up"/>
                          </button>
                          <button onClick={() => moveStep(activeDayIdx, s.id, 1)} disabled={i === (activeDay.steps || []).length - 1} data-testid={`dtb-step-down-${s.id}`}
                                  className="text-shTextMuted hover:text-shPrimary disabled:opacity-30 disabled:cursor-not-allowed text-xs leading-none h-3 mt-0.5">
                        ...
                        <input
                          ...
                          className="w-20 bg-[var(--sh-card-base)] border border-shBorder rounded p-2 text-shText text-sm text-center"
                        />
                        {(activeDay.steps || []).length > 1 && (
                          <button onClick={() => removeStep(activeDayIdx, s.id)} data-testid={`dtb-remove-step-${s.id}`}
                                  className="text-shTextMuted hover:text-red-400 px-2">
                            <i className="fas fa-times"/>
                          </button>
                        )}
                      </div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full ml-6 bg-[var(--sh-card-base)]/60 border border-shBorder rounded p-2 text-shTextMuted text-[13px] leading-snug"
                ...
                <button onClick={() => addStep(activeDayIdx)} data-testid="dtb-add-step"
                        className="mt-3 text-shPrimary hover:text-shPrimary/80 text-[13px] font-black uppercase tracking-widest">
                  <i className="fas fa-plus mr-1"/>Add a step
                </button>
              </div>

              {/* Per-day resources (Sprint 105) — printable handouts the client can take outside */}
              <div className="bg-[var(--sh-card-base)] border border-purple-400/30 rounded-xl p-4">
                  ...
                  {(activeDay.resources || []).map((r) => (
                    <div key={r.id} className="flex items-center gap-2" data-testid={`dtb-day-res-${r.id}`}>
                      ...
                      <button onClick={() => removeDayResource(activeDayIdx, r.id)} data-testid={`dtb-remove-day-res-${r.id}`}
                              className="text-shTextMuted hover:text-red-400 px-2"><i className="fas fa-times"/></button>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[14px] font-black uppercase tracking-widest text-shSecondary">
                    <i className="fas fa-list-check mr-1" />Steps to log ({(activeDay.fields || []).length})
                  </p>
                </div>
                <div className="space-y-2">
                  {(activeDay.fields || []).map((f, i) => (
                    <div key={f.id} className="bg-[var(--sh-card-base)] border border-shBorder rounded p-2.5 flex items-center gap-2" data-testid={`dtb-field-${f.id}`}>
                      <div className="flex flex-col">
                        <button onClick={() => moveField(activeDayIdx, f.id, -1)} disabled={i === 0} data-testid={`dtb-field-up-${f.id}`}
                                className="text-shTextMuted hover:text-shSecondary disabled:opacity-30 disabled:cursor-not-allowed text-xs leading-none h-3">
                          <i className="fas fa-chevron-up"/>
                        </button>
                        <button onClick={() => moveField(activeDayIdx, f.id, 1)} disabled={i === (activeDay.fields || []).length - 1} data-testid={`dtb-field-down-${f.id}`}
                                className="text-shTextMuted hover:text-shSecondary disabled:opacity-30 disabled:cursor-not-allowed text-xs leading-none h-3 mt-0.5">
                          <i className="fas fa-chevron-down"/>
                        </button>
                      </div>
                      <i className={`fas ${FIELD_KIND_OPTIONS.find(k=>k.value===f.kind)?.icon || "fa-circle"} text-shTextMuted w-5 text-center text-[14px]`} />
                      <input value={f.label} onChange={(e) => updateField(activeDayIdx, f.id, { label: e.target.value })}
                             placeholder="Custom label (e.g., How many recalls?)" className="flex-1 bg-[var(--sh-card-base)] border border-shBorder rounded p-1.5 text-shText text-sm" />
                      <select value={f.kind} onChange={(e) => updateField(activeDayIdx, f.id, { kind: e.target.value })}
                              className="bg-[var(--sh-card-base)] border border-shBorder rounded p-1.5 text-shText text-[13px]">
                        {FIELD_KIND_OPTIONS.map(k => <option key={k.value} value={k.value}>{k.label}</option>)}
                      </select>
                      <button onClick={() => removeField(activeDayIdx, f.id)} className="text-shTextMuted hover:text-red-400 px-2" data-testid={`dtb-remove-field-${f.id}`}>
                        <i className="fas fa-times" />
                      </button>
                    </div>
                  ))}
                ...
                <div className="mt-3 pt-3 border-t border-shBorder">
                  <p className="text-[13px] font-black text-shTextMuted uppercase tracking-widest mb-2">Add a step:</p>
                  <div className="flex flex-wrap gap-1.5">
                    {FIELD_KIND_OPTIONS.map(k => (
                      <button key={k.value} onClick={() => addField(activeDayIdx, k.value)} data-testid={`dtb-addfield-${k.value}`}
                              className="bg-[var(--sh-card-base)] border border-shBorder hover:border-shSecondary rounded px-2.5 py-1.5 text-[12px] text-shTextMuted font-black uppercase tracking-widest">
              ...
              <div className="flex justify-between gap-2 pt-2 border-t border-shBorder">
                <button onClick={() => setStep(1)} className="text-shTextMuted font-black uppercase text-[14px] tracking-widest">← Back</button>
                <button onClick={submit} disabled={busy || !canAssign} data-testid="dtb-assign"
                        className="bg-shPrimary text-black px-6 py-2.5 rounded font-black text-[14px] uppercase tracking-widest disabled:opacity-50 hover:bg-shPrimary/80">
