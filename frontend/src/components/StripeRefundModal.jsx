    ...
    <div className="fixed inset-0 z-[80] bg-black/75 backdrop-blur-sm overflow-y-auto p-4 grid place-items-start sm:place-items-center"
      ...
      <div className="w-full max-w-md my-8 bg-bgCard border border-shBorder rounded-2xl shadow-2xl"
        ...
        <div className="p-5 border-b border-shBorder flex items-start justify-between gap-4">
          <div>
            <p className="text-[11px] uppercase tracking-[0.3em] font-black text-shSecondary">Refund via Stripe</p>
            <h2 className="text-lg font-black text-shText mt-1">
        ...
        <div className="p-5 space-y-4">
          <div className="bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 text-sm text-shTextMuted space-y-1">
            <div className="flex justify-between"><span>Original payment</span><span className="text-shText font-bold">{money(payment.amount)}</span></div>
            <div className="flex justify-between"><span>Already refunded</span><span className="text-shText font-bold">{money(payment.refunded_amount)}</span></div>
            <div className="flex justify-between"><span>Remaining refundable</span><span className="text-shPrimary font-bold">{money(remaining)}</span></div>
          </div>
            ...
            <>
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setMode("full")}
                        className={`rounded-lg border p-3 text-left ${mode === "full" ? "border-shSecondary bg-shSecondary/10 text-shText" : "border-shBorder bg-[var(--sh-card-base)] text-shTextMuted hover:border-shSecondary/50"}`}>
                ...
                <button onClick={() => setMode("partial")}
                        className={`rounded-lg border p-3 text-left ${mode === "partial" ? "border-shSecondary bg-shSecondary/10 text-shText" : "border-shBorder bg-[var(--sh-card-base)] text-shTextMuted hover:border-shSecondary/50"}`}>
              ...
              {mode === "partial" && (
                <div>
                  <label className="text-[11px] uppercase tracking-widest text-shTextMuted font-black">Amount</label>
                  <input type="number" min="0.01" step="0.01" max={remaining} value={amount}
                         onChange={(e) => setAmount(e.target.value)} autoFocus
                         data-testid="stripe-refund-amount"
                         className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 text-shText text-xl font-black focus:border-shSecondary focus:outline-none" />
                </div>
              )}
              ...
              <div>
                ...
                <textarea rows={2} value={reason} onChange={(e) => setReason(e.target.value)}
                          ...
                          className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 text-shText resize-none focus:border-shSecondary focus:outline-none" />
              </div>
              ...
              <div className="flex justify-end gap-2">
                <button onClick={onClose} disabled={busy} className="px-4 py-2 rounded border border-shBorder text-shTextMuted font-black uppercase tracking-widest text-xs disabled:opacity-50">
                  Cancel
                </button>
                <button onClick={submit} disabled={!canSubmit} data-testid="stripe-refund-submit"
                        className="px-5 py-2 rounded bg-shSecondary text-shText font-black uppercase tracking-widest text-xs disabled:opacity-40">
          ...
          {succeeded && (
            <div className="space-y-4">
              <div className="bg-shPrimary/10 border border-shPrimary/40 text-shPrimary rounded-lg p-3 text-sm font-bold" data-testid="stripe-refund-success">
                <i className="fas fa-circle-check mr-2" />Refund successful — {money(refundAmount)} refunded via Stripe.
              </div>
              <div className="flex justify-end">
                <button onClick={finish} className="px-5 py-2 rounded bg-shPrimary text-bgHeader font-black uppercase tracking-widest text-xs">Done</button>
              </div>
            </div>
          )}
          ...
          {processing && (
            <div className="space-y-4">
              <div className="bg-shAccent/10 border border-shAccent/40 text-shAccent rounded-lg p-3 text-sm font-bold" data-testid="stripe-refund-processing">
                <i className="fas fa-circle-notch fa-spin mr-2" />Refund processing — Stripe hasn't confirmed it yet. It will finish shortly.
              </div>
              <div className="flex justify-end">
                <button onClick={finish} className="px-5 py-2 rounded bg-[var(--sh-card-base)] border border-shBorder text-gray-200 font-black uppercase tracking-widest text-xs">Close</button>
              </div>
            </div>
          )}
          ...
          {terminalFailure && (
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                <button onClick={onClose} className="px-4 py-2 rounded border border-shBorder text-shTextMuted font-black uppercase tracking-widest text-xs">Close</button>
                <button onClick={tryAgain} data-testid="stripe-refund-retry"
                        className="px-5 py-2 rounded bg-shSecondary text-shText font-black uppercase tracking-widest text-xs">Try Again</button>
              </div>
            </div>
          )}
        </div>
