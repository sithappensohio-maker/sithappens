    ...
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-bgBase p-4" data-testid="error-boundary">
        <div className="bg-bgPanel border border-bgHover rounded-2xl p-8 max-w-md text-center shadow-2xl">
          ...
          <p className="text-[11px] text-gray-400 font-black uppercase tracking-widest mt-2">A stale session or bad state crashed the UI. Reset to recover.</p>
          {this.state.msg && <pre className="text-[10px] text-red-400 bg-bgBase rounded p-3 mt-4 text-left whitespace-pre-wrap break-all">{this.state.msg}</pre>}
          <button onClick={this.reset} data-testid="error-reset"
                  className="mt-6 bg-shGreen text-bgHeader px-8 py-3 rounded font-black text-[10px] uppercase tracking-widest shadow-xl">
