...
const supportRemaining = (home, workspace, kind) => {
  ...
  const live = home?.support?.[`${key}_remaining`];
  if (live !== undefined) return live;
  const wsRemaining = workspace?.support?.[`${key}_remaining`];
  if (wsRemaining !== undefined) return wsRemaining;
  return workspace?.support?.[`${key}_allowance`] ?? null;
};
