        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {onAbout && (
            <button type="button" onClick={onAbout} data-testid="course-about-program"
                    className="min-h-[40px] px-2 text-[13px] font-black uppercase tracking-widest text-shSecondary hover:text-shText">
              About this program <i className="fas fa-circle-info ml-1 text-[11px]" />
            </button>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 mt-1">
          <div className="min-w-0 flex-1">
            ...
            {detail?.program_focus && (
              <p className="text-[16px] text-shTextMuted mt-1.5 leading-relaxed">{detail.program_focus}</p>
            )}
          </div>
          <div className="relative w-[92px] h-[92px] shrink-0" data-testid="course-dog-ring">
            ...
            <span className="absolute -right-1.5 bottom-0 px-2 py-0.5 rounded-full text-[13px] font-black text-[#071018] shadow-lg"
...
function LessonBadge({ lesson, currentLessonId, index }) {
  if (lesson.id === currentLessonId) {
    return (
      <span className="relative w-[34px] h-[34px] rounded-xl grid place-items-center shrink-0 text-[#04101a] shadow-[0_6px_18px_-6px_rgba(0,169,224,.85)]"
        ...
        <i className="fas fa-play text-[14px]" />
      </span>
    );
  }
  ...
  return (
    <span className="sh-display w-[34px] h-[34px] rounded-xl grid place-items-center shrink-0 border border-shBorder bg-white/[0.04] text-shTextMuted text-[16px]">
      {index + 1}
    </span>
  );
}
  ...
  return (
    <section data-testid={`course-module-${m.id}`} data-state={m.status}
      ...
      <button type="button" onClick={() => setOpen(v => !v)} aria-expanded={open}
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[17px] font-black text-shText leading-snug line-clamp-2">{m.name}</span>
          {m.description && <span className="block text-[14px] text-shTextMuted mt-0.5 line-clamp-2">{m.description}</span>}
          <span className="block text-[13px] font-black uppercase tracking-widest mt-1.5">
            <span className={st.cls.split(" ")[0]}><i className={`fas ${st.icon} mr-1 text-[11px]`} />{st.label}</span>
            {!locked && meta && <span className="text-shTextMuted"> · {meta}</span>}
          </span>
        </span>
        {!locked && <i className={`fas fa-chevron-${open ? "up" : "down"} text-[14px] text-shTextMuted shrink-0`} />}
      </button>

      {locked && m.lockedReason && (
        <p className="px-4 pb-3.5 -mt-1 text-[14px] text-shTextMuted" data-testid={`course-module-locked-${m.id}`}>
      ...
      {open && !locked && (
        <div className="px-1.5 pb-2" data-testid={`course-module-lessons-${m.id}`}>
          {lessons.length === 0
            ? <p className="text-[15px] text-shTextMuted italic px-3 py-3">Lessons for this module are being prepared.</p>
            : lessons.map((l, i) => (
    ...
    <section data-testid={testid} data-count={modules.length} className="space-y-3">
      <button type="button" onClick={() => setOpen(v => !v)} aria-expanded={open}
        ...
        <span className="w-11 h-11 rounded-xl grid place-items-center shrink-0 border border-shBorder bg-black/20 text-shTextMuted">
          <i className="fas fa-road text-[16px]" />
        </span>
        <span className="min-w-0 flex-1">
          <span className="block text-[16px] font-black text-shText leading-snug">
        ...
        <i className={`fas fa-chevron-${open ? "up" : "down"} text-[14px] text-shTextMuted shrink-0`} />
      </button>
