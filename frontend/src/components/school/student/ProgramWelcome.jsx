    ...
    <div className="border-t border-shBorder/50 first:border-t-0" data-testid={`welcome-index-module-${position}`}>
      <button type="button" onClick={() => setOpen(v => !v)} aria-expanded={open}
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[16px] font-black text-shText leading-snug">{m.name}</span>
          {m.description && <span className="block text-[14px] text-shTextMuted mt-0.5 line-clamp-2">{m.description}</span>}
        </span>
        <span className="text-[13px] font-black uppercase tracking-widest text-shTextMuted whitespace-nowrap shrink-0">{meta}</span>
        <i className={`fas fa-chevron-${open ? "up" : "down"} text-[13px] text-shTextMuted shrink-0`} />
      </button>
      {open && (
        <div className="px-4 pb-3" data-testid={`welcome-index-lessons-${position}`}>
          <ol className="space-y-1.5">
            {(m.lessons || []).map((l, i) => (
              <li key={i} className="flex items-baseline gap-2.5 text-[15px] text-shText">
                <span className="text-[13px] font-black text-shTextMuted min-w-[16px] tabular-nums">{i + 1}</span>
                <span className="min-w-0 flex-1 leading-snug">{l.name}</span>
                {l.estimated_minutes ? <span className="text-[13px] text-shTextMuted whitespace-nowrap tabular-nums">{l.estimated_minutes} min</span> : null}
              </li>
            ))}
          </ol>
          {m.quiz_question_count > 0 && (
            <p className="text-[13px] font-black uppercase tracking-widest text-shSecondary mt-2.5">
    ...
    <div className="space-y-4" data-testid="program-welcome">
      ...
      {(w.description || outcomes.length > 0) && (
        <section className="rounded-2xl border border-shBorder/60 bg-[var(--sh-card-base)] p-4 sm:p-5" data-testid="welcome-covered">
          <p className="text-[13px] font-black uppercase tracking-[0.18em] text-shSecondary">What this program covers</p>
          {w.description && <p className="text-[16px] text-shTextMuted mt-2 leading-relaxed">{w.description}</p>}
          {outcomes.length > 0 && (
            <ul className="mt-3 space-y-2" data-testid="welcome-outcomes">
              {outcomes.map((o) => (
                <li key={o} className="flex items-start gap-2.5 text-[16px] text-shText leading-relaxed">
                  <span className="w-[18px] h-[18px] rounded-md grid place-items-center shrink-0 mt-0.5 border border-shPrimary/40 bg-shPrimary/10 text-shPrimary text-[11px]">
      ...
      <section className="rounded-2xl border border-shBorder/60 bg-[var(--sh-card-base)] overflow-hidden" data-testid="welcome-index">
        ...
        {syllabus.length === 0 ? (
          ...
          <>
            ...
            <p className="px-4 py-3 border-t border-shBorder/50 bg-black/20 text-[14px] text-shTextMuted leading-relaxed">
      ...
      <section className="rounded-2xl border border-shBorder/60 bg-[var(--sh-card-base)] p-4 sm:p-5 flex items-center gap-3.5" data-testid="welcome-trainer-promise">
        <span className="w-11 h-11 rounded-full grid place-items-center shrink-0 bg-gradient-to-br from-shSecondary to-shPrimary text-[#071018]">
          <i className="fas fa-hand-holding-heart text-[18px]" />
        </span>
        <div className="min-w-0">
          <p className="text-[16px] font-black text-shText leading-snug">Your trainer is with you the whole way</p>
          <p className="text-[14px] text-shTextMuted leading-relaxed mt-0.5">Questions, video feedback, and graduation — a real person reviews your work, not an algorithm.</p>
        </div>
      </section>
    </div>
  );
