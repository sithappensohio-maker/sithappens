    ...
    <div className="max-w-3xl mx-auto space-y-4" data-testid="today-screen">
      {practiceJustCompleted && (
        <div className="rounded-2xl border border-shPrimary/40 bg-shPrimary/10 p-4" data-testid="today-practice-complete">
          <p className="text-shPrimary font-black text-[18px]"><i className="fas fa-circle-check mr-2" />Practice complete — nice work.</p>
          <p className="text-[16px] text-shTextMuted mt-1">Here's what's next for {dog}.</p>
        </div>
      )}
      ...
      <section className="rounded-2xl border p-5 sm:p-6"
        ...
        <p className="text-[15px] text-shTextMuted mt-2 uppercase tracking-widest font-bold truncate">
        ...
        {action.sublabel && (
          <p className="text-[17px] text-shTextMuted mt-2.5 leading-relaxed">{action.sublabel}</p>
        )}
        ...
        {copy.cta ? (
          ...
          <p className="mt-5 text-[16px] font-bold text-shTextMuted" data-testid="today-no-cta">
      ...
      {!["access_expired", "setup_required", "course_complete"].includes(action.type) && (
        <button type="button" onClick={onAskTrainer} className="w-full sm:w-auto min-h-[44px] px-4 rounded-xl border border-shSecondary/30 text-shSecondary text-[14px] font-black uppercase tracking-widest hover:text-shText" data-testid="today-ask-trainer">
      ...
      {showLessonContext && (lesson.client_overview || lesson.success_criteria) && (
        <section className="rounded-2xl border border-shBorder bg-[var(--sh-card-base)] p-4 sm:p-5 space-y-3" data-testid="today-context">
          {lesson.client_overview && (
            <div>
              <p className="text-[13px] font-black uppercase tracking-[0.2em] text-shSecondary mb-1">Today's goal</p>
              <p className="text-[17px] text-shText leading-relaxed">{lesson.client_overview}</p>
            </div>
          )}
          {lesson.success_criteria && (
            <div>
              <p className="text-[13px] font-black uppercase tracking-[0.2em] text-shPrimary mb-1"><i className="fas fa-flag-checkered mr-1" />Success looks like</p>
              <p className="text-[16px] text-shTextMuted leading-relaxed">{lesson.success_criteria}</p>
            </div>
          )}
        </section>
      )}
