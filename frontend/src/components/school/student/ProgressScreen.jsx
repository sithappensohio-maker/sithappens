  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5" data-testid="native-progress-screen">
      ...
      <section className="space-y-3" data-testid="dog-training-record">
        <div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-shSecondary">Permanent training record</p><p className="text-[12px] text-shTextMuted mt-1">Every Online School program and trainer-scored checkpoint stays attached to {home?.dog?.name || "your dog"}.</p></div>
        <div className="space-y-2">{(record.programs || []).map((program) => <div key={program.school_enrollment_id} className="rounded-xl border border-shBorder bg-[var(--sh-card-base)] p-3 flex items-center justify-between gap-3"><div><p className="text-[13px] font-black text-shText">{program.name || "Online School program"}</p><p className="text-[10px] text-shTextMuted mt-1">{program.started_at ? new Date(program.started_at).toLocaleDateString() : "Start date not recorded"}{program.completed_at ? ` → ${new Date(program.completed_at).toLocaleDateString()}` : ""}</p></div><span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-full ${program.status === "completed" ? "bg-shPrimary/15 text-shPrimary" : "bg-shBorder/25 text-shTextMuted"}`}>{program.status || "active"}</span></div>)}</div>
      </section>
