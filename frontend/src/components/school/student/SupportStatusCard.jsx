...
export default function SupportStatusCard({ home, onAskTrainer, onStartPractice }) {
  ...
  if (action.type === 'remediation') {
    ...
    return (
      <section className="rounded-2xl border border-shAccent/30 bg-shAccent/[0.055] p-4 sm:p-5" data-testid="school-remediation-card">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-shAccent"><i className="fas fa-rotate-left mr-1.5" />Your trainer's plan</p>
        <h3 className="text-[17px] font-black text-shText mt-1">More practice before you move on</h3>
        {cp.trainer_feedback && <p className="text-[13px] text-gray-200 mt-2 leading-relaxed border-l-2 border-shAccent/35 pl-3">“{cp.trainer_feedback}”</p>}
        <div className="mt-3 space-y-1.5 text-[13px] text-shTextMuted">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          {onStartPractice && <button onClick={onStartPractice} className="min-h-[44px] px-4 rounded-xl bg-shAccent text-bgHeader text-[11px] font-black uppercase tracking-widest" data-testid="school-remediation-start"><i className="fas fa-paw mr-1.5" />Start prescribed practice</button>}
          <button onClick={onAskTrainer} className="min-h-[44px] px-1 text-[12px] font-black uppercase tracking-widest text-shSecondary hover:text-shText"><i className="fas fa-comment-dots mr-1.5" />Ask about this plan</button>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="rounded-2xl border border-purple-400/30 bg-purple-500/[0.055] p-4 sm:p-5" data-testid="school-trainer-assist-card">
      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-purple-300"><i className="fas fa-hand-holding-heart mr-1.5" />Trainer Assist</p>
      <h3 className="text-[17px] font-black text-shText mt-1">{content[0]}</h3>
      <p className="text-[13px] text-shTextMuted mt-2 leading-relaxed">{content[1]}</p>
      {cp.trainer_feedback && <p className="text-[13px] text-gray-200 mt-3 leading-relaxed border-l-2 border-purple-400/35 pl-3">“{cp.trainer_feedback}”</p>}
      <button onClick={onAskTrainer} className="mt-4 text-[12px] font-black uppercase tracking-widest text-shSecondary hover:text-shText"><i className="fas fa-comment-dots mr-1.5" />Ask your trainer</button>
    </section>
  );
}
