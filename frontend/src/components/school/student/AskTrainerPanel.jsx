      ...
      <div className="w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl border border-shBorder bg-[var(--sh-card-base)] p-5 sm:p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        {sent ? (
          <div className="mt-5 rounded-2xl border border-shPrimary/35 bg-shPrimary/10 p-5 text-center" data-testid="school-ask-sent">
            ...
            <p className="text-[13px] text-shTextMuted mt-1">Your trainer has the dog, course, and lesson context attached.</p>
            <button onClick={onClose} className="mt-4 px-5 py-2.5 rounded-xl bg-shPrimary text-bgHeader text-[12px] font-black uppercase tracking-widest">Done</button>
          </div>
        ) : (
          <>
            <textarea value={text} onChange={(e) => setText(e.target.value)} rows={5} autoFocus placeholder="What are you stuck on?"
                      className="mt-4 w-full rounded-xl border border-shBorder bg-black/20 p-3 text-[14px] text-shText resize-y min-h-[130px]"
                      data-testid="school-ask-body" />
            {error && <p className="text-[12px] text-shDanger mt-2" data-testid="school-ask-error">{error}</p>}
            <button onClick={send} disabled={busy || !text.trim()} className="mt-3 w-full min-h-[50px] rounded-xl bg-shPrimary text-bgHeader text-[13px] font-black uppercase tracking-widest disabled:opacity-40" data-testid="school-ask-send">
