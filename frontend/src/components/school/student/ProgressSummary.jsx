...
export default function ProgressSummary({ progress, onView }) {
  ...
  const stat = (value, label) => (
    <div className="min-w-0">
      ...
      <p className="text-[10.5px] text-shTextMuted uppercase tracking-wide mt-1 leading-tight">{label}</p>
    </div>
  );
  return (
    <section className="rounded-2xl border border-shBorder bg-[var(--sh-card-base)] p-4 sm:p-5" data-testid="progress-summary">
      <div className="flex items-center justify-between gap-2 mb-3">
        <p className="text-[11px] font-black uppercase tracking-[0.28em] text-shTextMuted">
          <i className="fas fa-chart-line mr-1.5 text-shPrimary" />Progress
        </p>
        <span className="text-[15px] font-black text-shPrimary tabular-nums">{pct}%</span>
      </div>
      ...
      {progress.current_module_name && (
        <p className="text-[12px] text-shTextMuted mt-2 truncate">Current module: <span className="text-shText font-bold">{progress.current_module_name}</span></p>
      )}
      ...
      <button type="button" onClick={onView}
              className="mt-4 text-[12px] font-black uppercase tracking-widest text-shTextMuted hover:text-shText transition"
