...
function TrophyDetailModal({ award, onClose }) {
  ...
  return createPortal((
    <div className="fixed inset-0 z-[100] bg-black/85 backdrop-blur grid place-items-center p-4" onClick={onClose} data-testid="trophy-detail-modal">
      <div onClick={(e)=>e.stopPropagation()} className="border border-shBorder rounded-2xl w-full max-w-lg p-6 shadow-sh animate-slide-in max-h-[calc(var(--app-height)_-_2rem)] overflow-y-auto" style={{ background: "var(--sh-card-base)" }}>
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-4">
            <TrophyBadge trophy={award} size="lg"/>
            <div>
              <div className="text-[13px] font-bold uppercase tracking-widest text-shTextMuted">{award.trophy_tier} Trophy</div>
              <h3 className="text-2xl font-bold text-shText">{award.trophy_name}</h3>
              <div className="text-[14px] text-shTextMuted mt-1">Awarded to <span className="text-shText font-bold">{award.recipient_name}</span> · {fmtDate(award.awarded_at)}</div>
            </div>
          </div>
          <button onClick={onClose} className="text-shTextMuted hover:text-shText p-1" data-testid="trophy-detail-close"><i className="fas fa-times text-lg"/></button>
        </div>
        {award.trophy_description && <p className="text-[14px] text-shTextMuted mb-4 leading-relaxed">{award.trophy_description}</p>}
        {award.note && <p className="text-[15px] rounded p-3 mb-4 italic text-shTextMuted border border-shBorder" style={{ background: "var(--sh-card-base)" }}><i className="fas fa-comment-dots mr-2 text-shSecondary"/>{award.note}</p>}
        <div className="rounded-lg overflow-hidden border border-shBorder mb-4" style={{ background: "var(--sh-card-base)" }}>
        ...
        <div className="flex gap-2">
          <a href={cardUrl} download={`${award.trophy_code}-${award.recipient_name}.png`} data-testid="trophy-share-download"
             className="flex-1 bg-shSecondary/10 hover:bg-shSecondary/20 text-shSecondary text-center py-3 rounded-md font-bold text-[14px] uppercase tracking-widest transition">
            <i className="fas fa-download mr-1"/> Download
          </a>
          <button onClick={copy} data-testid="trophy-share-copy"
                  className="flex-1 bg-shAccent/10 hover:bg-shAccent/20 text-shAccent text-center py-3 rounded-md font-bold text-[14px] uppercase tracking-widest transition">
...
export default function TrophyWall({ awards = [], locked = [], emptyMsg = "No trophies yet — earn one by crushing your goals!", testIdPrefix = "trophywall" }) {
  ...
  if (!hasAny) {
    return (
      <div className="border border-dashed border-shBorder rounded-lg p-6 text-center" style={{ background: "var(--sh-card-base)" }} data-testid={`${testIdPrefix}-empty`}>
        <i className="fas fa-trophy text-3xl text-shTextMuted mb-2"/>
        <p className="text-sm text-shTextMuted">{emptyMsg}</p>
      </div>
    );
  }
  return (
