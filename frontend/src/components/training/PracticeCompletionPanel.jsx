  ...
  return (
    <div className="space-y-4 sm:space-y-5" data-testid={testid}>
      <section className={`rounded-3xl border p-5 sm:p-6 ${stoppedEarly ? "border-shAccent/35 bg-shAccent/[0.055]" : "border-shPrimary/35 bg-gradient-to-br from-shPrimary/[0.08] via-black/15 to-shSecondary/[0.035]"}`} data-testid={testid ? `${testid}-save-guide` : undefined}>
        <p className={`text-[15px] sm:text-[16px] font-black uppercase tracking-[0.12em] ${stoppedEarly ? "text-shAccent" : "text-shPrimary"}`}>Last Step · Save Today&apos;s Session</p>
        <h3 className="text-[22px] sm:text-[23px] font-black text-shText mt-1.5 leading-tight">
        ...
        <p className="text-[17px] sm:text-[18px] text-shTextMuted mt-2 leading-relaxed max-w-2xl">
          {stoppedEarly
        ...
        <div className="grid gap-2 sm:grid-cols-3 mt-4">
          <div className="rounded-xl border border-shBorder/55 bg-black/15 p-3">
            <p className="text-[15px] sm:text-[16px] font-black uppercase tracking-[0.11em] text-shSecondary">1 · Check the results</p>
            <p className="text-[16px] text-shTextMuted mt-1 leading-relaxed">Confirm the rounds, repetitions, time, or other details from what actually happened.</p>
          </div>
          <div className="rounded-xl border border-shBorder/55 bg-black/15 p-3">
            <p className="text-[15px] sm:text-[16px] font-black uppercase tracking-[0.11em] text-shSecondary">2 · Answer the wrap-up</p>
            <p className="text-[16px] text-shTextMuted mt-1 leading-relaxed">Choose the answers that come closest to how today really felt.</p>
          </div>
          <div className="rounded-xl border border-shBorder/55 bg-black/15 p-3">
            <p className="text-[15px] sm:text-[16px] font-black uppercase tracking-[0.11em] text-shSecondary">3 · Save & continue</p>
            <p className="text-[16px] text-shTextMuted mt-1 leading-relaxed">Tap the green button once. School saves the session and then shows the next training step.</p>
          </div>
        </div>
      </section>
      ...
      {fieldsSlot && (
        <SectionCard accent="cyan" intensity="subtle">
          <p className="text-[15px] font-black uppercase tracking-[0.11em] text-shSecondary">Today&apos;s Results</p>
          <p className="text-[17px] text-shTextMuted mt-1 mb-3 leading-relaxed">
      ...
      {allowDifficulty && (
        <SectionCard accent="lime" intensity="subtle">
          <p className="text-[15px] font-black uppercase tracking-[0.11em] text-shPrimary mb-1">How did today&apos;s practice feel?</p>
          <p className="text-[17px] text-shTextMuted mb-3 leading-relaxed">
      ...
      {onAskSubmit && (
        <SectionCard accent="purple" intensity="subtle">
          <p className="text-[15px] font-black uppercase tracking-[0.11em] text-purple-300 mb-1">Need help from your trainer?</p>
          <p className="text-[16px] text-shTextMuted mb-2">Ask a question about what happened during this practice.</p>
          <div className="flex flex-col sm:flex-row gap-2">
            <input value={askText || ""} onChange={(e) => onAskTextChange(e.target.value)}
                   ...
                   className="flex-1 min-w-0 min-h-[46px] bg-black/20 border border-shBorder/55 rounded-xl px-3 text-shText text-[18px] focus:outline-none focus:border-purple-400/40"/>
      ...
      {stoppedEarly ? (
        <SectionCard accent="orange" intensity="subtle">
          <label className="text-[15px] font-black uppercase tracking-[0.11em] text-shAccent">What made you stop?</label>
          <p className="text-[16px] text-shTextMuted mt-1">A short answer helps your trainer decide whether the next practice should be easier, shorter, or set up differently.</p>
          <textarea value={couldNotCompleteReason || ""} onChange={(e) => onCouldNotCompleteReasonChange(e.target.value)}
                    ...
                    className="w-full mt-3 bg-black/20 border border-shAccent/25 rounded-xl p-3 text-shText text-[18px] focus:outline-none focus:border-shAccent/45"/>
        </SectionCard>
        ...
        <div className={`rounded-2xl border p-4 transition ${couldNotComplete ? "border-shAccent/35 bg-shAccent/[0.055]" : "border-shBorder/50 bg-black/12"}`}>
          <label className="flex items-center gap-3 text-[18px] text-shText cursor-pointer min-h-[36px]" data-testid={testid ? `${testid}-cnc-toggle` : undefined}>
            ...
            <span><span className="font-black">I couldn&apos;t finish the planned practice today</span><span className="block text-[16px] text-shTextMuted mt-0.5">Use this only when you stopped before finishing the plan. That is useful training information, not a failure.</span></span>
          </label>
          {couldNotComplete && (
            <textarea value={couldNotCompleteReason || ""} onChange={(e) => onCouldNotCompleteReasonChange(e.target.value)}
      ...
      {saveState === "error" && (
        <p className="text-shDanger text-[16px] font-bold rounded-xl border border-shDanger/25 bg-shDanger/[0.05] p-3" data-testid={testid ? `${testid}-error` : undefined}>
