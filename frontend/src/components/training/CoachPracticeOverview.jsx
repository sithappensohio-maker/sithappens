    ...
    <div className="rounded-2xl border border-shBorder/50 bg-black/12 overflow-hidden" data-testid={testid}>
      <button type="button" onClick={() => hasDetail && setOpen(o => !o)}
        ...
        <span className="min-w-0 flex-1">
          <p className="text-[15px] sm:text-[16px] font-black text-shText leading-tight">{renderPracticeCoachText(step.title, tokens)}</p>
          <p className="text-[14px] text-shTextMuted leading-relaxed mt-1">{renderPracticeCoachText(step.instruction, tokens)}</p>
        </span>
    ...
    <div className="space-y-4 sm:space-y-5" data-testid={testid}>
      <section className="relative overflow-hidden rounded-3xl border border-shSecondary/30 bg-gradient-to-br from-shSecondary/[0.085] via-black/20 to-black/35 min-h-[250px] sm:min-h-[280px]">
        ...
        <div className="grid sm:grid-cols-[1.25fr_0.75fr] min-h-[250px] sm:min-h-[280px]">
          <div className="relative z-10 p-5 sm:p-6 flex flex-col justify-center order-2 sm:order-1">
            <div className="flex items-center gap-2 text-shSecondary text-[12px] font-black uppercase tracking-[0.11em] mb-3" data-testid={testid ? `${testid}-time` : undefined}>
              <i className="fas fa-clock"/>{practiceTimeLabel(pc.schedule) || "Today's practice"}
            </div>
            <p className="text-[12px] sm:text-[13px] font-black uppercase tracking-[0.1em] text-shPrimary mb-1.5"><i className="fas fa-bullseye mr-1.5"/>Today&apos;s Goal</p>
            ...
            {pc.success_today && (
              <p className="text-[14px] sm:text-[15px] text-shTextMuted mt-3 leading-relaxed"><span className="font-black text-shText">Success today: </span>{renderPracticeCoachText(pc.success_today, tokens)}</p>
            )}
            {pc.encouragement && <p className="text-[13px] sm:text-[14px] text-shTextMuted mt-2 italic">{renderPracticeCoachText(pc.encouragement, tokens)}</p>}
          ...
          <div className="relative h-[170px] sm:h-auto order-1 sm:order-2 overflow-hidden border-b sm:border-b-0 sm:border-l border-shSecondary/20">
            ...
            <div className="absolute left-4 bottom-4 z-20 px-3 py-1.5 rounded-lg bg-black/60 backdrop-blur border border-white/10 text-white text-[13px] font-black">{dogName}</div>
          </div>
        </div>
      </section>
      ...
      {(pc.setup_items || []).length > 0 && (
        <SectionCard accent="cyan" intensity="subtle">
          <p className="text-[12px] sm:text-[13px] font-black uppercase tracking-[0.1em] text-shSecondary mb-3">Before You Start</p>
      ...
      {pc.pro_tip && (
        <SectionCard accent="lime" intensity="subtle">
          <div className="flex items-start gap-3">
            ...
            <p className="text-[14px] sm:text-[15px] text-shText leading-relaxed"><span className="font-black text-shPrimary">Pro tip: </span>{renderPracticeCoachText(pc.pro_tip, tokens)}</p>
          </div>
        </SectionCard>
      )}
      ...
      {(pc.steps || []).length > 0 && (
        <div>
          <div className="flex items-end justify-between gap-3 mb-2.5">
            <div><p className="text-[12px] sm:text-[13px] font-black uppercase tracking-[0.1em] text-shTextMuted">How It Works</p><p className="text-[15px] text-shTextMuted mt-0.5">Keep it simple and move one rep at a time.</p></div>
            <span className="text-[12px] font-black text-shSecondary shrink-0">{pc.steps.length} step{pc.steps.length === 1 ? "" : "s"}</span>
          </div>
      ...
      {onOpenTroubleshooting && (pc.troubleshooting || []).length > 0 && (
        <button type="button" onClick={onOpenTroubleshooting} data-testid={testid ? `${testid}-troubleshooting-link` : undefined}
                className="w-full text-left rounded-2xl border border-shBorder/55 bg-black/15 px-4 py-3.5 text-[14px] font-bold text-shSecondary flex items-center justify-between gap-3 hover:border-shSecondary/35 hover:bg-shSecondary/[0.035] transition min-h-[48px]">
