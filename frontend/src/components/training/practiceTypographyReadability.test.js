...
test("Practice Coach section labels use the readable hierarchy", () => {
  expect(overview).toMatch(/text-\[15px\] sm:text-\[16px\] font-black uppercase/);
  expect(completion).toMatch(/text-\[15px\] sm:text-\[16px\] font-black uppercase/);
  expect(measurements).toMatch(/text-\[15px\] sm:text-\[16px\] font-black uppercase/);
});
...
test("Practice Coach has a scoped floor for legacy tiny helper text", () => {
  ...
  expect(css).toMatch(/font-size: 13px !important/);
  expect(css).toMatch(/font-size: 15px !important/);
