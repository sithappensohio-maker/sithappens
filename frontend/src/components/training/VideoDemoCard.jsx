  ...
  return (
    <a href={videoUrl} target="_blank" rel="noopener noreferrer" data-testid={testid}
      ...
      <div className="relative shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-shSecondary/12 border border-shSecondary/35 grid place-items-center group-hover:bg-shSecondary/20 transition">
        <i className="fas fa-play text-shSecondary text-[20px] sm:text-[22px]"/>
      </div>
      <div className="relative min-w-0 flex-1">
        <p className="text-[11px] sm:text-[13px] font-black uppercase tracking-[0.16em] text-shSecondary/80 mb-1">Demo</p>
        <p className="text-[19px] sm:text-[21px] font-black text-shText leading-tight">{label}</p>
        <p className="text-[14px] sm:text-[15px] text-shTextMuted mt-1">See what a clean rep should look like before you start.</p>
      </div>
      <i className="fas fa-arrow-up-right-from-square relative text-shSecondary/65 text-[14px] shrink-0"/>
    </a>
  );
}
