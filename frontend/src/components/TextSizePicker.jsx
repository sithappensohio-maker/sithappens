...
export function TextSizePicker({ testid = "text-size", compact = false }) {
        ...
        return (
          <button
            ...
            className={`flex-1 py-2 rounded-md text-[14px] font-black uppercase tracking-wider transition border ${
              active
                ? "bg-shPrimary text-bgBase border-shPrimary"
                : "bg-[var(--sh-card-base)] text-shTextMuted border-shBorder hover:border-shPrimary hover:text-shPrimary"
            }`}
            title={opt.label}
          >
            {opt.value}
      ...
      <div ref={wrapRef} className="relative" data-testid={testid}>
        <button
          ...
          className={`w-full flex items-center justify-center gap-2 py-2 rounded-md text-[14px] font-black uppercase tracking-widest transition border ${
            open ? "bg-shPrimary/15 border-shPrimary text-shPrimary" : "bg-[var(--sh-card-base)] border-shBorder text-shTextMuted hover:border-shPrimary hover:text-shPrimary"
          }`}
        >
        ...
        {open && (
          <div
            ...
            className="absolute bottom-full left-0 right-0 mb-2 bg-[var(--sh-card-base)] border border-shBorder rounded-lg shadow-2xl p-3 z-50 animate-slide-in"
          >
            <p className="text-[12px] text-shTextMuted font-black uppercase tracking-widest mb-2">
              <i className="fas fa-text-height text-shPrimary mr-1.5" />Text Size
            </p>
            <Pills />
            <button
              type="button"
              ...
              className="w-full mt-3 text-[12px] text-shTextMuted hover:text-shText font-black uppercase tracking-widest"
            >
              Close
            </button>
          </div>
  ...
  return (
    <div data-testid={testid} className="space-y-2">
      <p className="text-[15px] text-shTextMuted font-black uppercase tracking-widest">
        <i className="fas fa-text-height mr-2 text-shPrimary" />
        Text Size
      </p>
      <Pills />
      <p className="text-[14px] text-shTextMuted leading-tight">
