    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-start justify-center p-4 pt-24" onClick={onClose} data-testid="global-search">
      <div className="bg-bgPanel border border-bgHover rounded-2xl w-full max-w-xl shadow-2xl animate-slide-in" onClick={(e)=>e.stopPropagation()}>
        <div className="flex items-center px-4 py-3 border-b border-bgHover">
          ...
          <kbd className="text-[15px] font-black uppercase tracking-widest text-gray-500 bg-bgBase border border-bgHover rounded px-2 py-1">ESC</kbd>
        </div>
        <div className="max-h-96 overflow-y-auto" data-testid="search-results">
          ...
          {results.dogs.length > 0 && (
            <div>
              <p className="px-4 pt-3 pb-1 text-[15px] font-black uppercase tracking-widest text-shGreen">Dogs · {results.dogs.length}</p>
                  ...
                  <button key={d.id} onClick={()=>onNavigate({...d, kind:"dog"})}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[14px] text-gray-500 font-black uppercase tracking-widest">{d.breed || "Unknown"} · {d.owner_name}</p>
                      </div>
                    </div>
          ...
          {results.clients.length > 0 && (
            <div>
              <p className="px-4 pt-3 pb-1 text-[15px] font-black uppercase tracking-widest text-shBlue">Clients · {results.clients.length}</p>
                  ...
                  <button key={c.id} onClick={()=>onNavigate({...c, kind:"client"})}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[14px] text-gray-500 font-black uppercase tracking-widest">{c.email || c.phone || "—"}</p>
                      </div>
                    </div>
                    <i className="fas fa-arrow-right text-gray-600" />
                  </button>
        ...
        <div className="px-4 py-2 border-t border-bgHover flex items-center justify-between text-[15px] font-black uppercase tracking-widest text-gray-500">
