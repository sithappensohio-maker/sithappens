      ...
      <div className="w-full max-w-xl my-8 bg-bgCard border border-shBorder rounded-2xl shadow-2xl" onMouseDown={(e)=>e.stopPropagation()} data-testid="financial-correction-modal">
        <div className="p-5 border-b border-shBorder flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="text-xl font-black text-shText mt-1">{booking.dog_name} · {booking.service_name || booking.service_type}</h2>
            <p className="text-sm text-shTextMuted mt-1">Charged {money(booking.actual_price)} · Paid {money(paidCash)} · Due {money(balanceDue)}{Number(booking.financial_refund_total || 0) > 0 ? ` · Refunded ${money(booking.financial_refund_total)}` : ""}</p>
          </div>
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              ...
              <button key={key} onClick={()=>setAction(key)} className={`rounded-lg border p-3 text-left ${action===key ? "border-shAccent bg-shAccent/10 text-shText" : "border-shBorder bg-[var(--sh-card-base)] text-shTextMuted hover:border-shAccent/50"}`}>
                <i className={`fas ${icon} mr-1`}/><span className="text-[12px] uppercase tracking-widest font-black">{label}</span>
              </button>
            ))}
          </div>

          <button onClick={()=>setAction("reopen")} className={`w-full rounded-lg border p-3 text-left ${action==="reopen" ? "border-shSecondary bg-shSecondary/10 text-shText" : "border-shBorder bg-[var(--sh-card-base)] text-shTextMuted hover:border-shSecondary/50"}`}>
          ...
          {action === "refund" && (
            <div>
              <label className="text-[11px] uppercase tracking-widest text-shTextMuted font-black">Refund method</label>
              <select value={method} onChange={(e)=>setMethod(e.target.value)} className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 text-shText">
          ...
          <div>
            ...
            <textarea rows={3} value={reason} onChange={(e)=>setReason(e.target.value)} placeholder="Explain exactly why this correction is needed."
                      className="w-full mt-1 bg-[var(--sh-card-base)] border border-shBorder rounded-lg p-3 text-shText resize-none focus:border-shAccent focus:outline-none" />
          </div>

          {error && <div className="bg-red-500/10 border border-red-500/40 text-red-300 rounded-lg p-3 text-sm font-bold">{error}</div>}

          <div className="bg-shSecondary/5 border border-shSecondary/25 rounded-lg p-3 text-xs text-shTextMuted">
            <i className="fas fa-shield-halved text-shSecondary mr-2"/>The original checkout remains in history. This action creates a separate audit event instead of rewriting or deleting the old record.
          </div>

          <div className="flex justify-end gap-2">
            <button onClick={onClose} className="px-4 py-2 rounded border border-shBorder text-shTextMuted font-black uppercase tracking-widest text-xs">Cancel</button>
            <button onClick={submit} disabled={!canSubmit || busy} className="px-5 py-2 rounded bg-shAccent text-black font-black uppercase tracking-widest text-xs disabled:opacity-40">
