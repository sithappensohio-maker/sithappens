...
export default function ReportCardModal({ booking, onClose, moodTags: moodTagsProp }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      onClose();
    } catch (e) { toast.error("Failed to save report card"); }
    setSaving(false);
  };

  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-3 sm:p-4 z-50">
      <div className="bg-bgPanel border border-bgHover rounded-2xl w-full max-w-2xl p-5 sm:p-8 shadow-2xl animate-slide-in max-h-[calc(var(--app-height)_-_2rem)] overflow-y-auto sh-modal-surface" data-testid="report-card-modal">
