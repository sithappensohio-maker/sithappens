...
export default function HomeworkIncentivesPanel() {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-shPrimary/40 bg-gradient-to-br from-shPrimary/10 via-bgPanel to-shAccent/10 p-5 shadow-2xl"
      ...
      <div className="relative">
        <div className="flex items-end justify-between flex-wrap gap-2 mb-4">
          <div>
            ...
            <h2 className="text-2xl font-black text-shText uppercase italic tracking-tight pr-1">Your Achievements.</h2>
          </div>
        </div>

      {/* Streak tier — the headline */}
      <div className="bg-[var(--sh-card-base)] rounded-lg p-4 mb-4 border border-shBorder" data-testid="incentives-streak">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            <p className="text-[12px] font-black text-shTextMuted uppercase tracking-widest">Current streak</p>
            <p className="text-4xl font-black text-shAccent mt-1" data-testid="incentives-streak-days">
              {data.streak_days} <span className="text-base text-shTextMuted font-bold normal-case">day{data.streak_days === 1 ? "" : "s"}</span>
            </p>
            {current && (
              <p className="text-[13px] text-shPrimary font-black mt-1" data-testid="incentives-current-milestone">
          ...
          {next && (
            <div className="text-right">
              <p className="text-[11px] font-black text-shTextMuted uppercase tracking-widest">Next milestone</p>
              <p className="text-[13px] text-shText font-black" data-testid="incentives-next-milestone">
                <span className="mr-1">{next.emoji}</span>{next.label}
              </p>
              <p className="text-[12px] text-shSecondary font-black">
          ...
          {ladder.map((rung) => {
            ...
            return (
              <div key={rung.threshold}
                   className={`text-center py-2 rounded transition ${reached ? "bg-shAccent/20 ring-1 ring-shAccent" : "bg-shBorder/40"}`}
                   title={`${rung.label} · ${rung.threshold} days`}>
                <div className={`text-lg ${reached ? "" : "grayscale opacity-40"}`}>{rung.emoji}</div>
                <div className={`text-[10px] font-black uppercase tracking-widest ${reached ? "text-shAccent" : "text-shTextMuted"}`}>
                  {rung.threshold}d
                </div>
              </div>
            );
      ...
      {earnedTrophies.length > 0 && (
        <div className="mb-4" data-testid="incentives-earned">
          <p className="text-[12px] font-black text-shTextMuted uppercase tracking-widest mb-2">Earned</p>
      ...
      {upcomingTrophies.length > 0 && (
        <div className="mb-4" data-testid="incentives-upcoming">
          <p className="text-[12px] font-black text-shTextMuted uppercase tracking-widest mb-2">Up next</p>
          <div className="space-y-2">
            {upcomingTrophies.map(t => (
              <div key={t.code} data-testid={`incentives-upcoming-${t.code}`}
                ...
                <div className="h-1.5 bg-shBorder rounded overflow-hidden">
                  <div className="h-full bg-shPrimary rounded transition-all"
                       style={{ width: `${t.pct}%` }}/>
                </div>
              </div>
            ))}
      ...
      {(data.certificates || []).length > 0 && (
        <div data-testid="incentives-certificates">
          <p className="text-[12px] font-black text-shTextMuted uppercase tracking-widest mb-2">
            <i className="fas fa-certificate text-shPrimary mr-1"/>Shareable certificates
          </p>
          <div className="space-y-2">
            {data.certificates.map(c => (
              <div key={c.homework_id}
                   data-testid={`incentives-cert-${c.homework_id}`}
                   className="bg-[var(--sh-card-base)] rounded-lg p-3 border border-shBorder flex items-center gap-3 flex-wrap">
                <i className="fas fa-graduation-cap text-shPrimary text-xl"/>
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-black text-shText truncate">{c.title || "Training Plan"}</p>
                ...
                <button onClick={() => share(c)}
                        ...
                        className="bg-shSecondary text-bgHeader px-4 py-2 rounded text-[12px] font-black uppercase tracking-widest hover:opacity-90 disabled:opacity-50">
...
function ReferralCard({ referral }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-shBorder" data-testid="incentives-referral">
      <p className="text-[12px] font-black text-shTextMuted uppercase tracking-widest mb-2">
        <i className="fas fa-user-plus text-shAccent mr-1"/>Refer a friend, both get a trophy
      </p>
      <div className="bg-gradient-to-br from-shAccent/10 to-shSecondary/10 rounded-lg p-4 border border-shAccent/30">
        <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
          <div>
            <p className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Your code</p>
            <p className="text-3xl font-black text-shAccent tracking-widest font-mono" data-testid="incentives-referral-code">{referral.code}</p>
          </div>
          <div className="text-right">
            <p className="text-[11px] font-black uppercase tracking-widest text-shTextMuted">Friends joined</p>
            <p className="text-3xl font-black text-shPrimary" data-testid="incentives-referral-count">{referral.successful_count}</p>
            {referral.current_milestone && (
              <p className="text-[12px] text-shAccent font-black">
          ...
          {(referral.ladder || []).map(rung => {
            ...
            return (
              <div key={rung.threshold}
                   className={`text-center py-2 rounded transition ${reached ? "bg-shAccent/20 ring-1 ring-shAccent" : "bg-shBorder/40"}`}>
                <div className={`text-lg ${reached ? "" : "grayscale opacity-40"}`}>{rung.emoji}</div>
                <div className={`text-[10px] font-black uppercase tracking-widest mt-0.5 ${reached ? "text-shAccent" : "text-shTextMuted"}`}>
                  {rung.threshold} ref{rung.threshold === 1 ? "" : "s"}
                </div>
                <div className={`text-[10px] ${reached ? "text-shText" : "text-shTextMuted"}`}>{rung.label}</div>
              </div>
            );
          })}
        </div>
        ...
        {referral.next_milestone && (
          <p className="text-[12px] text-shTextMuted mb-3 text-center">
            <span className="mr-1">{referral.next_milestone.emoji}</span>
            <span className="text-shSecondary font-black">{referral.next_milestone.left} more</span> to unlock <span className="text-shText font-black">{referral.next_milestone.label}</span>
          </p>
        )}

        <p className="text-[12px] text-shTextMuted italic mb-3">"{referral.share_text}"</p>

        {(referral.recent || []).length > 0 && (
          <div className="mb-3" data-testid="incentives-referral-recent">
            <p className="text-[11px] font-black uppercase tracking-widest text-shTextMuted mb-1">
              <i className="fas fa-users text-shPrimary mr-1"/>Friends you've brought in
            </p>
            <div className="space-y-1">
              {referral.recent.map((r, i) => (
                <div key={i} className="flex items-center justify-between gap-2 text-[12px]" data-testid={`incentives-referral-recent-${i}`}>
                  <span className="text-shText font-black truncate">
                    <i className="fas fa-paw text-shAccent mr-1"/>{r.first_name}
                  </span>
                  <span className="text-shTextMuted shrink-0">joined {timeAgo(r.joined_at)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex gap-2">
          <button onClick={copy}
                  ...
                  className="flex-1 bg-shSecondary text-bgHeader px-3 py-2 rounded text-[12px] font-black uppercase tracking-widest hover:opacity-90">
            <i className={`fas ${copied ? "fa-check" : "fa-clipboard"} mr-1`}/>{copied ? "Copied!" : "Copy link"}
          </button>
          <button onClick={nativeShare}
                  data-testid="incentives-referral-share"
                  className="flex-1 bg-shAccent text-bgHeader px-3 py-2 rounded text-[12px] font-black uppercase tracking-widest hover:opacity-90">
