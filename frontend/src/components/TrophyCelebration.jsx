  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/85 backdrop-blur grid place-items-center p-6 animate-fade-in" data-testid="trophy-celebration">
      <div className="bg-bgPanel border border-shOrange/50 rounded-2xl w-full max-w-md p-8 shadow-2xl text-center animate-slide-in">
        <div className="text-shOrange text-[11px] font-black uppercase tracking-widest mb-2">🎉 New Trophy Earned!</div>
        ...
        <div className="text-[12px] text-gray-400 mt-1">awarded to <span className="text-shGreen font-black">{a.recipient_name}</span></div>
        {a.trophy_description && <p className="text-[14px] text-gray-300 mt-4 leading-relaxed">{a.trophy_description}</p>}
        {a.note && <p className="text-[13px] bg-bgBase rounded p-3 mt-3 italic text-gray-300"><i className="fas fa-comment-dots mr-2 text-shBlue"/>{a.note}</p>}
        <button onClick={dismiss} data-testid="trophy-celebration-dismiss"
                className="mt-6 w-full bg-shOrange text-white py-3 rounded font-black text-[13px] uppercase tracking-widest shadow-lg hover:bg-shOrange/90">
