...
def test_completed_day_same_lesson_reopens_existing_pm(monkeypatch):
    async def _t():
        calls = []
...
# Stage 13 — this venv (and CI's requirements-test) has no pytest-asyncio; run the body on the shared test loop
def test_advanced_school_pointer_opens_current_lesson_supplemental(monkeypatch):
    async def _t():
        calls = []
        server = _server(calls)
        db = _DB(_booking())
...
# Stage 13 — this venv (and CI's requirements-test) has no pytest-asyncio; run the body on the shared test loop
def test_unfinished_day_still_uses_existing_slot_resolver(monkeypatch):
    async def _t():
        calls = []
        server = _server(calls)
        db = _DB(_booking())
