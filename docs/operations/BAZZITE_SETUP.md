...
- Your Resend API key (from your existing backend `.env`, if already configured).
...
If the CRM is already in GitHub, clone it:
...
You're looking at a fresh empty database. To import a backup from the old hosted instance:

1. From the old instance, go to Settings → Backup & Restore → Download Backup.
...
That's it. You've replaced the old hosted deployment with a PC under your desk. The CRM, the data, the URL — all yours.
