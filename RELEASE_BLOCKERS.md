...
| `test_dog_trivia.py`, `test_backup_coverage.py::test_backup_v3_contains_today_data` | Curated trivia seed was not guaranteed on a fresh database | Missing local seed path | No | App now seeds the bundled curated trivia pool locally when the active pool is thin; tests use the same curated seed | Covered by trivia + backup regression tests |
